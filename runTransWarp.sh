...
for TAG in "${warps[@]}"; do
  for v in "${VARIABLE[@]}"; do
    MIGRATION_FILE="MCXSecInputs_20230731_ME1A_NewBinning_NOMINAL.root"
    WARPED_FILE="MCXSecInputs_20230731_ME1A_NewBinning_${TAG}.root"

    TransWarpExtraction --output_file ${OUTFILE_NAME}Warping_${TAG}_${v}.root --data effnum_${v} --data_file $WARPED_FILE --data_truth effnum_${v}_true --data_truth_file $WARPED_FILE --migration migration_${v} --migration_file $MIGRATION_FILE --reco effnum_${v} --reco_file $MIGRATION_FILE --truth effnum_${v}_true --truth_file $MIGRATION_FILE --num_uni 500 --step_chi2 0.5 --num_iter 0,1,2,3,4,5,10,20,30,50,100,200 --log_scale -C 0.5 # --max_chi2 ${Ximaxaxis[${counter}]}
