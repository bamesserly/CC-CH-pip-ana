...
class IMCTrackTool;
class IElectronUtils;
class INeutronInelasticReweightTool;
//***veto wall**//
...
class MasterAnaDev : public MinervaAnalysisTool {
  ...
  */
  ...
  //ElasticFilterTool
  IElasticFilterTool*       m_elasticFilterTool;
  std::string               m_elasticFilterToolAlias;
  ...
  IBlobFitTool*             m_blobFitTool;
  std::string               m_blobFitToolAlias;
  //Tool used to correct the muon momentum for the passive target material
  IEnergyCorrectionTool*     m_energyCorrectionTool;


  IElectronUtils* m_electronUtils;

  INeutronInelasticReweightTool* m_neutronInelasticReweight; //Saves information to reweight events by their neutron inelastic channels
  ...
  //! Set up and fill truth branches for reweighting neutrons by their inelastic interaction channels
  void declareInelasticReweightBranches();
  void fillInelasticReweightBranches(Minerva::GenMinInteraction* truth) const;
