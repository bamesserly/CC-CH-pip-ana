...
public:
  ...
  MasterAnaDev(const std::string& type,
  ...
  StatusCode reconstructEvent(Minerva::PhysicsEvent* event,
                              Minerva::GenMinInteraction* truth = NULL) const;
...
protected:
  ...
  /*!
    @brief Get the target of this event after applying a Z cut that the point is
    within N(M) planes US(DS) of the target.
    @return A pointer to the NuclearTarget for which point event passes the z
    cuts
    @param[in] p The Point you are checking against target z positions
    @param[out] targetCode targetID*1000 + targetZ.  (targetZ of 0 means unkown
    material)
   */
  const Minerva::NuclearTarget* getTarget(const Gaudi::XYZPoint& p,
                                          int& targetCode) const;

  //! Overload of getTarget for GenMinInteraction
  const Minerva::NuclearTarget* getTarget(
    const Minerva::GenMinInteraction* truth,
    int& targetCode) const;
  ...
  /*!
    ...
    material)
   */
  const Minerva::NuclearTarget* getExactTarget(const Gaudi::XYZPoint& p,
                                               int& targetCode) const;

  //! Overload of getExactTarget for GenMinInteraction
  const Minerva::NuclearTarget* getExactTarget(
    const Minerva::GenMinInteraction* truth,
    int& targetCode) const;

  /*!
    @brief Adjust the vtx to be the point where the muon intersects the zCenter
    of a valid nuclear target
   ...
   */
  bool adjustVertexIntoPassiveTarget(
    SmartRef<Minerva::Vertex>& vtx,
    const SmartRef<Minerva::Particle>& muonPart) const;

  /*!
  @brief If this xy point were in passive targets 1,2..5 what would the Z of the
  material be?
 ...
 */
  ...
  /*!
    @brief If this xy point were in passive targets 1,2..5 what would be the
    smallest distance to the division of materials?
    @return true if everything worked
    @param[out] dVec A vector of what the distance to the closet division
    between materials would be in the passive target
    @param[in] x X position of the point
    @param[in] y Y position of the point
   */
  bool getReferenceDistToDivision(std::vector<double>& dVec,
                                  double x,
                                  double y) const;
  ...
  /*! @brief Vertex must be closer than this to the upstream edge of the
     NuclearTarget
      @property{USVertexCut, 75*mm}*/
  double m_USVertexCut;

  /*! @brief Vertex must be closer than this to the downstream edge of the
     NuclearTarget
      @property{DSVertexCut, 75*mm}*/
  double m_DSVertexCut;

  /*! @brief Use the nplanes away from target criteria instead of z distance
     from target.
...
private:
  ...
  */
  ...
  /*! @brief Do you want to use the CaloTuning from the CC inclusives in tracker
    analysis for the tracker events? If false, NukeCC's tuning for the tracker
    will be used.
  ...
  ////////////////////////////////////////////////////////RECOIL////////////////////
  IIDAnchoredBlobCreator* m_vtxBlobCreator; ///< Tool used to make vertex blob
                                            ///< to blind the vertex activuty
  ...
  IRecoilReconstructor*
    m_recoilReconstructor; ///< Reconstructs the recoil system energy
  ...
  std::vector<std::pair<int, int>> discrPairToPaddle(
    unsigned int minervaID) const;
  ...
  //--------------------------------------------------------------------faiza
  IAnalysisToolUtils*
    m_ccUtilsWideTimeWindow; ///< Tool to perform functions common to CC
                             ///< analyses with a wide time window for
                             ///< systematics

  //! Get a pass control tag <pass>_<ana_signature>_<tgt>
  inline std::string getPassTag(const std::string& tgt) const
  {
    return std::string("pass_" + m_anaSignature + "_" + tgt);
  };
  //! Get a pass control tag <pass>_<ana_signature>
  inline std::string getPassTag() const
  {
    return std::string("pass_" + m_anaSignature);
  };
  //! Select the appropriate channel and use RecoilUtils to get the recoil
  //! energy
  std::string getRecoilChannel(const int muonCharge,
                               const int targetID,
                               const int targetZ) const;
  double getRecoilEnergy(const Minerva::PhysicsEvent* event,
                         const SmartRef<Minerva::Prong>& muonProng,
                         const std::string& channel,
                         Minerva::NeutrinoInt* eventHyp = 0) const;
  double getvisibleenergy(const Minerva::PhysicsEvent* event,
                          const SmartRef<Minerva::Prong>& muon) const;
  StatusCode getBestParticle(SmartRef<Minerva::Prong> prong,
  ...
  Minerva::IDClusterVect getHadronIDClusters(
    const Minerva::PhysicsEvent* event,
    SmartRef<Minerva::Prong> muonProng,
    SmartRef<Minerva::Prong> excludeProng = (Minerva::Prong*)NULL) const;
  double calcOpeningAngle(Gaudi::LorentzVector& vec1,
                          Gaudi::LorentzVector& vec2) const;
  ...
  bool getProtonProngs(Minerva::ProngVect prongs,
                       Minerva::ProngVect& secondaryProtonProngs,
                       SmartRef<Minerva::Prong>& protonProng,
                       SmartRef<Minerva::Particle>& protonPart) const;

  void TruthFittedMichel(Minerva::PhysicsEvent* event,
                         Minerva::GenMinInteraction* truth) const;

  bool tagMichels(Minerva::PhysicsEvent* event,
                  Minerva::GenMinInteraction* truth) const;
  bool ImprovedtagMichels(Minerva::PhysicsEvent* event,
                          Minerva::GenMinInteraction* truth) const;

  bool hasTruthMichelElectron(Minerva::PhysicsEvent* event = NULL) const;

  void tagProtonProngTruth(Minerva::PhysicsEvent* event,
                           SmartRef<Minerva::Prong> protonProng) const;
  void tagSecondaryProtonProngTruth(Minerva::PhysicsEvent* event,
                                    Minerva::ProngVect protonProng) const;
  ...
  SmartRefVector<Minerva::IDCluster> getBlobbingClusters(
    Minerva::PhysicsEvent* event,
    const SmartRef<Minerva::Prong>& primMuonProng) const;

  //! Remove Used clusters from the input param, return the unused clusters
  SmartRefVector<Minerva::IDCluster> removeUsedClusters(
    SmartRefVector<Minerva::IDCluster>& clusters) const;

  //! Fill the event branches for a blob using its clusters
  //! Returns the energy of this blob's nucl, tracker, ecal clusters (ccqe
  //! recoil regions)
  double fillBlobBranches(Minerva::PhysicsEvent* event,
  ...
  SmartRefVector<ClusterType> getClustersCreatedBy(
    SmartRefVector<ClusterType> clusters,
    MCPartType::t_type primType,
    MCPartType::t_type fsType = MCPartType::kAny) const;
  //
  //! Get particle type that created this cluster - Stolen from CCQEAntiNuTool
  template<typename ClusterType, typename DigitType>
  MCPartType::t_type getParticleCreatedBy(SmartRef<ClusterType> cluster,
                                          MCPartStatus::t_type status) const;
  //

  bool createParticles(Minerva::PhysicsEvent* event,
  ...
  std::vector<double> tagDigitsTruth(
    SmartRefVector<Minerva::IDDigit> digits) const;
...
protected:
  SmartRefVector<Minerva::IDCluster> getAnalyzableNonMuIDClusters(
    ...
    bool useWideWindow = false) const;
  SmartRefVector<Minerva::ODCluster> getAnalyzableNonMuODClusters(
    const Minerva::PhysicsEvent* event,
    const SmartRef<Minerva::Prong>& muon,
    bool useWideWindow = false) const;

  bool adjustDNNVertexIntoCenterOfSegment(
    SmartRef<Minerva::Vertex>& vtx,
    double zCenter,
    const SmartRef<Minerva::Particle>& muonPart) const;
  //      void calcMomentumByRangeVars(Minerva::PhysicsEvent* event) const;
  //      void tagMichelElectrons(Minerva::PhysicsEvent* event) const;
  //      void getParticleRecoilE(const Minerva::PhysicsEvent*
  //      event,SmartRef<Prong> muonProng, double& proton, double& neutron,
  //      double& pion,   double& kaon, double& em, double& other,  double&
  //      xtalk) const;
  int GetTargetFromSegment(int segment, int& vtx_module, int& vtx_plane) const;
  void lonerVertex(Minerva::PhysicsEvent* event,
                   Minerva::Track* muTrack,
                   SmartRefVector<Minerva::IDCluster> allClusters,
                   double targetZEnd) const;
};
