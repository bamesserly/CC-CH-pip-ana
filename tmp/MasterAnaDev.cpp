...
int
  MAX_TRACKER_MODULE; //!< No events downstream of this module will be recorded
const int FIRST_TRACKER_MODULE = 23; //!< Lowest module number in the tracker

// these numbers were calculated as half the distance between adjacent plane's
// zCenter
const double PLANE_WIDTH =
  18.5 * CLHEP::mm; //!< longitudinal distance from start to end of a DePlane
const double MOD_GAP =
  6.235 * CLHEP::mm; //!< Air gap between plane2-1 of adjacent modules
const double MOD_GAP_NUKE =
  5.235 *
  CLHEP::mm; //!< Air gap between plane2-1 of adjacent modules in nuclear region
const double PLANE_GAP =
  1.98 * CLHEP::mm; //!< Air gap between plane1-2 in the same module
// Oficial -- DSCAL
...
//=============================================================================
MasterAnaDev::MasterAnaDev(const std::string& type,
  ...
  : MinervaAnalysisTool(type, name, parent)
{
  ...
  declareProperty("NuclearTargetToolAlias",
                  m_nukeToolAlias = "NukeCCInclusiveNukeTool");
  declareProperty("AddMyNuclearTargets", m_addMyNuclearTargets = true);
  declareProperty("UsePlaneZCut", m_usePlanesZCut = true);
  declareProperty("DSVertexPlanes",
                  m_nplanes_DSVertexLimit =
                    2); // allow vertices for 2 planes DS
  declareProperty("USVertexPlanes",
                  m_nplanes_USVertexLimit =
                    1); // allow vertices in the plane just US
  ...
  declareProperty("CCQEBindingE_Antinu",
                  m_antinuCCQEBindingE = 30. * CLHEP::MeV);
  ...
  //! Binding Energy
  declareProperty("NuCCQEBindingEnergyMeV",
                  m_nuCCQEBindingEnergyMeV = 34.0 * CLHEP::MeV);

  //! Making Short Tracks
  declareProperty("MakeShortTracks", m_makeShortTracks = true);
  declareProperty("MakeShortTracksvtxAnchoredShortTracker",
                  m_runVertexAnchoredShortTracker = true);
  declareProperty("DoRecursiveVertexAnchoredShortTracking",
                  m_doRecursiveVertexAnchoredShortTracking = true);
  declareProperty("MakeShortTracksvtxEnergyStudyTool",
                  m_runVertexEnergyStudyTool = true);
  ...
  declareProperty("VertexBlobRadii_Nu",
                  m_nuVtxBlobRadii = default_nuVtxBlobRadii);
  declareProperty("VertexBlobRadii_Antinu",
                  m_antinuVtxBlobRadii = default_antinuVtxBlobRadii);
  ...
  declareProperty("SecondaryProtonTagName",
                  m_secondaryProtons = "SecondaryProtons");
  ...
  declareProperty("FStyleMaxSearchDistance",
                  m_maxSearchDistance = 300.0 * CLHEP::mm); // mm
  declareProperty("FStyleMaxStartingDistance",
                  m_maxStartingDistance = 300.0 * CLHEP::mm); // mm
  declareProperty("FStyleMaxAllowedSearchGap",
                  m_maxAllowedSearchGap = 1e6 * CLHEP::mm); // mm

  //! For making Vertex Blob Prongs
  declareProperty("MakeFilamentStyleVtxBlobProngs",
                  m_makeFilamentStyleVtxBlobProngs = false);
  declareProperty("MakeMultipleRadiiStyleVtxBlobProngs",
                  m_makeMultipleRadiiStyleVtxBlobProngs = true);
  declareProperty("MaxAllowedSeparationBlobVertex",
                  m_maxSeparationBlobVertex = 300.0 * CLHEP::mm);

  //! Loose Fiducial truth Michel electron cut
  declareProperty("MichelDownstreamZ",
                  m_michel_downstreamZ =
                    8524.19 * CLHEP::mm); // mm // Module 83?
  declareProperty("MichelUpstreamZ",
                  m_michel_upstreamZ = 5900.91 * CLHEP::mm); // mm //Module 25?

  //! Cone Parameters
  declareProperty("FillTruthTG4ProtonCone",
                  m_fillTruthTG4ProtonCone = false); // mm
  ...
  declareProperty("IsolatedBlobProngColor",
                  m_isolatedBlobProngColor = 0xFF0000); // pure red
  declareProperty("DispersedEnergyColor",
                  m_dispersedEnergyColor = 0x00CCFF); // cyan
  declareProperty("VertexBlobProngColor",
                  m_vertexBlobProngColor = 0x996600);                 // brown
  declareProperty("ProtonProngColor", m_protonProngColor = 0x990000); // maroon
  declareProperty("HadronProngsColor",
                  m_hadronProngsColor = 0xFFFF00); // pure yellow
  ...
  declareProperty("MinervaObjAssocAlias",
                  m_minObjAssocAlias = "MASTERANADEVMinObjAssociator");
  declareProperty("RecoObjTimeToolAlias",
                  m_recoObjTimeToolAlias = "MASTERANADEVRecoObjTimeTool");
  declareProperty("MinervaCoordSysToolAlias",
                  m_minCoordSysToolAlias = "MASTERANADEVMinCoordSysTool");
  declareProperty("MinervaMathToolAlias",
                  m_mathToolAlias = "MASTERANADEVMinMathTool");

  declareProperty("MASTERANADEVRecoUtilsAlias",
                  m_masterAnaDevRecoUtilsAlias = "MASTERANADEVRecoUtilsTool");
  declareProperty("BlobCreatorUtilsAlias",
                  m_blobCreatorUtilsAlias = "MASTERANADEVBlobCreatorUtils");
  declareProperty("PrimaryBlobProngToolAlias",
                  m_primaryBlobProngToolAlias =
                    "MASTERANADEVPrimaryBlobProngTool");
  declareProperty("MichelToolAlias",
                  m_michelToolAlias = "MASTERANADEVMichelFinderTool");
  declareProperty("ImprovedMichelToolAlias",
                  m_improvedmichelToolAlias =
                    "MASTERANADEVImprovedMichelFinderTool");
  declareProperty("CalorimetryUtilsAlias",
                  m_caloUtilsAlias = "MASTERANADEVCaloUtils");
  declareProperty("ProtonUtilsAlias",
                  m_protonUtilsAlias = "MASTERANADEVProtonUtils");
  declareProperty("ParticleToolAlias",
                  m_particleMakerAlias = "MASTERANADEVParticleMaker");

  declareProperty("HitTaggerToolAlias",
                  m_hitTaggerToolAlias = "MASTERANADEVHitTaggerTool");

  declareProperty("NeutronBlobRecoToolAlias",
                  m_neutronBlobRecoToolAlias = "NeutronBlobRecoTool");
  declareProperty("NeutronBlobUtilsAlias",
                  m_neutronBlobUtilsAlias = "NeutronBlobUtils");
  ...
  declareProperty("MuonCylindarCut",
                  m_muonAxisCylinderRadius = 100.0 * CLHEP::mm);
  ...
  declareProperty("TrackPropagatorToVeto",
                  m_propagateToVetoName = "TrackLinearPropagator");
  declareProperty("RecoObjectTimeToolName",
                  m_timeToolName = "RecoObjectTimeTool");
...
MasterAnaDev::initialize()
{
  ...
  if (m_useDNN) {
    ...
    if (DBPredFile == NULL) {
      fatal() << "Failed to fail ML file =" << filename << "CHECK OPTION FILE!!"
              << endmsg;
    }
    dbPred = (TTree*)DBPredFile->Get("NukeCC");
    if (dbPred == NULL) {
      fatal() << "Failed to fail Get correct Tree Check option for NukeCC-> "
                 "CHECK OPTION FILE!! "
              << endmsg;
    }
  }
  ...
  try {
    m_nukeTool = tool<INuclearTargetTool>("NuclearTargetTool",
                                          m_nukeToolAlias); // will be shared
    if (m_addMyNuclearTargets) {
      debug() << "telling " << m_nukeToolAlias << " to add NuclearTargets"
              << endmsg;

      // Add passive targets
      m_nukeTool->addAllPassiveNuclearTargets();
    } else
      debug() << "Trusting NuclearTargetTool to add all the targets I need."
              << endmsg;
  ...
  if (fillPlaneLowZMap().isFailure())
    return StatusCode::FAILURE;

  // Get MinervaObjectAssociator
  try {
    m_objectAssociator = tool<IMinervaObjectAssociator>(
      "MinervaObjectAssociator", m_minObjAssocAlias);
  } catch (GaudiException& e) {
  ...
  try {
    m_recoObjTimeTool =
      tool<IRecoObjectTimeTool>("RecoObjectTimeTool", m_recoObjTimeToolAlias);
  } catch (GaudiException& e) {
  ...
  try {
    m_minCoordSysTool =
      tool<IMinervaCoordSysTool>("MinervaCoordSysTool", m_minCoordSysToolAlias);
  } catch (GaudiException& e) {
    error() << "Could not obtain tool: " << m_minCoordSysToolAlias << endmsg;
    return StatusCode::FAILURE;
  }
  try {
    m_vtxBlobCreator = tool<IIDAnchoredBlobCreator>("VertexBlobCreator",
                                                    "NukeCCVertexBlobCreator");
    debug() << "Got VertexBlobCreator named: " << m_vtxBlobCreator->name()
            << endmsg;
  } catch (GaudiException& e) {
  ...
  try {
    m_pathLengthTool =
      tool<IPathLengthTool>("PathLengthTool", "PathLengthTool");
  } catch (GaudiException& e) {
    error() << "Could not obtain tool: "
            << "PathLengthTool" << endmsg;
    return StatusCode::FAILURE;
  }
  try {
    m_LikelihoodPIDTool =
      tool<ILikelihoodParticleTool>("LikelihoodPIDTool", m_LPIDToolAlias);
  } catch (GaudiException& e) {
    error() << "Could not obtain LikelihoodPIDTool: " << m_LPIDToolAlias
            << endmsg;
    return StatusCode::FAILURE;
  }
  ...
  try {
    m_masterAnaDevRecoUtils = tool<IMasterAnaDevRecoUtils>(
      "MasterAnaDevRecoUtils", m_masterAnaDevRecoUtilsAlias);
  } catch (GaudiException& e) {
    error() << "Could not obtain tool: " << m_masterAnaDevRecoUtilsAlias
            << endmsg;
    return StatusCode::FAILURE;
  }
  ...
  try {
    m_blobCreatorUtils =
      tool<IBlobCreatorUtils>("BlobCreatorUtils", m_blobCreatorUtilsAlias);
  } catch (GaudiException& e) {
    error() << "Could not obtain BlobCreatorUtils: " << m_blobCreatorUtilsAlias
            << endmsg;
    return StatusCode::FAILURE;
  }
  ...
  try {
    m_primaryBlobProngTool = tool<IPrimaryBlobProngTool>(
      "PrimaryBlobProngTool", m_primaryBlobProngToolAlias);
  } catch (GaudiException& e) {
    error() << "Could not obtain tool: " << m_primaryBlobProngToolAlias
            << endmsg;
    return StatusCode::FAILURE;
  }
  ...
  try {
    m_energyCorrectionTool =
      tool<IEnergyCorrectionTool>("EnergyCorrectionTool");
    debug() << " Got EnergyCorrectionTool named: "
            << m_energyCorrectionTool->name() << endmsg;
  } catch (GaudiException& e) {
  ...
  /*  //same tool, just used differently in ReconStructEvent
    try {
        m_michelTrkTool =
    tool<IMichelTool>("MichelTool","CCPionMichelTrackTool"); }
    catch(GaudiException& e){ error()<<"Could not obtain tool: MichelTool" <<
    endmsg; return StatusCode::FAILURE;
      }
  */
  try {
    m_InnerDetector = getDet<Minerva::DeDetector>(
      "/dd/Structure/Minerva/Detector/InnerDetector");
  } catch (GaudiException& e) {
  ...
  try {
    m_particleMaker =
      tool<IParticleMakerTool>("ParticleMakerTool", m_particleMakerAlias);
  } catch (GaudiException& e) {
  ...
  try {
    m_hitTaggerTool =
      tool<IHitTaggerTool>("HitTaggerTool", m_hitTaggerToolAlias);
  } catch (GaudiException& e) {
  ...
  if (m_doNeutron) {
    ...
    try {
      m_neutronBlobRecoTool =
        tool<INeutronBlobRecoTool>(m_neutronBlobRecoToolAlias);
    } catch (GaudiException& e) {
      error() << "Could not obtain tool: " << m_neutronBlobRecoToolAlias
              << endmsg;
      return StatusCode::FAILURE;
    }
    try {
  ...
  try {
    m_VetoDet =
      getDet<Minerva::DeVetoDetector>("/dd/Structure/Minerva/VetoDetector");
  } catch (GaudiException& e) {
    error() << "Failed to retrieve Veto Wall Geometry. Perhaps this is the "
               "Frozen Detector?"
            << endmsg;
    return StatusCode::FAILURE;
  }
  ...
  if (!m_hasVeto) {
    ...
    warning() << "Failed to get Veto geometry" << endmsg;
  }
  ...
  try {
    m_CryoDet =
      getDet<Minerva::DeCryoTarget>("/dd/Structure/Minerva/CryoTarget");
  } catch (...) {
    warning() << "Failed to retrieve Cryo Target Geometry. Perhaps this is "
                 "Frozen Detector data?"
              << endmsg;
  }
  m_hasCryo = m_geomUtilSvc->hasCryo();
  if (!m_hasCryo) {
    warning() << "Failed to get Cryo-Target geometry" << endmsg;
  } else {
  }

  m_ccUtils = tool<IAnalysisToolUtils>("CCInclusiveUtils");
  m_ccUtilsWideTimeWindow =
    tool<IAnalysisToolUtils>("CCInclusiveUtils", "CCUtilsWideWindow");
  m_recoilReconstructor = tool<IRecoilReconstructor>("RecoilReconstructor");
  m_caloUtils = tool<ICalorimetryUtils>(
    "CalorimetryUtils",
    "RecoilCalorimetryUtils"); // same one as RecoilCalorimetryUtils
  //=========================================
  ...
  if (m_getLatticeEnergies) {
    try {
      ...
      debug() << " Got MLVF Sample Prep Tool named: " << m_MLVFTool->name()
              << endmsg;
    } catch (GaudiException& e) {
      error() << "Could not obtain MLVF Sample Prep tool: " << m_MLVFToolAlias
              << endmsg;
      return StatusCode::FAILURE;
    }
  ...
  if (!m_doVtxBlobSystematic && m_antinuVtxBlobRadii.size() > 1) {
    m_nuVtxBlobRadii.erase(m_nuVtxBlobRadii.begin() + 1,
                           m_nuVtxBlobRadii.end());
    m_antinuVtxBlobRadii.erase(m_antinuVtxBlobRadii.begin() + 1,
                               m_antinuVtxBlobRadii.end());
  }
  ...
  // declareBoolEventBranch( getPassTag() );                     //< isCC &&
  // is_fiducial && targetCode>0
  //---------------------------------------
  ...
  declareBoolTruthBranch(
    "reco_has_muon"); //< Is there a muon in this PhysicsEvent?
  //! Event - general reco
  declareBoolEventBranch(getPassTag()); //< isCC && is_fiducial && targetCode>0
  declareIntEventBranch("multiplicity", -1);

  // for active targets, record Z of the material at that vtx_XY in each passive
  // target
  declareContainerIntBranch(m_hypMeths, "ref_targZ", 5, -1);
  // for active targets,record distance in XY to the nearest edge of a material
  // if this XY were in each passive target
  ...
  //< How far is the vertex from the nearest edge of a DeTargetSection in the
  //NuclearTarget in XY?
  declareDoubleBranch(m_hypMeths, "target_dist_to_division", -1000.);
  declareIntBranch(m_hypMeths,
                   "in_fiducial_area",
                   0); //< Is the vertex in the region of interest in XY
  declareIntBranch(m_hypMeths,
                   "vtx_module",
                   -999); //< What module is this vertex in (if appropriate)
  declareIntBranch(m_hypMeths,
                   "vtx_plane",
                   -999); //< What plane is this vertex in (if appropriate)
  declareIntTruthBranch("vtx_module",
                        -999); //< What module is the true vertex in
  ...
  declareIntTruthBranch(
    "target_code", 0); //< This is targetID*1000 + targetZ.  Occasionally useful
  declareIntTruthBranch("targetID",
                        0); //< True event is in nuclear target with this ID
  declareIntTruthBranch(
    "targetZ", 0); //< True event is in a nuclear target material with this Z
  declareIntTruthBranch(
    "in_fiducial_area",
    0); //< Is the true vertex in the region of interest in XY
  ...
  declareContainerDoubleEventBranch(
    "event_extra_track_PID"); // will store pid values from tracks not in the
                              // interaction vertex.
  ...
  declareContainerDoubleEventBranch(
    "vtx_blobs_iso_energy_clusters_outside_radius_in_prong");
  ...
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_start_position_x_in_prong");
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_start_position_y_in_prong");
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_start_position_z_in_prong");
  declareContainerDoubleEventBranch("nonvtx_iso_blobs_time_in_prong");
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_time_difference_in_prong");
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_lowest_module_x_in_prong");
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_lowest_module_u_in_prong");
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_lowest_module_v_in_prong");
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_highest_module_x_in_prong");
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_highest_module_u_in_prong");
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_highest_module_v_in_prong");
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_earliest_hit_time_in_prong");
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_latest_hit_time_in_prong");
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_highest_hit_energy_in_prong");
  ...
  declareContainerIntEventBranch(
    "nonvtx_iso_blobs_primary_particle_pdg_in_prong");
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_matched_energy_fraction_in_prong");
  declareContainerDoubleEventBranch(
    "nonvtx_iso_blobs_data_energy_fraction_in_prong");
  ...
  declareContainerDoubleEventBranch(
    "clusters_found_at_end_proton_prong_max_distance");
  ...
  // for Machine Learning
  declareDoubleEventBranch("muon_trackVertexTime",
                           -9999.); // needed by ML m_MLVFTool
  ...
  declareContainerDoubleEventBranch(
    "vtx_blob_radius", m_nuVtxBlobRadii.size(), 0.);

  /////////////////////////////////////////////////////////////////////////////////////////
  //-----------------------------------------------------
  //! Veto Wall Branches
  //-----------------------------------------------------
  declareIntEventBranch("VetoWall_NumberMatchToVeto",
                        0); // number of tracks in vtx options
  declareContainerDoubleEventBranch("VetoWall_muon_extrapVetoXY", 4, -999);
  declareContainerIntEventBranch(
    "VetoWall_PMTStatusMAP", 24, 0); // adding pandel configuration
  ...
  declareDoubleBranch(
    m_hypMeths, "Q2_Inclusive", -9999.0); // Q2 for Inclusive analysis
  ...
  declareIntBranch(m_hypMeths,
                   "target_code",
                   0); //< This is targetID*1000 + targetZ.  Occasionally useful
  declareIntBranch(
    m_hypMeths,
    "targetZ",
    0); //< Event was reconstructed into a nuclear target material with this Z
  declareIntBranch(
    m_hypMeths,
    "targetID",
    0); //< Event was reconstructed in nuclear target with this ID
  ...
  declareContainerDoubleBranch(
    m_hypMeths, "hadron_tm_endpt_fraction", 10, -1.0);
  ...
  declareContainerDoubleEventBranch(
    "matched_michel_fit_residual", MAX_TRACKS, -9999);

  declareContainerIntEventBranch(
    "matched_michel_avg_module", MAX_TRACKS, -9999);
  declareContainerIntEventBranch(
    "matched_michel_end_module1", MAX_TRACKS, -9999);
  declareContainerIntEventBranch(
    "matched_michel_end_module2", MAX_TRACKS, -9999);
  ...
  declareContainerDoubleEventBranch(
    "FittedMichel_all_piontrajectory_momentumx");
  declareContainerDoubleEventBranch(
    "FittedMichel_all_piontrajectory_momentumy");
  declareContainerDoubleEventBranch(
    "FittedMichel_all_piontrajectory_momentumz");
  ...
  declareContainerDoubleEventBranch(
    "FittedMichel_all_muontrajectory_momentumx");
  declareContainerDoubleEventBranch(
    "FittedMichel_all_muontrajectory_momentumy");
  declareContainerDoubleEventBranch(
    "FittedMichel_all_muontrajectory_momentumz");
  ...
  declareContainerDoubleEventBranch(
    "FittedMichel_all_micheltrajectory_momentum");
  declareContainerDoubleEventBranch(
    "FittedMichel_all_micheltrajectory_momentumx");
  declareContainerDoubleEventBranch(
    "FittedMichel_all_micheltrajectory_momentumy");
  declareContainerDoubleEventBranch(
    "FittedMichel_all_micheltrajectory_momentumz");
  ...
  declareContainerDoubleEventBranch(
    "FittedMichel_all_micheltrajectory_initialx");
  declareContainerDoubleEventBranch(
    "FittedMichel_all_micheltrajectory_initialy");
  declareContainerDoubleEventBranch(
    "FittedMichel_all_micheltrajectory_initialz");
  ...
  declareContainerDoubleEventBranch(
    "FittedMichel_true_primaryparent_momentumx");
  declareContainerDoubleEventBranch(
    "FittedMichel_true_primaryparent_momentumy");
  declareContainerDoubleEventBranch(
    "FittedMichel_true_primaryparent_momentumz");
  ...
  declareContainerIntEventBranch(
    "FittedMichel_reco_micheltrajectory_ParentPDG");
  declareContainerIntEventBranch("FittedMichel_reco_micheltrajectory_pdg");
  declareContainerDoubleEventBranch(
    "FittedMichel_reco_micheltrajectory_energy");
  declareContainerDoubleEventBranch(
    "FittedMichel_reco_micheltrajectory_momentum");
  declareContainerDoubleEventBranch(
    "FittedMichel_reco_micheltrajectory_momentumx");
  declareContainerDoubleEventBranch(
    "FittedMichel_reco_micheltrajectory_momentumy");
  declareContainerDoubleEventBranch(
    "FittedMichel_reco_micheltrajectory_momentumz");
  declareContainerDoubleEventBranch(
    "FittedMichel_reco_micheltrajectory_finalx");
  declareContainerDoubleEventBranch(
    "FittedMichel_reco_micheltrajectory_finaly");
  declareContainerDoubleEventBranch(
    "FittedMichel_reco_micheltrajectory_finalz");
  declareContainerDoubleEventBranch(
    "FittedMichel_reco_micheltrajectory_initialx");
  declareContainerDoubleEventBranch(
    "FittedMichel_reco_micheltrajectory_initialy");
  declareContainerDoubleEventBranch(
    "FittedMichel_reco_micheltrajectory_initialz");
  ...
  declareContainerDoubleBranch(
    m_hypMeths, "piFit_score1_Mass_biasUp", 10, -1.0);
  declareContainerDoubleBranch(
    m_hypMeths, "piFit_score1_Mass_biasDown", 10, -1.0);
  declareContainerDoubleBranch(
    m_hypMeths, "piFit_score1_BetheBloch_biasUp", 10, -1.0);
  declareContainerDoubleBranch(
    m_hypMeths, "piFit_score1_BetheBloch_biasDown", 10, -1.0);
  ...
  declareContainerDoubleBranch(
    m_hypMeths, "pion_E_BetheBloch_biasUp", 10, -1.0);
  declareContainerDoubleBranch(
    m_hypMeths, "pion_E_BetheBloch_biasDown", 10, -1.0);
  ...
  declareContainerDoubleBranch(m_hypMeths,
                               "sec_protons_score1_BetheBloch_biasUp");
  declareContainerDoubleBranch(m_hypMeths,
                               "sec_protons_score1_BetheBloch_biasDown");
  ...
  declareDoubleBranch(
    m_hypMeths, "recoil_E", -1.); //< Calorimetric recoil energy
  declareDoubleBranch(
    m_hypMeths,
    "visible_E",
    -1.); //< Recoil Energy with no calorimetric correction at all
  declareDoubleBranch(m_hypMeths,
                      "recoil_passivecorrected",
                      -1.); //< Recoil Energy with a passive matrl correction
  declareDoubleBranch(
    m_hypMeths,
    "recoil_E_wide_window",
    -1.); //< Calorimetric recoil energy using an extra wide time window
  ...
  if (m_doNeutron) {
    ...
    debug() << "BlobIntBranches Size = "
            << m_neutronBlobUtils->GetIntDataMemberNames().size() << endmsg;
    for (uint i = 0; i < m_neutronBlobUtils->GetIntDataMemberNames().size();
         i++)
      declareContainerIntBranch(m_hypMeths,
                                m_neutronBlobUtils->GetIntDataMemberNames()[i]);
    for (uint i = 0; i < m_neutronBlobUtils->GetDoubleDataMemberNames().size();
         i++)
      declareContainerDoubleBranch(
        m_hypMeths, m_neutronBlobUtils->GetDoubleDataMemberNames()[i]);
  }
...
//=============================================================================
StatusCode
MasterAnaDev::reconstructEvent(Minerva::PhysicsEvent* event,
                               Minerva::GenMinInteraction* truth) const
{
  ...
  if (getDAQHeader()->isMCTrigger()) {
    debug() << truthData() << " Truth Process Type " << truth->processType()
            << endmsg;
  }
  //========================================================================//
  // for Machine Learning
  ...
  // This fills a map which associates discriminatorPairs to the time when they
  // had their 6th push
  ...
  for (Minerva::VetoDigits::iterator v = vdigits->begin(); v != vdigits->end();
       ++v) {
  ...
  // Will tell us what the earliest time is that 6 pushes from the Veto Wall
  // occured and on what paddle
  if (sixPushes) {
    debug() << "Found a sixth push at time: " << earliestSixPushTime << endmsg;

    // Fill Paddle vectors with the dead paddles from both walls
    for (std::vector<std::pair<int, int>>::iterator it = WallPad.begin();
         it != WallPad.end();
         it++) {
      info() << "veto (wall,paddle) in saturated discriminator = (" << it->first
             << "," << it->second << ")" << endmsg;
      if (it->first == 1) {
        wall1Paddles.push_back(it->second);
      }
      if (it->first == 2) {
        wall2Paddles.push_back(it->second);
      }
    }
  ...
  const std::vector<const Minerva::DeVetoWall*> walls =
    m_VetoDet->getDeVetoWalls();
  ...
  for (itWall = walls.begin(); itWall != walls.end(); ++itWall) {
    ...
    for (int j = 1; j <= m_NumVetoPaddles; j++) {
      ...
      Minerva::VetoPMTID pmtid_1(
        Minerva::DetectorID::Veto, wall_number, j, m_VetoPaddlePMT1);
      Minerva::VetoPMTID pmtid_2(
        Minerva::DetectorID::Veto, wall_number, j, m_VetoPaddlePMT2);
      debug() << " Geting PMT ID -  For Wall_number: " << wall_number
              << "  Paddle:" << j << endmsg;
      int veto_1 = -999;
      int veto_2 = -999;
      // for a given PMT ID - check the pmt status returns true if status is
      // good
      ...
      if (veto_1 == -999) {
        warning() << "Failure:: getPMTStatus_hw() = " << veto_1
                  << " for PMTID1 = " << pmtid_1 << endmsg;
      }
      if (veto_2 == -999) {
        warning() << "Failure:: getPMTStatus_hw() = " << veto_2
                  << " for PMTID2 = " << pmtid_2 << endmsg;
      }

      debug() << "Geting PMT Status for pmtid_1( " << pmtid_1
              << " ) = " << veto_1 << endmsg;
      debug() << "Geting PMT Status for pmtid_2( " << pmtid_2
              << " ) = " << veto_2 << endmsg;
      ...
      if (veto_1 == false || veto_2 == false) {
        ...
        paddle_statusMap[j] = true;
        paddle_statusMap_1[std::make_pair(wall_number, j)] = true;
      }

      debug() << "wall_number = " << wall_number << "j =" << j
              << "  paddle_statusMap_1[make_pair(wall_number,j)] = "
              << paddle_statusMap_1[std::make_pair(wall_number, j)] << endmsg;
    }
  }
  ...
  for (int i = 0; i < m_totalVETOPMTS; i++) {
    ...
    debug() << "intputs of Map index= " << i
            << " Input =  " << counterVETOMAP[i] << endmsg;
  }

  event->setContainerIntData("VetoWall_PMTStatusMAP", MapVector);

  //! Skip events with a truth pointer whose processType is insufficiently
  //! defined The Truth pointer will be NULL while processing data
  //-----------------------------------------------------------------------------------
  if (truth &&
      (truth->processType() == Minerva::GenMinInteraction::kNoProcess ||
       truth->processType() == Minerva::GenMinInteraction::kUnknownProcess)) {
    debug() << "Truth pointer points to an insufficiently defined process - no "
               "sense in proceeding !"
            << endmsg;
    return StatusCode::SUCCESS;
  }
  ...
  if (truth) {
    if (exist<Minerva::GenMinHeader>(Minerva::GenMinHeaderLocation::Default)) {
      genMinHeader =
        get<Minerva::GenMinHeader>(Minerva::GenMinHeaderLocation::Default);
    } else {
  ...
  if (event->filtertaglist()->isFilterTagTrue(AnaFilterTags::BadObject())) {
    error()
      << "Found an event flagged with a BadObject! Refusing to analyze ..."
      << endmsg;
    counter("REFUSED_A_BADOBJECT") += 1;
    return StatusCode::SUCCESS; // Things are bad, but we didn't crash.
  }
  if (truth)
    truth->filtertaglist()->setOrAddFilterTag("reco_hasGoodObjects", true);
  //--------------------------------------------------------------------
  ...
  VertexVect unused_vtx =
    event->select<Minerva::Vertex>("Unused", "StartPoint");
  ...
  for (Minerva::VertexVect::iterator itvtx = used_vtx.begin();
       itvtx != used_vtx.end();
       ++itvtx) {
    debug() << "  Vertex Position          = " << (*itvtx)->position()
            << endmsg;
    allvtxtimes.push_back((*itvtx)->getTime());
    ProngVect primProngs;
    ProngVect unattachedProngs;
    m_objectAssociator->getProngs_fromSourceVertex(
      primProngs, unattachedProngs, *itvtx);

    ProngVect minosTrackProngs, minosStubProngs;
    minosTrackProngs.clear();
    minosStubProngs.clear();

    int isFV = m_minCoordSysTool->inFiducial((*itvtx)->position().X(),
                                             (*itvtx)->position().Y(),
                                             (*itvtx)->position().Z(),
                                             850.0,
                                             6127,
    ...
    for (ProngVect::const_iterator itProng = primProngs.begin();
         itProng != primProngs.end();
         ++itProng) {
      if ((*itProng)->MinosTrack())
        minosTrackProngs.push_back(*itProng);
      else if ((*itProng)->MinosStub())
        minosStubProngs.push_back(*itProng);
    }

    if (!minosTrackProngs.empty()) {
      allvtxtimes_minos_match.push_back((*itvtx)->getTime());
      allvtxfv_minos_match.push_back(isFV);
      debug() << " Found a vertex with " << minosTrackProngs.size()
              << " MinosTrack Prong(s)..." << endmsg;
      ++count_matches;
  ...
  event->setContainerDoubleData("all_event_start_vertex_time_minos_match",
                                allvtxtimes_minos_match);
  event->setContainerIntData("all_event_start_vertex_fv_minos_match",
                             allvtxfv_minos_match);
  ...
  Minerva::IDClusterVect sliceclusters =
    event->select<Minerva::IDCluster>("All", "!LowActivity&!XTalkCandidate");
  std::vector<double> slice_hit_time;
  std::vector<double> slice_hit_energy;
  int slice_n_hits = 0;
  for (Minerva::IDClusterVect::iterator itsc = sliceclusters.begin();
       itsc != sliceclusters.end();
       itsc++) {
    SmartRefVector<Minerva::IDDigit> digits = (*itsc)->digits();
    for (SmartRefVector<Minerva::IDDigit>::const_iterator itDig =
           digits.begin();
         itDig != digits.end();
         ++itDig) {
  ...
  truth &&
    truth->filtertaglist()->setOrAddFilterTag("reco_has_int_vtx", has_vtx);
  ...
  if (!has_vtx) {
    ...
    // if(fv_count==0)
    return StatusCode::SUCCESS; // none of the minos matches orginate from
                                // vertices in the FV or there were no minos
                                // matches. Both cases we don't care about.
  ...
  if (truth)
    truth->filtertaglist()->setOrAddFilterTag("reco_isGoodVertex", true);
  //---------------------------------------------------------------
  //! Get the interaction vertex
  //! If you can't get the vertex, tag the event as a BadObject
  //---------------------------------------------------------------
  SmartRef<Minerva::Vertex> vertexOfPhysEvent =
    (event->interactionVertex()).target();
  ...
  */
  ...
  if (truth) {
    ...
    Gaudi::XYZPoint vtx_smear(
      vtx_position.x() + m_randomGen->Gaus(0.0, 0.91 * CLHEP::mm),
      vtx_position.y() + m_randomGen->Gaus(0.0, 1.25 * CLHEP::mm),
      vtx_position.z() + m_randomGen->Gaus(0.0, 10.0 * CLHEP::mm));
    if (FiducialPointTool->isFiducial(
          vtx_smear, m_fidHexApothem, m_fidUpStreamZ, m_fidDownStreamZ)) {
      truth->filtertaglist()->setOrAddFilterTag("reco_isFidVol_smeared", true);
    } else
      truth->filtertaglist()->setOrAddFilterTag("reco_isFidVol_smeared", false);
  }
  counter("is_fiducial") += is_fiducial;
  /*  if( !FiducialPointTool->isFiducial(vtx_position, m_fidHexApothem,
    m_fidUpStreamZ, m_fidDownStreamZ ) ) { debug() << "Event vertex is outside
    fiducial volume" << endmsg; return StatusCode::SUCCESS;
    }*/
  ...
  if (truth) {
    truth->filtertaglist()->setOrAddFilterTag("reco_muon_charge",
                                              (muon_charge != 0));
  ...
  if (muonPart->score() >= m_minMuonScore) {
    muonProng->filtertaglist()->setOrAddFilterTag(AnaFilterTags::PrimaryMuon(),
                                                  true);

    if (muonProng->MinosTrack())
      is_minos_track = true;
    if (muonProng->MinosStub())
      is_minos_stub = true;
    if (is_minos_stub && is_minos_track)
      counter("MuonHasMinosStubAndTrack")++;
    if (!is_minos_stub && !is_minos_track)
      counter("MuonIsNotMinosMatched")++;

    event->filtertaglist()->setOrAddFilterTag("isMinosMatchTrack",
                                              is_minos_track);
    event->filtertaglist()->setOrAddFilterTag("isMinosMatchStub",
                                              is_minos_stub);
    if (truth) {
      truth->filtertaglist()->setOrAddFilterTag("reco_isMinosMatchTrack",
                                                is_minos_track);
      truth->filtertaglist()->setOrAddFilterTag("reco_isMinosMatchStub",
                                                is_minos_stub);
  ...
  // should be used, but for other analyses, the value might need to be
  // different.
  ...
  if (muonProng && m_useDNN) {
    std::string Cuts;
    if (truth)
      Cuts = Form("run==%d && subrun==%d && gate==%d && slice_number==%d",
                  genMinHeader->RunNumber(),
                  genMinHeader->SubRunNumber(),
                  truth->NumEventInFile(),
                  event->sliceNumbers()[0]);
    else
      Cuts = Form("run==%d && subrun==%d && gate==%d && slice_number==%d",
                  header->runNumber(),
                  header->subRunNumber(),
                  header->gateNumber(),
                  event->physicsEventNumber());

    // Draw with option goff and generate three variables
    debug() << "DNN Cuts = " << Cuts << endmsg;
    // first segment predictions
    Int_t n = dbPred->Draw(
      "ANN_best_segment:ANN_best_prediction", Cuts.c_str(), "goff");
    ...
    if (n == 1) {
      ...
      info() << "Found prediction: DNN segment, DNN plane probs = "
             << DNN_segment << " " << DNN_plane_probs << endmsg;
      ANN_segments.push_back(DNN_segment);
      ANN_plane_probs.push_back(DNN_plane_probs);
    } else {
      if (truth)
        error() << "No ML Prediction for this event: "
                << genMinHeader->RunNumber() << ", "
                << genMinHeader->SubRunNumber() << ", "
                << truth->NumEventInFile() << ", " << event->sliceNumbers()[0]
                << endmsg;
      else
        error() << "No ML Prediction for this event: " << header->runNumber()
                << ", " << header->subRunNumber() << ", "
                << header->gateNumber() << ", " << event->physicsEventNumber()
                << endmsg;
      return StatusCode::SUCCESS;
    }
  ...
  //--------------------------------------------------------------------------------------------------------------------
  // Find chi2 of MINERvA part (not MINOS) of muon track (for comparisons with
  // J. Wolcott's chiPerDoF for electrons)
  //--------------------------------------------------------------------------------------------------------------------
  ...
  if (foundMinervaPart && muonTrack->nNodes()) {
    verbose() << "Chi2 of the MINERvA part of the muon track is "
              << muonTrack->chi2PerDoF() << " Nodes: " << muonTrack->nNodes()
              << endmsg;
    event->setDoubleData("muon_minerva_trk_chi2PerDoF",
                         muonTrack->chi2PerDoF());
  }

  // Get muontime
  double muonTime =
    m_recoObjTimeTool->trackVertexTime(muonProng->minervaTracks()[0]);
  //---------------------------------------------------
  //! Store all node angles to fix angular biases due to hadronic energy near
  //! the vertex (docdb 12047)
  //---------------------------------------------------
  ...
  //----------------------------------------------------------------------------------------
  //! Add PID to prongs not associated with the interaction vertex, but are in
  //! time
  //----------------------------------------------------------------------------------------
  //----------------------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------
  // Now we're going to check if the muon Prong was matched in time to the veto
  // wall
  if ((muonProng)->Veto()) {

    found_VetoMatchedTrk = true;
    debug() << "FOUND A MUON PRONG THAT WAS MATCHED IN TIME TO THE VETO WALL"
            << endmsg;

  } // if muonProng is veto matched in time

  // -------------------------------------------------------------------
  // Lets get the veto wall extrapolated location from the prong, regardless of
  // a match in time Fill arrays with information stored in the prong, about the
  // veto track extrapolation WallHit: 0 to 3.  0 = no walls, 1 = only Wall 1, 2
  // = only Wall 2, 3 = both Walls

  if ((muonProng)->hasIntData("WallHit"))
    wallHit = (muonProng)->getIntData("WallHit");
  if ((muonProng)->hasDoubleData("WallOneXPosition"))
    xPos[0] = (muonProng)->getDoubleData("WallOneXPosition");
  if ((muonProng)->hasDoubleData("WallOneYPosition"))
    yPos[0] = (muonProng)->getDoubleData("WallOneYPosition");
  if ((muonProng)->hasDoubleData("WallTwoXPosition"))
    xPos[1] = (muonProng)->getDoubleData("WallTwoXPosition");
  if ((muonProng)->hasDoubleData("WallTwoYPosition"))
    yPos[1] = (muonProng)->getDoubleData("WallTwoYPosition");

  // if this track has wallHit = 0 it didn't land on any walls, so skip (in
  // which case, how do we get the filter set?)
  if (wallHit == 0)
    event->filtertaglist()->setOrAddFilterTag("VetoWall_extrapnowalls", true);
  ...
  // -------------------------------------------------------------------
  // Now we're going to check if the muon track time was after a sixth Push from
  // a discriminator on the veto wall
  if (sixPushes) {

    double earlytracktime = 99999999.9;

    SmartRefVector<Minerva::Track> minervaTracks = (muonProng)->minervaTracks();
    for (SmartRefVector<Minerva::Track>::iterator itTrack =
           minervaTracks.begin();
         itTrack != minervaTracks.end();
         itTrack++) {
      double time = m_timeTool->trackVertexTime(*itTrack);
      debug() << "Track time is " << time << endmsg;
      if (time < earlytracktime) {
        earlytracktime = time;
        debug() << "Updating earliest track time to " << earlytracktime
                << endmsg;
      }
    ...
    // first veto wall is ~3m away from front face of MINERvA, gives a 1ns time
    // difference, any more detail is smaller than our timing resolution
    earlytracktime -= (3 / CLHEP::c_light);

    debug() << "The earliest track time (TOF corrected) = " << earlytracktime
            << " and the earliest 6 push time = " << earliestSixPushTime
            << endmsg;

    if (earlytracktime > earliestSixPushTime) {
      debug() << "This event likely happened in the veto wall dead time, let's "
                 "check the paddle extrap"
              << endmsg;
      ...
      if (muonProng->hasIntData("WallOnePaddleExtrap")) {
        ...
        if ((wall1Paddles[0] ==
             (muonProng->getIntData("WallOnePaddleExtrap"))) ||
            (wall1Paddles[1] ==
             (muonProng->getIntData("WallOnePaddleExtrap")))) {
          sixPushes_1 = true;
      ...
      if (muonProng->hasIntData("WallTwoPaddleExtrap")) {
        if ((wall2Paddles[0] ==
             (muonProng->getIntData("WallTwoPaddleExtrap"))) ||
            (wall2Paddles[1] ==
             (muonProng->getIntData("WallTwoPaddleExtrap")))) {
          sixPushes_2 = true;
      ...
      if (sixPushes_1 == true && sixPushes_2 == true) {
        sixPush_matchtrack = 1;
      } else {
        sixPush_matchtrack = 0;
      }
  ...
  // ******************************************
  // Zero out quantities here. This way something sensible gets written to the
  // TES even if we don't have a candidate track.
  ...
  for (itWall = walls.begin(); itWall != walls.end(); ++itWall) {
    ...
    m_propagateToVeto->propagate(
      muonTrack, wall_z + m_PanelOffset, positive_point_in_wall);
    m_propagateToVeto->propagate(
      muonTrack, wall_z - m_PanelOffset, negative_point_in_wall);

    // ******************************************
    // First query the geometry to see if the point is any paddle, try positive
    // offset paddle first.
    try {
    ...
    if (retry) {
      try {
        ...
        debug() << "Point outside Veto Wall, Moving on to the next wall"
                << endmsg;
        debug() << " .. pos point x, y = " << positive_point_in_wall.x() << ","
                << positive_point_in_wall.y() << endmsg;
        debug() << " .. neg point x, y = " << negative_point_in_wall.x() << ","
                << negative_point_in_wall.y() << endmsg;
        continue;
      }
    ...
    if (overlap) {
      ...
      // if in overlap and have wrong difference between paddle numbers, exit
      // this prong!
      if (-1 != paddleDiff && 1 != paddleDiff) {
        warning() << "Difference between overlaping paddles is not 1!!!"
                  << endmsg;
        warning() << "Diference between overlaping paddles is " << paddleDiff
                  << endmsg;
        continue;
      }

      // after we've exited the prong if bad delta paddle - so now we have an
      // overlap situation, where delta paddle is valid
    }
    ...
    if (overlap) {
      debug() << "Track also extrapolate to Second Paddle: "
              << SecondPaddleEx.paddle() << endmsg;
      overlap_paddlevector.push_back(SecondPaddleEx.paddle());
      extrapolate_Veto.push_back(
        std::make_pair(wall_number, SecondPaddleEx.paddle()));
    }
    // ******************************************
    ...
    if ((overlap) ||
        (!overlap && !retry)) { // if overlap is false and retry is false, found
                                // a point only in pos z
    ...
    } else if (!overlap && retry) { // if overlap is false and retry is true,
                                    // found a point only in neg z
  ...
  if (extrapVeto == true) {
    if (countwallveto == 2) {
      event->setContainerDoubleData("VetoWall_muon_extrapVetoXY",
                                    XandYextrapolate_VETO);
    ...
    for (std::vector<std::pair<int, int>>::const_iterator it =
           extrapolate_Veto.begin();
         it != extrapolate_Veto.end();
         ++it) {

      // Using the list of Paddle ©that were extrapolated too and see what
      // corrpening Paddle_map of veto status.
      pmtOfforOn = paddle_statusMap_1[std::make_pair(it->first, it->second)];
      debug() << "it->first " << it->first << "it->second = " << it->first
              << "   paddle_statusMap_1[std::make_pair(it->first,it->first)]= "
              << paddle_statusMap_1[std::make_pair(it->first, it->second)]
              << endmsg;
    ...
    if (pmtOfforOn == false) {
      ...
      event->filtertaglist()->setOrAddFilterTag(
        "VetoWall_MuonTrkMatchToVETOwalloff", true);
    } else {
      // didn't match to map a paddle that was off
      event->filtertaglist()->setOrAddFilterTag(
        "VetoWall_MuonTrkMatchToVETOwalloff", false);
    }
  ...
  Minerva::ProngVect extraProngs =
    m_masterAnaDevRecoUtils->getInTimeTrackPID(event, muonTime);
  ...
  for (Minerva::ProngVect::iterator itProng = extraProngs.begin();
       itProng != extraProngs.end();
       ++itProng) {
    debug() << "Make particles from extra prongs" << endmsg;
    IParticleMakerTool::NameAliasListType toolsToUse;
    toolsToUse.push_back(
      std::make_pair("dEdXTool", "dEdXTool")); // name, alias pair

    StatusCode sc_dEdX =
      m_particleMaker->makeParticles(*itProng, particleHypotheses, toolsToUse);
    if (!sc_dEdX) {
  ...
  bool hasextraProton = getProtonProngs(
    extraProngs, extrasecondaryProtonProngs, extraprotonProng, extraprotonPart);
  // now get PID scores for each prong (only save the equivalent of score1 which
  // is used for protons associated with the primary vtx
  // TODO: add systematic variations into the flow....
  if (hasextraProton) {
    extraPID.push_back(
      extraprotonPart->getDoubleData(ParticleExtraDataDefs::dEdXScore1()));
    for (Minerva::ProngVect::iterator itProng =
           extrasecondaryProtonProngs.begin();
         itProng != extrasecondaryProtonProngs.end();
         ++itProng) {
  ...
  //! Does it make sense to refit the Vtx reliably with such short tracks ? Not
  //! sure
  //-------------------------------------------------------------------------------------
  ...
  //-------------------------------------------------------------------------------
  Minerva::IDClusterVect idClusterVector =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableIDClusters(event, muonProng);
  if (!idClusterVector.size()) {
    debug() << "Size of vector containing ID Clusters is "
            << idClusterVector.size()
            << " - nothing to make short Tracks from !" << endmsg;
  }
  event->setIntData("ID_Hits_Post_Long", idClusterVector.size());

  //-------------------------------------------------------------------------------------------
  //! A vector for holding all the newly made short tracks (from the clusters
  //! near the Vtx)
  //-------------------------------------------------------------------------------------------
  std::vector<Minerva::Track*>* deShortTrackVector =
    new std::vector<Minerva::Track*>;

  //----------------------------------------------------------------------------------------------------
  //! Make short tracks either with the VertexAnchoredShortTracker or
  //! VertexEnergyStudyTool or both
  //----------------------------------------------------------------------------------------------------
  if (m_makeShortTracks && idClusterVector.size() != 0) {
    sc =
      m_masterAnaDevRecoUtils->runShortTrackers(event,
                                                deShortTrackVector,
                                                m_runVertexAnchoredShortTracker,
                                                m_runVertexEnergyStudyTool);
    if (sc.isFailure())
      return sc;
  }
  ...
  //---------------------------------------------------------------------
  // for (std::vector<Minerva::Track*>::iterator itTrk
  // =deShortTrackVector->begin(); itTrk != deShortTrackVector->end(); ++itTrk)
  // { delete *itTrk; }
  deShortTrackVector->clear();
  delete deShortTrackVector;

  int n_anchored_short_trk_prongs =
    event->primaryProngs().size() - n_anchored_long_trk_prongs - 1;
  ...
  SmartRefVector<Minerva::Track> Tracks =
    vertexOfPhysEvent->getOutgoingTracks();
  debug() << "Outgoing tracks size =" << Tracks.size() << endmsg;
  for (SmartRefVector<Minerva::Track>::iterator itTrack = Tracks.begin();
       itTrack != Tracks.end();
       ++itTrack) {
    verbose() << "Track address = " << *itTrack
              << " Track Pat Rec History = " << (*itTrack)->patRecHistory()
              << " Track type = " << (*itTrack)->type()
              << " Track nodes = " << (*itTrack)->nNodes()
              << " Track VisE = " << (*itTrack)->visibleEnergy()
              << " Track theta = " << (*itTrack)->theta()
              << " Track phi = " << (*itTrack)->phi()
              << " Track first cluster time = "
              << (*itTrack)->firstNode()->idcluster()->time()
              << " Track time = " << (*itTrack)->time() << endmsg;
  }

  int fitConverged = 0;
  event->interactionVertex()->getIntData(VertexExtraDataDefs::FitConverged(),
                                         fitConverged);

  /////////////////////////////////////////////////////////////////////////////////////////////////
  // Adjust the vertex position and muon momentum to the center of a passive
  ...
  if (has_vtx && has_muon && muonProng && muonPart && !m_useDNN) {
    ...
    if (1 <= targetCode && targetID <= 5) {
      ...
      // then change the vertex position at the position where the muon
      // intersects the zCenter of the relevant passive target.
      if (!fitConverged) {
        debug()
          << " This is a passive target event without a good vertex fit.  I "
             "will adjust the vertex position to the center of the target."
          << endmsg;
        SmartRef<Minerva::Vertex> primVtx = event->interactionVertex();
        adjustVertexIntoPassiveTarget(primVtx, muonPart);
      }

      // Correct muon momentum for muon energy loss in passive targets if event
      // is in passive target sample.
      debug() << " This is a passive target event, I will adjust muon energy"
              << endmsg;
      double vertexZ = event->interactionVertex()->position().z();
      Gaudi::LorentzVector muonfourVec;
      bool corrOK =
        m_energyCorrectionTool
          ->getCorrectedEnergy(muonProng, muonPart, vertexZ, muonfourVec)
          .isSuccess();
      ...
      if (corrOK) {
        ...
        debug() << "  Delta           = "
                << (muonfourVec - muonPart->momentumVec()) << endmsg;
        vector<double> corrP;
  ...
  //==============================for Machine
  //Learning====================================//
  // Adjust the vertex position and muon momentum to the center of a segment
  // where applicable
  if (has_vtx && has_muon && muonProng && muonPart && m_useDNN) {
    for (int targetid = 1; targetid < 6; targetid++) {
      Minerva::NuclearTarget* tgt = m_nukeTool->getNuclearTarget(targetid);
      verbose() << "***** targetID, z start, z center, z end = "
                << tgt->getTargetID() << " " << tgt->getZStart() << " "
                << tgt->getZCenter() << " " << tgt->getZEnd() << endmsg;
    }
    ...
    if (DNN_vtx_modules.size() > 0)
      DNN_vtx_modules.clear();
    if (DNN_vtx_planes.size() > 0)
      DNN_vtx_planes.clear();

    if (1 <= ANN_segments[0] &&
        ANN_segments[0] <= 214) // modified from 174 to 214
    {
      ...
      double DNN_z_center;
      targetID =
        GetTargetFromSegment(ANN_segments[0], DNN_vtx_module, DNN_vtx_plane);
      error() << "first error targetID=" << targetID
              << "ANN_segments=" << ANN_segments
              << "DNN_vtx_module = " << DNN_vtx_module
              << "DNN_vtx_plane=" << DNN_vtx_plane << endmsg;
      ...
      if (targetID == -1) {
        ...
        for (vector<const Minerva::DePlane*>::const_iterator itPlane =
               planes.begin();
             itPlane != planes.end();
             ++itPlane) {

          if (DNN_vtx_module == (*itPlane)->getPlaneID().module() &&
              DNN_vtx_plane == (int)((*itPlane)->getPlaneID().plane())) {
            DNN_z_center = (*itPlane)->getZCenter();
            debug() << "plane, module, z center = " << DNN_vtx_module << ", "
                    << DNN_vtx_plane << ", " << DNN_z_center << endmsg;
          }
        }
      } else {
        debug() << "NukeCC getNuclearTarget and get the z center of target "
                << targetID << endmsg;
        Minerva::NuclearTarget* target = m_nukeTool->getNuclearTarget(targetID);
        if (0 == target) {
          warning() << "Cannot project to target with targetID = " << targetID
                    << " because the targetID does not exist" << endmsg;
          return 0;
        }
        // set the offset for the target positions here:
        if (targetID == 1)
          DNN_z_center = target->getZCenter() - 2.3;
        if (targetID == 2)
          DNN_z_center = target->getZCenter() - 3.5;
        if (targetID == 3)
          DNN_z_center = target->getZCenter();
        if (targetID == 4)
          DNN_z_center = target->getZCenter() - 3.4;
        if (targetID == 5)
          DNN_z_center = target->getZCenter() - 3.05;
        if (targetID == 6)
          DNN_z_center = 5305.3;
        debug() << "DNN z center = " << DNN_z_center << endmsg;
      }

      error() << "second error targetID=" << targetID
              << "ANN_segments=" << ANN_segments
              << "DNN_vtx_module = " << DNN_vtx_module
              << "DNN_vtx_plane=" << DNN_vtx_plane << endmsg;
      SmartRef<Minerva::Vertex> primVtx = event->interactionVertex();
      ...
      // target_id = GetTargetFromSegment( DNN_segment, DNN_vtx_module,
      // DNN_vtx_plane );
      DNN_vtx_modules.push_back(DNN_vtx_module);
      DNN_vtx_planes.push_back(DNN_vtx_plane);

      // error() << "DNN_segment=" << DNN_segment << "DNN_vtx_module = " <<
      // DNN_vtx_module << "DNN_vtx_plane=" << DNN_vtx_plane << endmsg;
      event->setContainerIntData("ANN_vtx_modules", DNN_vtx_modules);
      event->setContainerIntData("ANN_vtx_planes", DNN_vtx_planes);
    }

    // Correct muon momentum for muon energy loss in passive targets if event is
    // in passive target sample. for Muon Energy correction by Huma
    debug() << " This is a machine learning predicted event, I will adjust "
               "muon energy"
            << endmsg;
    double vertexZ = event->interactionVertex()->position().z();
    Gaudi::LorentzVector muonfourVec;
  ...
  event->filtertaglist()->setOrAddFilterTag("hasNoImprovedMichelElectrons",
                                            !hasImprovedMichels);
  ...
  for (unsigned int prong = 0; prong < primary_prongs.size(); ++prong) {
    if (primary_prongs[prong]->filtertaglist()->filterTagExists(
          "PrimaryHadron") &&
        !primary_prongs[prong]->filtertaglist()->filterTagExists(
          "PrimaryMuon")) {
  ...
  /*  double muonE = muonPart->momentumVec().E();
    ...
    double MECAna_recoilE = RecoilUtils->calcRecoilEFromClusters( event,
    muonProng, recoilChannel); double totalE = muonE + MECAna_recoilE; double
    qsquared   = PhysicsCalculator->qSquared( totalE, muonE, muon_theta );
  ...
  */
  ...
  //! Michel Electron
  //---------------------------------------------------
  /*for (ProngVect::iterator itProngs = primaryProngs.begin(); itProngs !=
  primaryProngs.end(); ++itProngs){
  ...
  const SmartRefVector<Minerva::Vertex> tmp_vertices =
    event->select<Minerva::Vertex>();
  for (SmartRefVector<Minerva::Vertex>::const_iterator itVtx =
         tmp_vertices.begin();
       itVtx != tmp_vertices.end();
       ++itVtx) {
    double vertex_time =
      (*itVtx)->getTime(); // m_recoObjTimeTool->trackVertexTime((*itVtx));
    ...
    if (time_diff > m_lowerTimeWindow && time_diff < m_upperTimeWindow)
      vtx_types_intime.push_back((*itVtx)->type());
  ...
  const SmartRefVector<Minerva::Track> tmp_tracks =
    event->select<Minerva::Track>();
  for (SmartRefVector<Minerva::Track>::const_iterator itTrack =
         tmp_tracks.begin();
       itTrack != tmp_tracks.end();
       ++itTrack) {
    ...
    if (time_diff > m_lowerTimeWindow && time_diff < m_upperTimeWindow)
      track_energy_intime.push_back((*itTrack)->type());
  ...
  event->setContainerDoubleData("event_in_time_tracks_energy",
                                track_energy_intime);
  ...
  for (Minerva::IDClusterVect::iterator itsc = sliceclusters.begin();
    ...
    for (Minerva::IDClusterVect::iterator muit = muonClusters.begin();
         muit != muonClusters.end();
         muit++) {
      if ((*muit) == (*itsc)) {
    ...
    if (nonMuonCluster)
      mod_slice_clusters.push_back(*itsc);
  }
  ...
  for (int low = -20; low != 0; low++) {
    for (int high = 35; high != 0; high--) {
      ...
      for (Minerva::IDClusterVect::iterator itsc = mod_slice_clusters.begin();
           itsc != mod_slice_clusters.end();
           itsc++) {
        ...
        if (time_diff > low && time_diff < high) {
          ...
          for (itDig = clusDigits.begin(); itDig != clusDigits.end(); itDig++) {
            if ((*itDig)->strip() <= 3 || (*itDig)->strip() >= 124)
              selectedclustersedge.push_back(*itsc);
          }
        }
      }
      std::vector<const Minerva::IDCluster*> idclusters;
      idclusters.insert(
        idclusters.begin(), selectedclusters.begin(), selectedclusters.end());
      if (truth)
        recoil_data_fraction.push_back(
          TruthMatcher->getDataFraction(idclusters));
      else
        recoil_data_fraction.push_back(-1);
      // now apply constants
      double rec_energy =
        m_caloUtils->applyCalConsts(selectedclusters, "Default", false);
      double rec_energy_edge =
        m_caloUtils->applyCalConsts(selectedclustersedge, "Default", false);
      // now store
  ...
  event->setContainerIntData("recoil_lower_time_limit",
                             recoil_lower_time_limit);
  event->setContainerIntData("recoil_upper_time_limit",
                             recoil_upper_time_limit);
  event->setContainerDoubleData("recoil_data_fraction", recoil_data_fraction);
  event->setContainerDoubleData("recoil_summed_energy_edge",
                                recoil_summed_energy_edge);
  ...
  if (muonProng) {
    ...
    SmartRefVector<Minerva::IDCluster> blobbingClusters =
      getBlobbingClusters(event, muonProng);
    ...
    SmartRefVector<Minerva::IDCluster> isoBlobbedClusters =
      removeUsedClusters(blobbingClusters);
    ...
    SmartRefVector<Minerva::IDCluster> idClusters =
      getAnalyzableNonMuIDClusters(event, muonProng);
    SmartRefVector<Minerva::ODCluster> odClusters =
      getAnalyzableNonMuODClusters(event, muonProng);
    ...
    allBlobbedClusters.insert(allBlobbedClusters.end(),
                              isoBlobbedClusters.begin(),
                              isoBlobbedClusters.end());
  } // done blobbing
  ...
  bool hasProton = getProtonProngs(
    hadronProngs, secondaryProtonProngs, protonProng, protonPart);
  ...
  if (hasProton) {
    ...
    if (truth)
      tagProtonProngTruth(event, protonProng);
  }
  debug() << "Number of secondary proton prongs = "
          << secondaryProtonProngs.size() << endmsg;
  ...
  if (hasProton) {
    ...
    sc = m_masterAnaDevRecoUtils->makeConeAtEndProtonTrack(
      event,
      protonProng,
      muonProng,
      foundClusters,
      max_cluster_distance,
      idClustersInsideCone,
      m_fillTruthTG4ProtonCone);

    verbose()
      << "Number of clusters inside cone in MasterAnaDev.cpp (hasProton=true) "
      << idClustersInsideCone.size() << endmsg;
    verbose() << "Value of foundClusters (hasProton=true) " << foundClusters
              << endmsg;

    //----------------------------------------------------------------------------------
    // Find total visible energy of clusters present at end of Primary Proton
    // track
    //----------------------------------------------------------------------------------
    for (Minerva::IDClusterVect::iterator itClust =
           idClustersInsideCone.begin();
         itClust != idClustersInsideCone.end();
         ++itClust) {
      totalVisEAtProtonTrackEnd += (*itClust)->energy();
    }

    //-----------------------------------------------------------------------------------
    // Find total calibrated energy of clusters present at end of Primary Proton
    // track
    //-----------------------------------------------------------------------------------
    double calibEAtProtonTrackEnd =
      m_caloUtils->applyCalConsts(idClustersInsideCone, "Default", false);
    ...
    m_objectAssociator->getVertex_fromTrackBack(
      BackVertex, protonProng->minervaTracks()[0]);
    double protonTrackLength = -9999.;
    if (BackVertex) {
      const Gaudi::XYZPoint protonTrackEndPos = BackVertex->position();
      protonTrackLength = m_mathTool->distanceBetween(
        vertexOfPhysEvent->position(), protonTrackEndPos);
    ...
    for (Minerva::IDClusterVect::iterator itClust =
         ...
         itClust != idClustersInsideCone.end();
         ++itClust) {
      if (0 <= m_coneEnergyColor)
        m_hitTaggerTool->applyColorTag(*itClust, m_coneEnergyColor);
    }
  ...
  if (secondaryProtonProngs.size() > 0) {
    ...
    for (ProngVect::iterator secProtonProng = secondaryProtonProngs.begin();
         secProtonProng != secondaryProtonProngs.end();
         ++secProtonProng) {
      bool foundClusters = false;
      double max_cluster_distance = -9999.;
      Minerva::IDClusterVect idClustersInsideCone;
      double totalVisEAtProtonTrackEnd = 0;
      sc =
        m_masterAnaDevRecoUtils->makeConeAtEndProtonTrack(event,
                                                          *secProtonProng,
                                                          muonProng,
                                                          foundClusters,
                                                          max_cluster_distance,
      ...
      verbose() << "Number of clusters inside cone in MasterAnaDev.cpp "
                   "(secondaryProtonProngs.size()>0) "
                << idClustersInsideCone.size() << endmsg;
      verbose() << "Value of foundClusters (secondaryProtonProngs.size()>0) "
                << foundClusters << endmsg;

      //-----------------------------------------------------------------------------------
      // Find total visible energy of clusters present at end of Secondary
      // Proton track
      //-----------------------------------------------------------------------------------
      for (Minerva::IDClusterVect::iterator itClust =
             idClustersInsideCone.begin();
      ...
      //-------------------------------------------------------------------------------------
      // Find total calibrated energy of clusters present at end of Secondary
      // Proton track
      //-------------------------------------------------------------------------------------
      double calibEAtProtonTrackEnd =
        m_caloUtils->applyCalConsts(idClustersInsideCone, "Default", false);
      ...
      maxDistanceClusterFromEndOfProtonProng.push_back(max_cluster_distance);
      numOfClustersAtEndOfProtonProng.push_back(idClustersInsideCone.size());
      visEnergyOfClustersAtEndOfProtonProng.push_back(
        totalVisEAtProtonTrackEnd);
      calibEnergyOfClustersAtEndOfProtonProng.push_back(calibEAtProtonTrackEnd);
    ...
    if (truth)
      tagSecondaryProtonProngTruth(event,
                                   secondaryProtonProngs); // tag secondaries!!
  ...
  //---------------------------------------------------------------
  event->setContainerIntData("clusters_found_at_end_proton_prong",
                             clustersFoundAtEndOfProtonProng);
  event->setContainerDoubleData(
    "clusters_found_at_end_proton_prong_max_distance",
    maxDistanceClusterFromEndOfProtonProng);
  event->setContainerIntData("number_clusters_at_end_proton_prong",
                             numOfClustersAtEndOfProtonProng);
  event->setContainerDoubleData("visE_clusters_at_end_proton_prong",
                                visEnergyOfClustersAtEndOfProtonProng);
  event->setContainerDoubleData("calibE_clusters_at_end_proton_prong",
                                calibEnergyOfClustersAtEndOfProtonProng);

  //----------------------------------------------------------------------------------------------------------------
  //! Blobbing Stuff
  //! JO has chosen to make VertexBlobProngs first, then add EM Blobs to
  //! MuonProng, then make IsoBlobProngs Reason for this is to pick out the
  //! recoil "near the Vertex" first, attach "muon-related brems/deltas" to the
  //! MuonProng second, then determine the "non-Vertex" recoil. Kenyi - please
  //! let me know if you disagree with this !
  //----------------------------------------------------------------------------------------------------------------

  //----------------------------------------------------------------------------------------------------
  //! Create Vertex Blob Prongs
  //! User can choose to make VtxBlobProngs either "Filament Style" or "Multiple
  //! Radii Style"
  // V.V.I. - The vector of clusters 'idClusterVector' contains clusters "in
  // time" with the muon V.V.I. - Is this okay ? What if there is an out-of-time
  // overlay activity right near the vertex V.V.I. - INVESTIGATE !
  //----------------------------------------------------------------------------------------------------

  // Minerva::ProngVect newVtxBlobProngs;
  // if( (RecoilUtils->createVtxBlobProngs( event, newVtxBlobProngs
  // )).isFailure() ) return StatusCode::FAILURE;

  debug() << "Creating vertex blob prongs" << endmsg;
  std::vector<Minerva::Prong*>* newVtxBlobProngs =
    new std::vector<Minerva::Prong*>;
  if (m_makeFilamentStyleVtxBlobProngs) {
    sc = m_primaryBlobProngTool->makeFilamentStyleVtxBlobProngs(
      event,
      idClusterVector,
      vertexOfPhysEvent,
      m_maxSearchDistance,
      m_maxStartingDistance,
      m_maxAllowedSearchGap,
      m_maxSeparationBlobVertex,
      newVtxBlobProngs);

  } else if (m_makeMultipleRadiiStyleVtxBlobProngs) {
    sc = m_primaryBlobProngTool->makeMultipleRadiiStyleVtxBlobProngs(
      event,
      idClusterVector,
      vertexOfPhysEvent,
      m_searchStepSize,
    ...
    info() << "You have not specified a preference for making VtxBlobProngs, "
              "please specify one if you would like Vertex Blob Prongs !"
           << endmsg;
  }
  ...
  debug() << "Found " << newVtxBlobProngs->size()
          << " new Vertex Blob Prongs ....." << endmsg;
  ...
  //-----------------------------------------------------------------------
  if (newVtxBlobProngs->size())
    addObject(event, *newVtxBlobProngs);
  ...
  for( std::vector<Minerva::Prong*>::iterator p=newVtxBlobProngs->begin();
  p!=newVtxBlobProngs->end(); ++p ) { event->promoteProngToPrimary( *p );
  }
  */
  ...
  //-----------------------------------------------------------------
  for (std::vector<Minerva::Prong*>::iterator itProng =
         newVtxBlobProngs->begin();
       itProng != newVtxBlobProngs->end();
       ++itProng) {
    if (0 <= m_vertexBlobProngColor)
      m_hitTaggerTool->applyColorTag(*itProng, m_vertexBlobProngColor);
  }
  ...
  //! Create primary Blob Prongs - NOT SURE IF WE SHOULD RUN THIS GENERAL METHOD
  //! NOW !
  ...
  //! V.V.I. - We have noticed events (10200/69/521, 10203/54/601) where there
  //! is activity from overlay in the time slice. However this is out-of-time
  //! (i.e. -20, 35 ns) with the muon time. This activity will not be blobbed if
  //! we stick to "in time" clusters only. Hence we are resorting to collecting
  //! "ALL UNUSED" clusters (except XTalk & LowAct) in the event, for blobbing
  //! purposes. For event selection, we cut only on the blobs that are in time
  //! with the neutrino interaction we are interested in.
  //------------------------------------------------------------------------------------------------------------------------------
  debug() << "Create Shower Blob Prongs from remaining \"unused\" clusters"
          << endmsg;

  std::vector<Minerva::Prong*>* newIsoIdProngs =
    new std::vector<Minerva::Prong*>;

  Minerva::IDClusterVect unusedClustsForBlobs =
    event->select<Minerva::IDCluster>("Unused", "!LowActivity&!XTalkCandidate");

  sc = m_primaryBlobProngTool->makeShowerBlobProngs(
    event, unusedClustsForBlobs, newIsoIdProngs);
  if (sc.isFailure())
    return sc;
  ...
  //-----------------------------------------------------------------
  for (std::vector<Minerva::Prong*>::iterator itProng = newIsoIdProngs->begin();
       itProng != newIsoIdProngs->end();
       ++itProng) {
    if (0 <= m_isolatedBlobProngColor)
      m_hitTaggerTool->applyColorTag(*itProng, m_isolatedBlobProngColor);

    debug() << "Prong Type " << (*itProng)->type() << endmsg;

    SmartRefVector<Minerva::IDCluster> prongClusts =
      (*itProng)->getAllIDClusters();
    for (SmartRefVector<Minerva::IDCluster>::iterator itClust =
           prongClusts.begin();
         itClust != prongClusts.end();
         ++itClust) {
  ...
  debug() << "Found " << newIsoIdProngs->size()
          << " new Isolated ID Blob Prongs ....." << endmsg;

  //--------------------------------------------------------------------------
  //! You can add other conditions here, for examining the ShowerBlobProngs
  //! before you start adding them to the evtMgr, user has control !
  //--------------------------------------------------------------------------
  if (newIsoIdProngs->size())
    addObject(event, *newIsoIdProngs);
  ...
  //! a certain (spheric?) region from the vertex to still be defined (30cm
  //! radius?)
  ...
  Minerva::ProngVect prongs =
    event->select<Minerva::Prong>("Used:Unused", "All");
  debug() << "Event has now " << prongs.size() << " prongs" << endmsg;
  /// check point
  for (Minerva::ProngVect::iterator p = prongs.begin(); p != prongs.end();
       p++) {
    debug() << "Prong Creation Signature = " << (*p)->creationSignature()
            << endmsg;
    // Get calibrated vertex energy
    if (((*p)->creationSignature() ==
         PrimaryBlobProngExtraDataDefs::InteractionVtxBlobProngName()) ||
        ((*p)->creationSignature() ==
         PrimaryBlobProngExtraDataDefs::StartPointVtxBlobProngName())) {

      double calE = m_caloUtils->applyCalConsts(*p, "Default", false);
      vtx_blobs_energy += calE;
      debug() << "Found a " << (*p)->creationSignature()
              << ", nblobs: " << (*p)->idblobs().size() << ", energy: " << calE
              << " MeV" << endmsg;

      // loop over blobs in prong
      Minerva::IDBlobVect idBlobs = (*p)->idblobs();
      for (Minerva::IDBlobVect::iterator itBlob = idBlobs.begin();
           itBlob != idBlobs.end();
           itBlob++) {
        if ((*itBlob)->patRecHistory() == Minerva::IDBlob::VertexBlobPatRec) {
        ...
        } else if ((*itBlob)->patRecHistory() ==
                   Minerva::IDBlob::IsolatedIDBlobPatRec) {
          ...
          /*
          const Gaudi::XYZPoint blob_pos_us =
          m_blobCreatorUtils->calcUpstreamBlobStartPoint(*itBlob); const
          Gaudi::XYZPoint blob_pos_ds =
          m_blobCreatorUtils->calcDownstreamBlobStartPoint(*itBlob); const
          Gaudi::XYZPoint vtx_pos = vertexOfPhysEvent->position(); double
          distance1 = m_mathTool->distanceBetween(blob_pos_us, vtx_pos); double
          distance2 = m_mathTool->distanceBetween(blob_pos_ds, vtx_pos); double
          distance = (distance1<distance2)? distance1 : distance2;
          isoBlobDistanceInVtxBlobProng.push_back(distance);
          debug()<<"Iso vertex blob US (x,y,z) = "<< blob_pos_us.x() << ", "<<
          blob_pos_us.y()<<", "<< blob_pos_us.z()<< endmsg; debug()<<"Iso vertex
          blob DS (x,y,z) = "<< blob_pos_ds.x() << ", "<< blob_pos_ds.y()<<",
          "<< blob_pos_ds.z()<< endmsg; debug()<<"vertex position (x,y,z) = "<<
          vtx_pos.x() << ", "<< vtx_pos.y()<<", "<< vtx_pos.z()<< endmsg;
          debug()<<"distance iso vertex blob US from vertex="<<
          distance1<<endmsg; debug()<<"distance iso vertex blob DS from vertex
          ="<< distance2<<endmsg; debug()<<"distance iso vertex blob minimum
          from vertex="<< distance<<endmsg; debug()<<"distance iso vertex blob
          default start point ="<< (*itBlob)->startPoint()<<endmsg;
          */

          debug() << "Vertex Iso Blob centroid position X,Z = "
                  << ((*itBlob)->energyCentroidXZ()).first << ", "
                  << ((*itBlob)->energyCentroidXZ()).second << endmsg;
          debug() << "Vertex Iso Blob centroid position U,Z = "
                  << (*itBlob)->energyCentroidUZ().first << ", "
                  << (*itBlob)->energyCentroidXZ().second << endmsg;
          debug() << "Vertex Iso Blob centroid position V,Z = "
                  << (*itBlob)->energyCentroidVZ().first << ", "
                  << (*itBlob)->energyCentroidXZ().second << endmsg;
          //! Also, calculate the energy of the clusters from these blobs that
          //! are outside the sphere of radius: m_searchStepSize
          SmartRefVector<Minerva::IDCluster> vtxisoblobClusters =
            (*itBlob)->clusters();
          SmartRefVector<Minerva::IDCluster> vtxisoblobClusters_outside;
          SmartRefVector<Minerva::IDCluster>::iterator itCluster =
            vtxisoblobClusters.begin();
          double minclus_distance = 1e6;
          for (; itCluster != vtxisoblobClusters.end(); itCluster++) {
            double dist = m_mathTool->twoDDistance(
              *itCluster, vertexOfPhysEvent->position());
            if (minclus_distance > dist)
              minclus_distance = dist;
          ...
          debug() << "Distance iso vertex blob min cluster = "
                  << minclus_distance << endmsg;
          double clusE_outside = m_caloUtils->applyCalConsts(
            vtxisoblobClusters_outside, "Default", false);
    ...
    //! the event, in time or out of time w.r.t. muon So activity from overlay
    //! will also appear as blobs, if they are in the same time slice.
    //------------------------------------------------------------------------------------------------------------------------------
    if ((*p)->creationSignature() ==
        PrimaryBlobProngExtraDataDefs::IsolatedIDBlobProngName()) {
      n_nonvtx_iso_blobs_all += (*p)->idblobs().size();
      double calE = m_caloUtils->applyCalConsts(*p, "Default", false);
      nonvtx_iso_blobs_energy_all += calE;

      //------------------------------------------------------------------------------------------------------------------
      //! Loop over Isolated Blobs in prong that are in time with the neutrino
      //! interaction that we are interested in. Get their calibrated energy.
      //! For event selection purposes, we cut ONLY on these blobs
      //------------------------------------------------------------------------------------------------------------------
      Minerva::IDBlobVect isoBlobs = (*p)->idblobs();
      for (Minerva::IDBlobVect::iterator itBlob = isoBlobs.begin();
           itBlob != isoBlobs.end();
           itBlob++) {
        ...
        if (time_diff > m_lowerTimeWindow && time_diff < m_upperTimeWindow) {
          n_nonvtx_iso_blobs++;
          double calE_inTime =
            m_caloUtils->applyCalConsts(*itBlob, "Default", false);
          ...
          m_blobCreatorUtils->calcUpstreamBlobStartPoint(*itBlob); const
          Gaudi::XYZPoint blob_pos_ds =
          m_blobCreatorUtils->calcDownstreamBlobStartPoint(*itBlob);
          ...
          */
          // double distance = sqrt( pow(blob_pos.x()-vtx_pos.x(),2) +
          // pow(blob_pos.y()-vtx_pos.y(),2) + pow(blob_pos.z()-vtx_pos.z(),2) );
          ...
          nonVtxIsoBlobHighestModuleXInProng.push_back(
            (*itBlob)->moduleHighX());
          nonVtxIsoBlobHighestModuleUInProng.push_back(
            (*itBlob)->moduleHighU());
          nonVtxIsoBlobHighestModuleVInProng.push_back(
            (*itBlob)->moduleHighV());
          verbose() << "Distance non-vtx iso blob-vertex = " << distance
                    << endmsg;

          Minerva::IDDigitVect blobhits = (*itBlob)->getAllDigits();
          std::vector<const Minerva::IDCluster*> blobidclusters;
          SmartRefVector<Minerva::IDCluster> clustersblob =
            (*itBlob)->clusters();
          blobidclusters.insert(
            blobidclusters.begin(), clustersblob.begin(), clustersblob.end());

          double earlyhit = 1e20;
          double latehit = -1e20;
          double highesthitenergy = 0;
          int nblobhits = 0;
          for (Minerva::IDDigitVect::iterator itBlobHits = blobhits.begin();
               itBlobHits != blobhits.end();
               itBlobHits++) {
            if ((*itBlobHits)->calTime() < earlyhit)
              earlyhit = (*itBlobHits)->time();
            if ((*itBlobHits)->calTime() > latehit)
              latehit = (*itBlobHits)->time();
            if ((*itBlobHits)->normEnergy() > highesthitenergy)
              highesthitenergy = (*itBlobHits)->normEnergy();
            nblobhits += 1;
          }
  ...
  verbose() << "nonvtx_iso_blobs_energy = " << nonvtx_iso_blobs_energy
            << endmsg;
  verbose() << "nonvtx_iso_blobs_energy_all = " << nonvtx_iso_blobs_energy_all
            << endmsg;

  event->setDoubleData("vtx_blobs_energy", vtx_blobs_energy);
  event->setDoubleData("vtx_iso_blobs_energy_outside_radius",
                       vtxisoblob_clusE_outsideradius);
  event->setContainerDoubleData("vtx_blobs_vtx_energy_in_prong",
                                vtxBlobEnergyInVtxBlobProng);
  event->setContainerDoubleData("vtx_blobs_iso_energy_in_prong",
                                isoBlobEnergyInVtxBlobProng);
  event->setContainerDoubleData(
    "vtx_blobs_iso_energy_clusters_outside_radius_in_prong",
    isoBlobEnergyOutsideRadiusInVtxBlobProng);
  event->setContainerDoubleData("vtx_blobs_iso_distance_in_prong",
                                isoBlobDistanceInVtxBlobProng);
  ...
  event->setDoubleData("nonvtx_iso_blobs_energy_all",
                       nonvtx_iso_blobs_energy_all);
  event->setContainerDoubleData("nonvtx_iso_blobs_energy_in_prong",
                                nonVtxIsoBlobEnergyInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_distance_in_prong",
                                nonVtxIsoBlobDistanceInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_start_position_x_in_prong",
                                nonVtxIsoBlobStartXInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_start_position_y_in_prong",
                                nonVtxIsoBlobStartYInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_start_position_z_in_prong",
                                nonVtxIsoBlobStartZInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_time_in_prong",
                                nonVtxIsoBlobTimeInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_time_difference_in_prong",
                                nonVtxIsoBlobTimeDiffInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_lowest_module_x_in_prong",
                                nonVtxIsoBlobLowestModuleXInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_lowest_module_u_in_prong",
                                nonVtxIsoBlobLowestModuleUInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_lowest_module_v_in_prong",
                                nonVtxIsoBlobLowestModuleVInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_highest_module_x_in_prong",
                                nonVtxIsoBlobHighestModuleXInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_highest_module_u_in_prong",
                                nonVtxIsoBlobHighestModuleUInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_highest_module_v_in_prong",
                                nonVtxIsoBlobHighestModuleVInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_earliest_hit_time_in_prong",
                                nonVtxIsoBlobEarliestHitTimeInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_latest_hit_time_in_prong",
                                nonVtxIsoBlobLatestHitTimeInProng);
  event->setContainerDoubleData("nonvtx_iso_blobs_highest_hit_energy_in_prong",
                                nonVtxIsoBlobHighestHitEnergyInProng);
  event->setContainerIntData("nonvtx_iso_blobs_n_hits_in_prong",
                             nonVtxIsoBlobNHitsInProng);
  event->setContainerIntData("nonvtx_iso_blobs_particle_pdg_in_prong",
                             nonVtxIsoBlobParticlePDGInProng);
  event->setContainerIntData("nonvtx_iso_blobs_primary_particle_pdg_in_prong",
                             nonVtxIsoBlobPrimaryParticlePDGInProng);
  event->setContainerDoubleData(
    "nonvtx_iso_blobs_matched_energy_fraction_in_prong",
    nonVtxIsoBlobMatchedEnergyFractionInProng);
  event->setContainerDoubleData(
    "nonvtx_iso_blobs_data_energy_fraction_in_prong",
    nonVtxIsoBlobDataEnergyFractionInProng);

  //-------------------------------------------------------------------------------------
  //! Look at the remaining ID clusters in the Tracker/ECAL regions (dispersed
  //! energy) in the [-20,35] ns time window
  //-------------------------------------------------------------------------------------
  Minerva::IDClusterVect unusedIDClusters =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableIDClusters(event, muonProng);
  double iddisEnergy =
    m_caloUtils->applyCalConsts(unusedIDClusters, "Default", false);
  ...
  Minerva::IDClusterVect nonMuonNonVtxClusters0mm =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableNonMuonNonVtxIDClusters(
      event, muonProng, 0);
  Minerva::IDClusterVect nonMuonNonVtxClusters50mm =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableNonMuonNonVtxIDClusters(
      event, muonProng, 50);
  Minerva::IDClusterVect nonMuonNonVtxClusters100mm =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableNonMuonNonVtxIDClusters(
      event, muonProng, 100);
  Minerva::IDClusterVect nonMuonNonVtxClusters150mm =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableNonMuonNonVtxIDClusters(
      event, muonProng, 150);
  Minerva::IDClusterVect nonMuonNonVtxClusters200mm =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableNonMuonNonVtxIDClusters(
      event, muonProng, 200);
  Minerva::IDClusterVect nonMuonNonVtxClusters250mm =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableNonMuonNonVtxIDClusters(
      event, muonProng, 250);
  Minerva::IDClusterVect nonMuonNonVtxClusters300mm =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableNonMuonNonVtxIDClusters(
      event, muonProng, 300);

  double nonMuonNonVtxEnergy0mm =
    m_caloUtils->applyCalConsts(nonMuonNonVtxClusters0mm, "Default", false);
  double nonMuonNonVtxEnergy50mm =
    m_caloUtils->applyCalConsts(nonMuonNonVtxClusters50mm, "Default", false);
  double nonMuonNonVtxEnergy100mm =
    m_caloUtils->applyCalConsts(nonMuonNonVtxClusters100mm, "Default", false);
  double nonMuonNonVtxEnergy150mm =
    m_caloUtils->applyCalConsts(nonMuonNonVtxClusters150mm, "Default", false);
  double nonMuonNonVtxEnergy200mm =
    m_caloUtils->applyCalConsts(nonMuonNonVtxClusters200mm, "Default", false);
  double nonMuonNonVtxEnergy250mm =
    m_caloUtils->applyCalConsts(nonMuonNonVtxClusters250mm, "Default", false);
  double nonMuonNonVtxEnergy300mm =
    m_caloUtils->applyCalConsts(nonMuonNonVtxClusters300mm, "Default", false);

  event->setDoubleData("recoil_energy_nonmuon_nonvtx0mm",
                       nonMuonNonVtxEnergy0mm);
  event->setDoubleData("recoil_energy_nonmuon_nonvtx50mm",
                       nonMuonNonVtxEnergy50mm);
  event->setDoubleData("recoil_energy_nonmuon_nonvtx100mm",
                       nonMuonNonVtxEnergy100mm);
  event->setDoubleData("recoil_energy_nonmuon_nonvtx150mm",
                       nonMuonNonVtxEnergy150mm);
  event->setDoubleData("recoil_energy_nonmuon_nonvtx200mm",
                       nonMuonNonVtxEnergy200mm);
  event->setDoubleData("recoil_energy_nonmuon_nonvtx250mm",
                       nonMuonNonVtxEnergy250mm);
  event->setDoubleData("recoil_energy_nonmuon_nonvtx300mm",
                       nonMuonNonVtxEnergy300mm);

  Minerva::IDClusterVect nonMuonVtxClusters0mm =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableNonMuonVtxIDClusters(
      event, muonProng, 0);
  Minerva::IDClusterVect nonMuonVtxClusters50mm =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableNonMuonVtxIDClusters(
      event, muonProng, 50);
  Minerva::IDClusterVect nonMuonVtxClusters100mm =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableNonMuonVtxIDClusters(
      event, muonProng, 100);
  Minerva::IDClusterVect nonMuonVtxClusters150mm =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableNonMuonVtxIDClusters(
      event, muonProng, 150);
  Minerva::IDClusterVect nonMuonVtxClusters200mm =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableNonMuonVtxIDClusters(
      event, muonProng, 200);
  Minerva::IDClusterVect nonMuonVtxClusters250mm =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableNonMuonVtxIDClusters(
      event, muonProng, 250);
  Minerva::IDClusterVect nonMuonVtxClusters300mm =
    m_masterAnaDevRecoUtils->getInTimeAnalyzableNonMuonVtxIDClusters(
      event, muonProng, 300);
  debug() << "first try: nonMuonVtxClusters0mm size: "
          << nonMuonVtxClusters0mm.size() << endmsg;

  double nonMuonVtxEnergy0mm =
    m_caloUtils->applyCalConsts(nonMuonVtxClusters0mm, "Default", false);
  double nonMuonVtxEnergy50mm =
    m_caloUtils->applyCalConsts(nonMuonVtxClusters50mm, "Default", false);
  double nonMuonVtxEnergy100mm =
    m_caloUtils->applyCalConsts(nonMuonVtxClusters100mm, "Default", false);
  double nonMuonVtxEnergy150mm =
    m_caloUtils->applyCalConsts(nonMuonVtxClusters150mm, "Default", false);
  double nonMuonVtxEnergy200mm =
    m_caloUtils->applyCalConsts(nonMuonVtxClusters200mm, "Default", false);
  double nonMuonVtxEnergy250mm =
    m_caloUtils->applyCalConsts(nonMuonVtxClusters250mm, "Default", false);
  double nonMuonVtxEnergy300mm =
    m_caloUtils->applyCalConsts(nonMuonVtxClusters300mm, "Default", false);
  ...
  //---------------------------------------------------
  for (Minerva::IDClusterVect::iterator itClust = unusedIDClusters.begin();
       itClust != unusedIDClusters.end();
       ++itClust) {
    if (0 <= m_dispersedEnergyColor)
      m_hitTaggerTool->applyColorTag(*itClust, m_dispersedEnergyColor);
  }
  ...
  SmartRef<Minerva::Particle> neutronPart =
    m_neutronBlobUtils->computeNeutronFromHydrogen(muonPart);
  ...
  if (m_doNeutron) {
    ...
    SmartRefVector<Minerva::Track> vertexTracks =
      event->interactionVertex()->getTracks();
    debug() << "Start Looping" << endmsg;
    for (SmartRefVector<Minerva::Track>::iterator itTrack =
           vertexTracks.begin();
         itTrack != vertexTracks.end();
         ++itTrack) {
      std::vector<Minerva::IDCluster*> clustersOnTrack =
        (*itTrack)->idclusters();
      for (std::vector<Minerva::IDCluster*>::iterator itC =
             clustersOnTrack.begin();
           itC != clustersOnTrack.end();
           ++itC)
        trackedClusterSets.insert(*itC);
    }
    // Add clusters on the hadronProngs
    for (SmartRefVector<Minerva::Prong>::iterator itProng =
           hadronProngs.begin();
         itProng != hadronProngs.end();
         ++itProng) {
      SmartRefVector<Minerva::IDCluster> allIDClusters =
        (*itProng)->getAllIDClusters();
      for (SmartRefVector<Minerva::IDCluster>::iterator itC =
             allIDClusters.begin();
           itC != allIDClusters.end();
           ++itC)
        trackedClusterSets.insert(*itC);
    }
    Minerva::IDClusterVect trackedClusters(trackedClusterSets.begin(),
                                           trackedClusterSets.end());
    Minerva::IDClusterVect untrackedClusters;

    for (Minerva::IDClusterVect::iterator itC =
           nonMuonNonVtxClusters0mm.begin();
         itC != nonMuonNonVtxClusters0mm.end();
         ++itC) {
      bool duplicate =
        std::find(trackedClusters.begin(), trackedClusters.end(), *itC) !=
        trackedClusters.end();
      if (!duplicate)
        untrackedClusters.push_back(*itC);
    }

    // for( SmartRefVector<Minerva::Track>::iterator itTrack =
    // vertexTracks.begin(); itTrack != vertexTracks.end(); ++itTrack )
    //{
    //  //loop through each track attached to the vertex and remove duplicate
    //  from untrackedClusters Minerva::IDClusterVect::iterator itCluster;
    //  std::vector< Minerva::IDCluster* > clustersOnTrack =
    //  (*itTrack)->idclusters(); debug()<<"Cluster and iterator
    //  created"<<endmsg; for( itCluster = untrackedClusters.begin(); itCluster
    //  != untrackedClusters.end(); )
    //  {
    //    bool duplicate = std::find( clustersOnTrack.begin(),
    //    clustersOnTrack.end(), *itCluster) != clustersOnTrack.end() ;
    //    (duplicate)? debug()<<"Duplicate"<<endmsg : debug()<<"Not
    //    duplicate"<<endmsg; if (duplicate) untrackedClusters.erase( itCluster
    //    ); else ++itCluster;
    //  }
    //}
    ...
    if (cleanTrack) {
      for (SmartRefVector<Minerva::IDCluster>::iterator itC =
             untrackedClusters.begin();
           itC != untrackedClusters.end();
           ++itC) {
        bool inTracker = ((*itC)->subdet() == Minerva::IDCluster::Tracker);
        if (!inTracker && m_doNeutronInTrackerOnly)
          continue;

        bool isTrackable = ((*itC)->type() == Minerva::IDCluster::Trackable);
        bool isHeavyIonizing =
          ((*itC)->type() == Minerva::IDCluster::HeavyIonizing);
        bool closeToMuon = m_neutronBlobUtils->isClusterCloseToMuon(
          muonPart, (*itC), m_muonAxisCylinderRadius);

        bool saveCluster = (!closeToMuon && isTrackable) || isHeavyIonizing;
        if (saveCluster)
          nonMuonClusters.push_back(*itC);
      }
    } else
      nonMuonClusters = untrackedClusters;

    // Clear vector and pass on the event if there are too many clusters
    if (nonMuonClusters.size() < m_maxRecoCluster)
      event->setIntData("PassNeutronMaxClusterCut", 1);
    else
      nonMuonClusters.clear();
    ...
    SmartRefVector<Minerva::IDBlob> AllIDBlobs =
      m_neutronBlobRecoTool->findTrackableCandidates(
        nonMuonClusters, 100, 0.0); // form all blobs
    SmartRefVector<Minerva::IDBlob> AllIDContiguousBlobs =
      m_neutronBlobRecoTool->combineContiguous2DBlobs(
        AllIDBlobs); // form all blobs

    int AllIDBlobsNClusters = 0, AllIDContiguousBlobsNClusters = 0;
    for (SmartRefVector<Minerva::IDBlob>::iterator itall = AllIDBlobs.begin();
         itall != AllIDBlobs.end();
         ++itall)
      AllIDBlobsNClusters += (*itall)->nclusters();
    for (SmartRefVector<Minerva::IDBlob>::iterator itall =
           AllIDContiguousBlobs.begin();
         itall != AllIDContiguousBlobs.end();
         ++itall)
      AllIDContiguousBlobsNClusters += (*itall)->nclusters();
    ...
    debug() << "nonMuonNonVtxClusters0mm size: "
            << nonMuonNonVtxClusters0mm.size() << endmsg;
    ...
    debug() << "AllIDContinugousBlobs size: " << AllIDContiguousBlobs.size()
            << endmsg;
    debug() << "AllIDContiguousBlobs clusters size: "
            << AllIDContiguousBlobsNClusters << endmsg;
    ...
    std::vector<NeutronDataStruct::BlobInfo>
      BlobsInfo; // create blobinfo data container
    debug() << "Created BlobInfo" << endmsg;
    Gaudi::XYZTVector neutron_vtx(vertexOfPhysEvent->position().X(),
                                  vertexOfPhysEvent->position().Y(),
                                  vertexOfPhysEvent->position().Z(),
                                  vertexOfPhysEvent->getTime());
    debug() << "neutron_vtx set at (" << neutron_vtx.X() << ", "
            << neutron_vtx.Y() << ", " << neutron_vtx.Z() << ")" << endmsg;
    for (SmartRefVector<Minerva::IDBlob>::iterator it =
           AllIDContiguousBlobs.begin();
         it != AllIDContiguousBlobs.end();
         it++) {
      debug() << "Fill data for blob " << it - AllIDContiguousBlobs.begin()
              << endmsg;
    ...
    m_neutronBlobUtils->FillData(
      BlobsInfo, haveNeutrinoMC()); // Fill reco related blobInfo branches and
                                    // truth if haveNeutrinoMC
    debug() << "NBlobs Before Filling: " << AllIDContiguousBlobs.size()
            << endmsg;
  ...
  event->setIntData("pass_MasterAnaDev_precuts",
                    (int)pass_MasterAnaDev_precuts);
  event->filtertaglist()->setOrAddFilterTag("pass_MasterAnaDev_precuts",
                                            pass_MasterAnaDev_precuts);
  ...
  if (haveNeutrinoMC()) {
    ...
    for (unsigned int prong = 0; prong < primaryProngs.size(); prong++) {
      if (primaryProngs[prong]->filtertaglist()->filterTagExists(
            "PrimaryMuon")) {
        muonProng = primaryProngs[prong];
        info() << "Muon Prong Size " << muonProng->getAllIDClusters().size()
               << endmsg;
        break;
      }
    }
    const Minerva::IDClusterVect& muonClustersID =
      muonProng->getAllIDClusters();
    for (unsigned int prong = 0; prong < primaryProngs.size(); prong++) {
      if (primaryProngs[prong]->filtertaglist()->filterTagExists(
            "PrimaryPion")) {
        Minerva::IDClusterVect tmp_idclusters =
          primaryProngs[prong]->getAllIDClusters();
        Minerva::IDClusterVect idclusters;
        Minerva::ODClusterVect odclusters;
        idClusters += tmp_idclusters.size();
        Minerva::IDClusterVect::iterator i_tmp_idclusters =
          tmp_idclusters.begin();
        for (; i_tmp_idclusters != tmp_idclusters.end(); ++i_tmp_idclusters) {
          // skip if not in time window
          if ((*i_tmp_idclusters)->time() < mintimeCut ||
              maxtimeCut < (*i_tmp_idclusters)->time())
            continue;
          // skip if it's a muon cluster
          if (std::find(muonClustersID.begin(),
                        muonClustersID.end(),
                        (*i_tmp_idclusters)) != muonClustersID.end())
            continue;
          // blegh.  This is because the short tracker will use a cluster twice.
          // This should remove duplicate clusters in all clusters and prong
          // clusters.
          // This is very hacky
          if (std::find(allProngIDClusters.begin(),
                        allProngIDClusters.end(),
                        (*i_tmp_idclusters)) != allProngIDClusters.end())
            continue;
          // skip xtalk
          if ((*i_tmp_idclusters)->type() == 5)
            continue;
    ...
    info() << "Container size " << container_idclusters.size() << " "
           << container_odclusters.size() << endmsg;
    std::sort(allProngIDClusters.begin(), allProngIDClusters.end());
    Minerva::IDClusterVect::iterator duplicate =
      std::adjacent_find(allProngIDClusters.begin(), allProngIDClusters.end());
    if (duplicate != allProngIDClusters.end())
      info() << "Found a cluster duplicate in part response" << endmsg;
    Minerva::IDClusterVect hadClustersID =
      getHadronIDClusters(event, muonProng);
    Minerva::ODClusterVect hadClustersOD;
    if (iClusters > hadClustersID.size())
      info() << "Number of pion prong clusters > hadClusters" << endmsg;
    ...
    fillModifiedParticleResponseBranches(event,
                                         container_idclusters,
                                         container_odclusters,
                                         hadClustersID,
                                         hadClustersOD);
  ...
  for (Minerva::ProngVect::iterator p = prongs.begin(); p != prongs.end();
    ...
    if ((*p)->creationSignature() ==
      ...
      for (Minerva::IDClusterVect::iterator itCluster = clusters.begin();
  ...
  for (Minerva::IDClusterVect::iterator itCluster = unusedIDClusters.begin();
       itCluster != unusedIDClusters.end();
       itCluster++)
    nonVtxIDClusters.push_back(*itCluster);
  ...
  if (!nuInt.empty()) {
    ...
    for (vector<Minerva::NeutrinoInt*>::const_iterator it = nuInt.begin();
         it != nuInt.end();
         ++it) {
      // there should only be one NeutrinoInt.  choose the one with the best
      // score to set the event hadronic energy.
      double hypRecoilE = 0.;
...
//=============================================================================
StatusCode
MasterAnaDev::interpretEvent(const Minerva::PhysicsEvent* event,
                             const Minerva::GenMinInteraction* truth,
                             std::vector<Minerva::NeutrinoInt*>& nuInt) const
{
  ...
  if (!event->hasInteractionVertex()) {
    ...
    return StatusCode::SUCCESS; // have to do this for events without
                                // interaction vertices because basically all
                                // the objects below are not valid. The code
                                // will crash and burn.
  }
  SmartRef<Minerva::Prong> muonProng;
  SmartRef<Minerva::Particle> muonPart;
  ...
  bool isFiducialXY = FiducialPointTool->isFiducial(
    vtx, m_signalApothem, m_usFiducialModID, m_dsFiducialModID);
  bool isAnalyzableXY = FiducialPointTool->isFiducial(
    vtx, m_analyzeApothem, m_usAnalyzableModID, m_dsAnalyzableModID);

  if (!isAnalyzableXY) {
    debug() << "    Projected vertex is not in the analyzable volume."
            << endmsg;
    // if( ! force )
    return StatusCode::SUCCESS;
  }
  ...
  int muon_charge = 0;
  if (MuonUtils->muonCharge(muonProng, muon_charge, m_qOverpChargeCut) ==
      StatusCode::FAILURE) {
    debug() << "Could not get a muon charge.  Mark the event with a low score "
               "and do your best."
            << endmsg;
  }
  ...
  double muon_theta = MuonUtils->muTheta(muonPart);
  double muon_score = muonPart->score(), muon_qp = -9999.9,
         muon_qpqpe = -9999.9;
  double muon_theta_biasUp =
    m_minCoordSysTool->thetaWRTBeam(muon_4p, m_beamAngleBias) - muon_theta;
  double muon_theta_biasDown =
    m_minCoordSysTool->thetaWRTBeam(muon_4p, -1.0 * m_beamAngleBias) -
    muon_theta;
  ...
  //! Hadronic Recoil
  //--------------------------------------------------------------
  double hadron_recoilE_CCInc =
    RecoilUtils->calcRecoilEFromClusters(event, muonProng, "CCInclusive", -1);
  double hadron_recoilE =
    RecoilUtils->calcRecoilEFromClusters(event, muonProng, "CCNuPionInc");
  double hadron_recoilE_default =
    RecoilUtils->calcRecoilEFromClusters(event, muonProng, "Default");
  double hadron_recoilE_two_track = RecoilUtils->calcRecoilEFromClusters(
    event, muonProng, "NukeCCPion_TwoTrack_Nu_Tracker");
  ...
  //-----------------------------------------------
  double proton_P = -9999., proton_E = -9999., proton_T = -9999.,
         proton_theta = -9999., proton_Px = -9999., proton_Py = -9999.,
         proton_Pz = -9999.;
  ...
  //------------------------------------------------------------------------
  double proton_score1_Mass_Up = -9999.9, proton_score1_Mass_Down = -9999.9,
         proton_score1_BetheBloch_Up = -9999.9,
         proton_score1_BetheBloch_Down = -9999.9;
  double proton_score1_MEU_Up = -9999.9, proton_score1_MEU_Down = -9999.9,
         proton_score1_Birks = -9999.9;
  double proton_E_Mass_Up = -9999.9, proton_E_Mass_Down = -9999.9,
         proton_E_BetheBloch_Up = -9999.9, proton_E_BetheBloch_Down = -9999.9;
  double proton_E_MEU_Up = -9999.9, proton_E_MEU_Down = -9999.9,
         proton_E_Birks = -9999.9;
  int proton_patternRec = -1;

  std::vector<double> sec_protons_score1_Mass_Up, sec_protons_score1_Mass_Down,
    sec_protons_score1_BetheBloch_Up, sec_protons_score1_BetheBloch_Down;
  std::vector<double> sec_protons_score1_MEU_Up, sec_protons_score1_MEU_Down,
    sec_protons_score1_Birks;
  std::vector<double> sec_protons_E_Mass_Up, sec_protons_E_Mass_Down,
    sec_protons_E_BetheBloch_Up, sec_protons_E_BetheBloch_Down;
  std::vector<double> sec_protons_E_MEU_Up, sec_protons_E_MEU_Down,
    sec_protons_E_Birks;
  std::vector<int> sec_protons_patternRec;

  debug()
    << "Getting most energetic (Primary) proton and Secondary proton prongs"
    << endmsg;
  bool protonFound = false;
  bool secondaryProtonsFound = false;

  for (ProngVect::iterator itProng = primaryProngs.begin();
       itProng != primaryProngs.end();
       ++itProng) {
    ...
    if ((*itProng)->filtertaglist()->filterTagExists("PrimaryHadron")) {
      (*itProng)->filtertaglist()->checkFilterTag("PrimaryHadron",
                                                  isPrimaryHadron);
    }

    // if (isPrimaryMuon && !isPrimaryHadron) muonProng = *itProng;
    if (isPrimaryHadron && !isPrimaryMuon)
      hadronProngs.push_back(*itProng);
    if (isPrimaryMuon && isPrimaryHadron) {
      warning() << "Prong is two primary particles!" << endmsg;
    }
  }

  for (ProngVect::iterator itProngs = primaryProngs.begin();
       itProngs != primaryProngs.end();
       ++itProngs) {
    if ((*itProngs)->filtertaglist()->filterTagExists(m_primaryProton)) {
      protonProng = (*itProngs);

      totalCalibEPrimaryProton =
        m_caloUtils->applyCalConsts(protonProng, "Default", false, true);
      debug() << "Total calib energy of primary proton "
              << totalCalibEPrimaryProton << endmsg;

      SmartRefVector<Minerva::Particle> pionparticles =
        (*itProngs)->particlesWithIDCode(Minerva::Particle::Pion, "dEdX");
      if (pionparticles.size() > 0)
        pionPart = pionparticles[0];
      protonPart = (*itProngs)->bestParticle();
      if (protonPart)
        protonFound = true;
    } else if ((*itProngs)->filtertaglist()->filterTagExists(
                 m_secondaryProtons)) {
  ...
  double proton_startPointX = -9999., proton_startPointY = -9999.,
         proton_startPointZ = -9999.;
  double proton_endPointX = -9999., proton_endPointY = -9999.,
         proton_endPointZ = -9999.;
  ...
  if (protonFound) {
    ...
    proton_score1 =
      protonPart->getDoubleData(ParticleExtraDataDefs::dEdXScore1());
    proton_score2 =
      protonPart->getDoubleData(ParticleExtraDataDefs::dEdXScore2());

    // Set proton prong type, kinked=1, forked=2, ok = 0
    if (protonProng->Kinked())
      proton_prongType = 1;
    else if (protonProng->Forked())
      proton_prongType = 2;
    else
      proton_prongType = 0;
    ...
    proton_startPointX =
      (protonProng->minervaTracks().back())->firstState().x();
    proton_startPointY =
      (protonProng->minervaTracks().back())->firstState().y();
    proton_startPointZ =
      (protonProng->minervaTracks().back())->firstState().z();

    proton_nodes_nodesNormE =
      m_masterAnaDevRecoUtils->getNormalizedLastNodesEnergy(
        protonProng->minervaTracks().back(), true);
    proton_nodes_E = m_masterAnaDevRecoUtils->getNormalizedLastNodesEnergy(
      protonProng->minervaTracks().back(), false);

    //------------------------------------------------------
    // dEdx Tool variations for Primary Proton
    //------------------------------------------------------
    if (protonPart->hasDoubleData("score1_Mass_Up")) {
      proton_score1_Mass_Up =
        protonPart->getDoubleData("score1_Mass_Up") - proton_score1;
      proton_E_Mass_Up = protonPart->getDoubleData("FitE_Mass_Up") - proton_E;
    }
    if (protonPart->hasDoubleData("score1_Mass_Down")) {
      proton_score1_Mass_Down =
        protonPart->getDoubleData("score1_Mass_Down") - proton_score1;
      proton_E_Mass_Down =
        protonPart->getDoubleData("FitE_Mass_Down") - proton_E;
    }
    if (protonPart->hasDoubleData("score1_BetheBloch_Up")) {
      proton_score1_BetheBloch_Up =
        protonPart->getDoubleData("score1_BetheBloch_Up") - proton_score1;
      proton_E_BetheBloch_Up =
        protonPart->getDoubleData("FitE_BetheBloch_Up") - proton_E;
    }
    if (protonPart->hasDoubleData("score1_BetheBloch_Down")) {
      proton_score1_BetheBloch_Down =
        protonPart->getDoubleData("score1_BetheBloch_Down") - proton_score1;
      proton_E_BetheBloch_Down =
        protonPart->getDoubleData("FitE_BetheBloch_Down") - proton_E;
    }
    if (protonPart->hasDoubleData("score1_MEU_Up")) {
      proton_score1_MEU_Up =
        protonPart->getDoubleData("score1_MEU_Up") - proton_score1;
      proton_E_MEU_Up = protonPart->getDoubleData("FitE_MEU_Up") - proton_E;
    }
    if (protonPart->hasDoubleData("score1_MEU_Down")) {
      proton_score1_MEU_Down =
        protonPart->getDoubleData("score1_MEU_Down") - proton_score1;
      proton_E_MEU_Down = protonPart->getDoubleData("FitE_MEU_Down") - proton_E;
    }
    if (protonPart->hasDoubleData("score1_Birks")) {
      proton_score1_Birks =
        protonPart->getDoubleData("score1_Birks") - proton_score1;
    ...
    // double proton_thetaX =
    // m_minCoordSysTool->thetaXWRTBeam(protonPart->momentumVec()); double
    // proton_thetaY =
    // m_minCoordSysTool->thetaYWRTBeam(protonPart->momentumVec());

    debug()
      << "Calculated from dEdx: proton_E, proton_T, proton_P, proton_theta"
      << endmsg;

  } else
    debug() << " Could not get Primary proton prong" << endmsg;

  if (secondaryProtonsFound) {
    int index = 0;
    for (ProngVect::iterator secprongs = secondaryProtonProngs.begin();
         secprongs != secondaryProtonProngs.end();
         ++secprongs) {
      double secproton_proton_score = -9999., secproton_proton_score1 = -9999.,
             secproton_proton_score2 = -999.;
      double totalCalibESecondaryProton = -9999.;

      totalCalibESecondaryProton =
        m_caloUtils->applyCalConsts((*secprongs), "Default", false, true);
      debug() << "Total calib energy of secondary proton "
              << totalCalibESecondaryProton << endmsg;

      SmartRef<Minerva::Particle> secprongPart = (*secprongs)->bestParticle();

      secproton_proton_score = secprongPart->score();
      secproton_proton_score1 =
        secprongPart->getDoubleData(ParticleExtraDataDefs::dEdXScore1());
      secproton_proton_score2 =
        secprongPart->getDoubleData(ParticleExtraDataDefs::dEdXScore2());
      ...
      sec_protons_T.push_back(secprongPart->momentumVec().E() -
                              MinervaUnits::M_p);
      ...
      sec_protons_patternRec.push_back(
        protonProng->minervaTracks()[0]->patRecHistory());
      // Add px,py,pz here
      int prongtype = -9999;
      if ((*secprongs)->Kinked())
        prongtype = 1;
      else if ((*secprongs)->Forked())
        prongtype = 2;
      else
        prongtype = 0;
      ...
      if (secprongPart->hasDoubleData("score1_Mass_Up")) {
        sec_protons_score1_Mass_Up.push_back(
          secprongPart->getDoubleData("score1_Mass_Up") - proton_score1);
        sec_protons_E_Mass_Up.push_back(
          secprongPart->getDoubleData("FitE_Mass_Up") - proton_E);
      }
      if (secprongPart->hasDoubleData("score1_Mass_Down")) {
        sec_protons_score1_Mass_Down.push_back(
          secprongPart->getDoubleData("score1_Mass_Down") - proton_score1);
        sec_protons_E_Mass_Down.push_back(
          secprongPart->getDoubleData("FitE_Mass_Down") - proton_E);
      }
      if (secprongPart->hasDoubleData("score1_BetheBloch_Up")) {
        sec_protons_score1_BetheBloch_Up.push_back(
          secprongPart->getDoubleData("score1_BetheBloch_Up") - proton_score1);
        sec_protons_E_BetheBloch_Up.push_back(
          secprongPart->getDoubleData("FitE_BetheBloch_Up") - proton_E);

        debug() << "Proton Score1 BB Up Variation "
                << secprongPart->getDoubleData("score1_BetheBloch_Up")
                << endmsg;
      }
      if (secprongPart->hasDoubleData("score1_BetheBloch_Down")) {
        sec_protons_score1_BetheBloch_Down.push_back(
          secprongPart->getDoubleData("score1_BetheBloch_Down") -
          proton_score1);
        sec_protons_E_BetheBloch_Down.push_back(
          secprongPart->getDoubleData("FitE_BetheBloch_Down") - proton_E);

        debug() << "Proton Score1 BB Down Variation "
                << secprongPart->getDoubleData("score1_BetheBloch_Down")
                << endmsg;
      }
      if (secprongPart->hasDoubleData("score1_MEU_Up")) {
        sec_protons_score1_MEU_Up.push_back(
          secprongPart->getDoubleData("score1_MEU_Up") - proton_score1);
        sec_protons_E_MEU_Up.push_back(
          secprongPart->getDoubleData("FitE_MEU_Up") - proton_E);
      }
      if (secprongPart->hasDoubleData("score1_MEU_Down")) {
        sec_protons_score1_MEU_Down.push_back(
          secprongPart->getDoubleData("score1_MEU_Down") - proton_score1);
        sec_protons_E_MEU_Down.push_back(
          secprongPart->getDoubleData("FitE_MEU_Down") - proton_E);
      }
      if (secprongPart->hasDoubleData("score1_Birks")) {
        sec_protons_score1_Birks.push_back(
          secprongPart->getDoubleData("score1_Birks") - proton_score1);
        sec_protons_E_Birks.push_back(
          secprongPart->getDoubleData("FitE_Birks") - proton_E);
      }

      //----------------------------------------
      // ESC proton variables
      //----------------------------------------
      std::vector<double> thisNodesNormE =
        m_masterAnaDevRecoUtils->getNormalizedLastNodesEnergy(
          (*secprongs)->minervaTracks().back(), true);
      std::vector<double> thisNodesE =
        m_masterAnaDevRecoUtils->getNormalizedLastNodesEnergy(
          (*secprongs)->minervaTracks().back(), false);

      sec_protons_nodes_index.insert(
        sec_protons_nodes_index.end(), thisNodesE.size(), index);
      sec_protons_nodes_nodesNormE.insert(sec_protons_nodes_nodesNormE.end(),
                                          thisNodesNormE.begin(),
                                          thisNodesNormE.end());
      sec_protons_nodes_E.insert(
        sec_protons_nodes_E.end(), thisNodesE.begin(), thisNodesE.end());
      //----------------------------------------
      // Rotated proton's angle w.r.t beam
      //----------------------------------------
      sec_protons_theta.push_back(
        m_minCoordSysTool->thetaWRTBeam(secprongPart->momentumVec()));
      index++;

    } // End of for loop over Secondary proton prongs

  } else
    verbose() << "No secondary protons were found" << endmsg;
  ...
  std::vector<int> hadron_tm_PDGCode, hadron_tm_trackID, hadron_tm_destructCode,
    hadron_tm_isTruthMatched;
  ...
  // dEdXTool biasing
  std::vector<double> hadron_piFit_score1_Mass_Up,
    hadron_piFit_score1_Mass_Down, hadron_piFit_score1_BetheBloch_Up,
    hadron_piFit_score1_BetheBloch_Down, hadron_piFit_score1;
  std::vector<double> hadron_piFit_score1_MEU_Up, hadron_piFit_score1_MEU_Down,
    hadron_piFit_score1_Birks;
  std::vector<double> hadron_pion_E_Mass_Up, hadron_pion_E_Mass_Down,
    hadron_pion_E_BetheBloch_Up, hadron_pion_E_BetheBloch_Down;
  ...
  for (iter = 0; iter < 10; iter++) {
    pion_E.push_back(-1.);
    pion_T.push_back(-9999.);
    pion_P.push_back(-1.);
    pion_Px.push_back(-1.);
    pion_Py.push_back(-1.);
    pion_Pz.push_back(-1.);
    pion_theta.push_back(-9.);
    hadron_piFit_scoreLLR.push_back(-101.0);
    hadron_isTracker.push_back(-1);
    hadron_isSideECAL.push_back(-1);
    hadron_isExiting.push_back(-1);
    hadron_isODMatch.push_back(-1);
    hadron_isForked.push_back(-1);
    hadron_endMichel_category.push_back(0);
    hadron_endMichel_slice_energy.push_back(-1.0);
    hadron_endMichel_ndigits.push_back(-1);
    hadron_endMichel_energy.push_back(-1.0);
    hadron_pion_theta_biasUp.push_back(-9.0);
    hadron_pion_theta_biasDown.push_back(-9.0);
    hadron_piFit_score1_Mass_Up.push_back(-999.9);
    hadron_piFit_score1_Mass_Down.push_back(-999.9);
    hadron_piFit_score1_BetheBloch_Up.push_back(-999.9);
    hadron_piFit_score1_BetheBloch_Down.push_back(-999.9);
    hadron_piFit_score1_MEU_Up.push_back(-999.9);
    hadron_piFit_score1_MEU_Down.push_back(-999.9);
    hadron_piFit_score1_Birks.push_back(-999.9);
    hadron_piFit_score1.push_back(-1.0);
    hadron_pion_E_Mass_Up.push_back(-999.9);
    hadron_pion_E_Mass_Down.push_back(-999.9);
    hadron_pion_E_BetheBloch_Up.push_back(-999.9);
    hadron_pion_E_BetheBloch_Down.push_back(-999.9);
    hadron_pion_E_Birks.push_back(-999.9);
    hadron_tm_PDGCode.push_back(0);
    hadron_tm_destructCode.push_back(-3);
    hadron_tm_beginKE.push_back(-1.0);
    hadron_tm_trackID.push_back(-1);
    hadron_tm_isTruthMatched.push_back(-1);
    hadron_tm_fraction.push_back(-1.0);
    hadron_tm_otherE.push_back(-1.0);
    ...
    // Passive-corrected pion kinematics
    hadron_pion_E_corr.push_back(-1.0);
    hadron_pion_p_corr.push_back(-1.0);
    hadron_pion_px_corr.push_back(-1.0);
    hadron_pion_py_corr.push_back(-1.0);
    hadron_pion_pz_corr.push_back(-1.0);
    hadron_pion_E_recoil_corr.push_back(-1.0);
  }
  ...
  for (ProngVect::iterator itProngs = hadronProngs.begin();
       itProngs != hadronProngs.end() && iter < 10;
       ++itProngs, ++iter) {
    ...
    if (hadronProng->hasIntData("hasEndpointMichel")) {
      hadron_endMichel_category[iter] =
        hadronProng->getIntData("endMichel_category");
      hadron_endMichel_slice_energy[iter] =
        hadronProng->getDoubleData("endMichel_slice_energy");
      hadron_endMichel_ndigits[iter] =
        hadronProng->getIntData("endMichel_ndigits");
      hadron_endMichel_energy[iter] =
        hadronProng->getDoubleData("endMichel_energy");
    }

    if (pionPart1) {

      Gaudi::LorentzVector pion_4p = pionPart1->momentumVec();
      double p_mag = sqrt(pow(pion_4p.px(), 2.0) + pow(pion_4p.py(), 2.0) +
                          pow(pion_4p.pz(), 2.0));
      ...
      Gaudi::LorentzVector tmp_pion_4p(
        pion_Px[iter], pion_Py[iter], pion_Pz[iter], pion_E[iter]);
      pion_theta[iter] = m_minCoordSysTool->thetaWRTBeam(tmp_pion_4p);
      hadron_pion_theta_biasUp[iter] =
        m_minCoordSysTool->thetaWRTBeam(pion_4p, m_beamAngleBias) -
        pion_theta[iter];
      hadron_pion_theta_biasDown[iter] =
        m_minCoordSysTool->thetaWRTBeam(pion_4p, -1.0 * m_beamAngleBias) -
        pion_theta[iter];
      ...
      StatusCode sc = m_energyCorrectionTool->getCorrectedEnergy(
        hadronProng, pionPart1, vtx_z, pion_4p_corr);
      if (!sc)
      ...
      double p_cal_correction =
        sqrt(pow(e_cal_correction, 2) - pow(pionPart1->mass(), 2));

      hadron_pion_E_corr[iter] =
        sqrt(p_cal_correction * p_cal_correction +
             MinervaUnits::M_pion * MinervaUnits::M_pion);
      hadron_pion_px_corr[iter] =
        p_cal_correction * sin(pion_theta[iter]) * cos(pion_theta[iter]);
      hadron_pion_py_corr[iter] =
        p_cal_correction * sin(pion_theta[iter]) * sin(pion_theta[iter]);
      hadron_pion_pz_corr[iter] = p_cal_correction * cos(pion_theta[iter]);
      hadron_pion_p_corr[iter] =
        sqrt(hadron_pion_px_corr[iter] * hadron_pion_px_corr[iter] +
             hadron_pion_py_corr[iter] * hadron_pion_py_corr[iter] +
             hadron_pion_pz_corr[iter] * hadron_pion_pz_corr[iter]);

      // score1
      if (pionPart1->hasDoubleData("score1")) {
        double sc1 =
          pionPart1->getDoubleData(ParticleExtraDataDefs::dEdXScore1());
        ...
        if (pionPart1->hasDoubleData("score1_Mass_Up")) {
          hadron_piFit_score1_Mass_Up[iter] =
            pionPart1->getDoubleData("score1_Mass_Up") - sc1;
          hadron_pion_E_Mass_Up[iter] =
            pionPart1->getDoubleData("FitE_Mass_Up") - Epi;
          std::cout << "Pass hadron pion Mass Up "
                    << hadron_pion_E_Mass_Up[iter] << "\n";
        }

        if (pionPart1->hasDoubleData("score1_Mass_Down")) {
          hadron_piFit_score1_Mass_Down[iter] =
            pionPart1->getDoubleData("score1_Mass_Down") - sc1;
          hadron_pion_E_Mass_Down[iter] =
            pionPart1->getDoubleData("FitE_Mass_Down") - Epi;
          std::cout << "Pass hadron pion Mass Down "
                    << hadron_pion_E_Mass_Down[iter] << "\n";
        }

        if (pionPart1->hasDoubleData("score1_Birks")) {
          hadron_piFit_score1_Birks[iter] =
            pionPart1->getDoubleData("score1_Birks") - sc1;
          hadron_pion_E_Birks[iter] =
            pionPart1->getDoubleData("FitE_Birks") - Epi;
          std::cout << "Pass hadron pion E Birks " << hadron_pion_E_Birks[iter]
                    << "\n";
        }
        if (pionPart1->hasDoubleData("score1_BetheBloch_Up")) {
          hadron_piFit_score1_BetheBloch_Up[iter] =
            pionPart1->getDoubleData("score1_BetheBloch_Up") - sc1;
          hadron_pion_E_BetheBloch_Up[iter] =
            pionPart1->getDoubleData("FitE_BetheBloch_Up") - Epi;
          std::cout << "Pass hadron pion Mass Up "
                    << hadron_pion_E_BetheBloch_Up[iter] << "\n";
        }
        if (pionPart1->hasDoubleData("score1_BetheBloch_Down")) {
          hadron_piFit_score1_BetheBloch_Down[iter] =
            pionPart1->getDoubleData("score1_BetheBloch_Down") - sc1;
          hadron_pion_E_BetheBloch_Down[iter] =
            pionPart1->getDoubleData("FitE_BetheBloch_Down") - Epi;
          std::cout << "Pass hadron pion Mass Up "
                    << hadron_pion_E_BetheBloch_Down[iter] << "\n";
        }
      } else {
      ...
      m_LikelihoodPIDTool->makeParticles(
        hadronProng, particlesLikelihood, particleHypotheses);
      ...
      if (pionPart1->hasDoubleData("score1_Mass_Up"))
        hadron_piFit_score1_Mass_Up[iter] =
          pionPart1->getDoubleData("score1_Mass_Up") -
          hadron_piFit_score1[iter];
    }
    ...
    for (int jj = 0; jj < nMax; jj++) {
      ...
      if (iread >= 0) {
        ...
        pEnergies[jj] =
          node->idcluster()->energy() /
          sqrt(1 + pow(node->state().ax(), 2) + pow(node->state().ay(), 2));
      }
    }
    ...
    StatusCode sc = TruthMatcher->getPrimaryTG4Trajectory(
      hadronProng, hadron_trajectory, fraction, other_energy, false);
    ...
    hadron_tm_beginKE[iter] = hadron_trajectory->GetInitialMomentum().E() -
                              hadron_trajectory->GetInitialMomentum().M();
    ...
    if (MCTracksVector.size() > 0) {
      ...
      for (unsigned int i_mct = 0; i_mct < MCTracksVector.size(); ++i_mct) {
        ...
        if (MCTracksVector[i_mct].GetParentTrackId() != -1)
          continue;
        ...
        // block.
        if (mct_pdg == hadron_tm_PDGCode[iter] &&
            mct_E == hadron_trajectory->GetInitialMomentum().E()) {
          foundMCT = true;
          ...
          while (isElastic && counter_isElastic < 50) {
            ...
            first_inelastic_distance +=
              sqrt(pow(MCTracksVector[parent_idx].GetInitialPosition().x() -
                         MCTracksVector[parent_idx].GetFinalPosition().x(),
                       2.0) +
                   pow(MCTracksVector[parent_idx].GetInitialPosition().y() -
                         MCTracksVector[parent_idx].GetFinalPosition().y(),
                       2.0) +
                   pow(MCTracksVector[parent_idx].GetInitialPosition().z() -
                         MCTracksVector[parent_idx].GetFinalPosition().z(),
                       2.0));
            DaughtersVector = MCTracksVector[parent_idx].GetDaughterIndices();
            debug() << "    Daughter size is " << DaughtersVector.size()
                    << endmsg;
            if (DaughtersVector.empty())
              isElastic = false;
            else {
              for (std::vector<int>::iterator itD = DaughtersVector.begin();
                   itD != DaughtersVector.end();
                   ++itD) {
                int d_idx = *itD;
                debug() << "      Daughter PDG: "
                        << MCTracksVector[d_idx].GetPDGCode() << " Process: "
                        << MCTracksVector[d_idx].GetProcessName() << endmsg;

                // Elastic scatter
                if (MCTracksVector[d_idx].GetPDGCode() == mct_pdg &&
                    MCTracksVector[d_idx].GetProcessName() ==
                      "IsTheMotherParticle") {
                  parent_idx = d_idx; // update parent_idx to get new daughters

                  // it's elastic, but is it a small scattering angle (<15 deg)?
                  Gaudi::LorentzVector tmp_daught_4p(
                    MCTracksVector[d_idx].GetInitialMomentum());
                  Gaudi::LorentzVector tmp_parent_4p(
                    MCTracksVector[parent_idx].GetFinalMomentum());
                  double scat_angle_rad =
                    calcOpeningAngle(tmp_daught_4p, tmp_parent_4p);
                  if (scat_angle_rad < 0.2618)
                    isElastic = true;
                  else
                    isElastic = false;

                  debug() << "      Daughter angle is " << scat_angle_rad
                          << endmsg;
                  break;
                }

                // Inelastic scatter
                else if (MCTracksVector[d_idx].GetProcessName() !=
                           "IsTheMotherParticle" &&
                         MCTracksVector[d_idx].GetProcessName() !=
                           "hadElastic") {
                  isElastic = false;
          ...
          if (counter_isElastic >= 50) {
            ...
            first_inelastic_distance =
              sqrt(pow(MCTracksVector[i_mct].GetInitialPosition().x() -
                         MCTracksVector[i_mct].GetFinalPosition().x(),
                       2.0) +
                   pow(MCTracksVector[i_mct].GetInitialPosition().y() -
                         MCTracksVector[i_mct].GetFinalPosition().y(),
                       2.0) +
                   pow(MCTracksVector[i_mct].GetInitialPosition().z() -
                         MCTracksVector[i_mct].GetFinalPosition().z(),
                       2.0));
          }
          ...
          // now get information about the first hadronic interaction for
          // reweighting, starting with the daughter particle the previous loops
          // ends with
          counter_isElastic = 0;
          int tmp_part_idx = parent_idx;
          while (first_inelastic == 0 && counter_isElastic < 50) {
            counter_isElastic++;
            first_inelastic_endp =
              MCTracksVector[tmp_part_idx].GetFinalMomentum().P();

            // already counted the distance for the initial particle in the
            // previous sum
            if (counter_isElastic != 0)
              first_inelastic_distance +=
                sqrt(pow(MCTracksVector[tmp_part_idx].GetInitialPosition().x() -
                           MCTracksVector[tmp_part_idx].GetFinalPosition().x(),
                         2.0) +
                     pow(MCTracksVector[tmp_part_idx].GetInitialPosition().y() -
                           MCTracksVector[tmp_part_idx].GetFinalPosition().y(),
                         2.0) +
                     pow(MCTracksVector[tmp_part_idx].GetInitialPosition().z() -
                           MCTracksVector[tmp_part_idx].GetFinalPosition().z(),
                         2.0));
            std::vector<int> tmp_dvec =
              MCTracksVector[tmp_part_idx].GetDaughterIndices();
            if (tmp_dvec.empty()) {
              first_inelastic = 0;
              break;
            } else {
              ...
              for (std::vector<int>::iterator itD = tmp_dvec.begin();
                   itD != tmp_dvec.end();
                   ++itD) {
                int d_idx = *itD;
                if (MCTracksVector[d_idx].GetProcessName() ==
                    "IsTheMotherParticle") {
                  tmp_part_idx = d_idx;
                  isElastic = true;
                  break;
              ...
              if (isElastic)
                ...
                first_inelastic = 2;
                break;
              } else if (particle_counter > 0)
                first_inelastic = 4;
              else
                first_inelastic = 1;
            }
          }
          if (first_inelastic != 2 && first_inelastic_endp < 25.0 * CLHEP::MeV)
            first_inelastic = 0;
          ...
          for (unsigned int n = 0; n < DaughtersVector.size(); ++n) {
            ...
            if (MCTracksVector[daught_index].GetProcessName() == "Decay")
              isDecay = true;
            else if (MCTracksVector[daught_index].GetProcessName() ==
                     "IsTheMotherParticle")
              isHighAngleElastic = true;
            else if (MCTracksVector[daught_index].GetPDGCode() == 111)
              isPi0 = true;
            else if (abs(MCTracksVector[daught_index].GetPDGCode()) == 2212)
              isProton = true;
            else if (abs(MCTracksVector[daught_index].GetPDGCode()) == 2112)
              isNeutron = true;
            else if (abs(MCTracksVector[daught_index].GetPDGCode()) == 211)
              isChargedPion = true;

            if (mct_pdg == 211)
              debug() << "  Mother (" << mct_p << ") Daughter ("
                      << MCTracksVector[daught_index].GetInitialMomentum().P()
                      << "," << MCTracksVector[daught_index].GetPDGCode()
                      << ") " << n << " created by: "
                      << MCTracksVector[daught_index].GetProcessName()
                      << endmsg;
          }

          // NI: 0   AIF: 1   DIF: 2   QEX: 3   K: 4  Elastic-K: 5
          if (abs(mct_pdg) == 211) {
            if (mct_p < 25.0 * CLHEP::MeV)
              topology = 0;
            else if (isDecay)
              topology = 2;
            else if (isHighAngleElastic)
              topology = 5;
    ...
    //        TG4Trajectory* hadron_daughter_trajectory =
    //        getMostEnergeticDaughter(hadron_trajectory); if
    //        (hadron_daughter_trajectory) {
    //          hadron_tm_PDGCodeDaughter[iter] =
    //          hadron_daughter_trajectory->GetPDGCode();
    //        }
  ...
  if (pionPart) {
    if (!pionPart->isMultiMass()) {
      ...
      pion_score1 =
        pionPart->getDoubleData(ParticleExtraDataDefs::dEdXScore1());
      pion_score2 =
        pionPart->getDoubleData(ParticleExtraDataDefs::dEdXScore2());
    }
  }
  ...
  if (secondaryProtonsFound) {
    for (ProngVect::iterator secprongs = secondaryProtonProngs.begin();
         secprongs != secondaryProtonProngs.end();
         ++secprongs) {
      double secproton_pion_score = -9999., secproton_pion_score1 = -9999.,
             secproton_pion_score2 = -999.;

      SmartRefVector<Minerva::Particle> secpionparticles =
        (*secprongs)->particlesWithIDCode(Minerva::Particle::Pion, "dEdX");
      if (secpionparticles.size() > 0) {
        ...
        if (!secprongPart->isMultiMass()) {
          ...
          secproton_pion_score1 =
            secprongPart->getDoubleData(ParticleExtraDataDefs::dEdXScore1());
          secproton_pion_score2 =
            secprongPart->getDoubleData(ParticleExtraDataDefs::dEdXScore2());
  ...
  enu_muon = PhysicsCalculator->nuEnergyCCQE(
    muon_E, muon_P, muon_theta, muon_charge, m_nuCCQEBindingEnergyMeV);
  q2_muon = PhysicsCalculator->qSquaredCCQE(
    muon_E, muon_P, muon_theta, muon_charge, m_nuCCQEBindingEnergyMeV);
  ...
  //--------------------------------------------------------
  if (protonFound)
    enu_proton = m_protonUtils->nuEnergyCCQE(
      proton_theta, proton_T, m_nuCCQEBindingEnergyMeV);
  ...
  if (targ)
    debug() << "   This is a passive target event with TargetCode = "
            << targetCode << endmsg;
  ...
  if (targ) {
    vector<double>
      orig_short_vtx; // what was the vertex before adustment to target?
    if (ccqeHyp->getContainerDoubleData("orig_short_vtx", orig_short_vtx))
      ccqeHyp->setDoubleData("target_zDist",
                             targ->distanceToTarget(orig_short_vtx[2]));
    else
      ccqeHyp->setDoubleData("target_zDist",
                             targ->distanceToTarget(vtxPos.z()));

    ccqeHyp->setDoubleData("target_dist_to_division",
                           targ->distanceToDivision(vtxPos.x(), vtxPos.y()));
  }
  ...
  ccqeHyp->setContainerDoubleData("hadron_pion_E_recoil_corr",
                                  hadron_pion_E_recoil_corr);
  ccqeHyp->setContainerDoubleData("hadron_piFit_scoreLLR",
                                  hadron_piFit_scoreLLR);
  ...
  ccqeHyp->setContainerIntData("hadron_tm_destructCode",
                               hadron_tm_destructCode);
  ...
  ccqeHyp->setContainerIntData("hadron_tm_isTruthMatched",
                               hadron_tm_isTruthMatched);
  ccqeHyp->setContainerDoubleData("hadron_tm_fraction", hadron_tm_fraction);
  ccqeHyp->setContainerIntData("hadron_endMichel_category",
                               hadron_endMichel_category);
  ccqeHyp->setContainerDoubleData("hadron_endMichel_slice_energy",
                                  hadron_endMichel_slice_energy);
  ccqeHyp->setContainerIntData("hadron_endMichel_ndigits",
                               hadron_endMichel_ndigits);
  ccqeHyp->setContainerDoubleData("hadron_endMichel_energy",
                                  hadron_endMichel_energy);
  ccqeHyp->setContainerDoubleData("piFit_score1_Mass_biasUp",
                                  hadron_piFit_score1_Mass_Up);
  ccqeHyp->setContainerDoubleData("piFit_score1_Mass_biasDown",
                                  hadron_piFit_score1_Mass_Down);
  ccqeHyp->setContainerDoubleData("piFit_score1_BetheBloch_biasUp",
                                  hadron_piFit_score1_BetheBloch_Up);
  ccqeHyp->setContainerDoubleData("piFit_score1_BetheBloch_biasDown",
                                  hadron_piFit_score1_BetheBloch_Down);
  ccqeHyp->setContainerDoubleData("piFit_score1_Birks",
                                  hadron_piFit_score1_Birks);
  ccqeHyp->setContainerDoubleData("pion_E_Mass_biasUp", hadron_pion_E_Mass_Up);
  ccqeHyp->setContainerDoubleData("pion_E_Mass_biasDown",
                                  hadron_pion_E_Mass_Down);
  ccqeHyp->setContainerDoubleData("pion_E_BetheBloch_biasUp",
                                  hadron_pion_E_BetheBloch_Up);
  ccqeHyp->setContainerDoubleData("pion_E_BetheBloch_biasDown",
                                  hadron_pion_E_BetheBloch_Down);
  ccqeHyp->setContainerDoubleData("pion_theta_biasUp",
                                  hadron_pion_theta_biasUp);
  ccqeHyp->setContainerDoubleData("pion_theta_biasDown",
                                  hadron_pion_theta_biasDown);
  ...
  ccqeHyp->setContainerDoubleData("sec_protons_theta_fromdEdx",
                                  sec_protons_theta);
  ccqeHyp->setContainerDoubleData("sec_protons_T_fromCalo", sec_protons_T_Calo);
  ccqeHyp->setContainerIntData("sec_protons_patternRec",
                               sec_protons_patternRec);
  ccqeHyp->setContainerDoubleData("sec_protons_proton_scores",
                                  sec_protons_proton_scores);
  ccqeHyp->setContainerDoubleData("sec_protons_proton_scores1",
                                  sec_protons_proton_scores1);
  ccqeHyp->setContainerDoubleData("sec_protons_proton_scores2",
                                  sec_protons_proton_scores2);
  ccqeHyp->setContainerDoubleData("sec_protons_pion_scores",
                                  sec_protons_pion_scores);
  ccqeHyp->setContainerDoubleData("sec_protons_pion_scores1",
                                  sec_protons_pion_scores1);
  ccqeHyp->setContainerDoubleData("sec_protons_pion_scores2",
                                  sec_protons_pion_scores2);
  ...
  ccqeHyp->setDoubleData("proton_score1_Mass_biasDown",
                         proton_score1_Mass_Down);
  ccqeHyp->setDoubleData("proton_score1_BetheBloch_biasUp",
                         proton_score1_BetheBloch_Up);
  ccqeHyp->setDoubleData("proton_score1_BetheBloch_biasDown",
                         proton_score1_BetheBloch_Down);
  ...
  ccqeHyp->setDoubleData("proton_E_BetheBloch_biasDown",
                         proton_E_BetheBloch_Down);
  ...
  ccqeHyp->setContainerDoubleData("sec_protons_score1_Mass_biasUp",
                                  sec_protons_score1_Mass_Up);
  ccqeHyp->setContainerDoubleData("sec_protons_score1_Mass_biasDown",
                                  sec_protons_score1_Mass_Down);
  ccqeHyp->setContainerDoubleData("sec_protons_score1_BetheBloch_biasUp",
                                  sec_protons_score1_BetheBloch_Up);
  ccqeHyp->setContainerDoubleData("sec_protons_score1_BetheBloch_biasDown",
                                  sec_protons_score1_BetheBloch_Down);
  ccqeHyp->setContainerDoubleData("sec_protons_score1_MEU_biasUp",
                                  sec_protons_score1_MEU_Up);
  ccqeHyp->setContainerDoubleData("sec_protons_score1_MEU_biasDown",
                                  sec_protons_score1_MEU_Down);
  ccqeHyp->setContainerDoubleData("sec_protons_score1_Birks_bias",
                                  sec_protons_score1_Birks);
  ccqeHyp->setContainerDoubleData("sec_protons_E_Mass_biasUp",
                                  sec_protons_E_Mass_Up);
  ccqeHyp->setContainerDoubleData("sec_protons_E_Mass_biasDown",
                                  sec_protons_E_Mass_Down);
  ccqeHyp->setContainerDoubleData("sec_protons_E_BetheBloch_biasUp",
                                  sec_protons_E_BetheBloch_Up);
  ccqeHyp->setContainerDoubleData("sec_protons_E_BetheBloch_biasDown",
                                  sec_protons_E_BetheBloch_Down);
  ccqeHyp->setContainerDoubleData("sec_protons_E_MEU_biasUp",
                                  sec_protons_E_MEU_Up);
  ccqeHyp->setContainerDoubleData("sec_protons_E_MEU_biasDown",
                                  sec_protons_E_MEU_Down);
  ccqeHyp->setContainerDoubleData("sec_protons_E_Birks_bias",
                                  sec_protons_E_Birks);
  ...
  ccqeHyp->setContainerDoubleData("proton_nodes_nodesNormE",
                                  proton_nodes_nodesNormE);

  ccqeHyp->setContainerIntData("sec_protons_prongType", sec_protons_prongType);
  ccqeHyp->setContainerIntData("sec_protons_nodes_index",
                               sec_protons_nodes_index);
  ccqeHyp->setContainerDoubleData("sec_protons_nodes_nodesNormE",
                                  sec_protons_nodes_nodesNormE);
  ...
  if (m_doNeutron) {
    ...
    neutron4P.push_back(neutronPart->momentumVec().X());
    neutron4P.push_back(neutronPart->momentumVec().Y());
    neutron4P.push_back(neutronPart->momentumVec().Z());
    ...
    for (uint i = 0; i < m_neutronBlobUtils->GetIntDataMemberNames().size();
      ...
      for (uint j = 0; j < m_neutronBlobUtils->GetDataInt()[key].size(); j++) {
        debug() << "Key " << key << " contains "
                << m_neutronBlobUtils->GetDataInt()[key][j] << endmsg;

        if (key == "BlobNTracks")
          NTracks += m_neutronBlobUtils->GetDataInt()[key][j];
        else if (key == "BlobNIncTracks")
          NIncTracks += m_neutronBlobUtils->GetDataInt()[key][j];
      }
    ...
    for (uint i = 0; i < m_neutronBlobUtils->GetDoubleDataMemberNames().size();
      ...
      for (uint j = 0; j < m_neutronBlobUtils->GetDataDouble()[key].size(); j++)
        debug() << "Key " << key << " contains "
                << m_neutronBlobUtils->GetDataDouble()[key][j] << endmsg;
      ccqeHyp->setContainerDoubleData(key,
                                      m_neutronBlobUtils->GetDataDouble()[key]);
    }
  ...
  // this isn't the best way to get the mass of the struck nucleon, but it's
  // fine for now
  ...
  SmartRefVector<Minerva::IDCluster> idClusters =
    getAnalyzableNonMuIDClusters(event, muonProng, true);
  SmartRefVector<Minerva::ODCluster> odClusters =
    getAnalyzableNonMuODClusters(event, muonProng, true);

  double recoilE_passive =
    m_caloUtils->applyCalConsts(idClusters, odClusters, "Default", false);
  // double recoilE_passive = m_caloUtils->applyCalConsts( idClusters,
  // odClusters , recoilChannel , true);

  double totalE = muon_E + recoilE;

  // double bindingE   = ( muon_charge < 0 ) ? m_nuCCQEBindingE :
  // m_antinuCCQEBindingE; double qsquaredQE = PhysicsCalculator->qSquaredCCQE(
  // muonPart->momentumVec().E(), muonPart->momentumVec().P(), muon_theta,
  // muon_charge, m_nuCCQEBindingEnergyMeV );

  // Calculate W & Bjorken x
  // double qsquared   = PhysicsCalculator->qSquared( totalE, muonE, mu_theta );
  // original
  ...
  //      I recommend we think about putting this in a separate CCInclusive
  //      hypothesis.
  ...
  {
    double recoilE_wide = ccqeHyp->getDoubleData(
      "recoil_E_wide_window"); // we trust that it is filled in getRecoilEnergy
    double totalE_wide = muon_E + recoilE_wide;

    // Calculate E, W, x, y, q2 for the shfited recoil
    double qsquared_wide =
      PhysicsCalculator->qSquared(totalE_wide, muon_E, muon_theta);
    double W_wide =
      PhysicsCalculator->W(nucleonMass, recoilE_wide, qsquared_wide);
    double xBj_wide =
      PhysicsCalculator->xBjorken(qsquared_wide, nucleonMass, recoilE_wide);
...
//================================================================================
bool
MasterAnaDev::adjustDNNVertexIntoCenterOfSegment(
  SmartRef<Minerva::Vertex>& vtx,
  double zCenter,
  const SmartRef<Minerva::Particle>& muonPart) const
{
  ...
  double projx =
    muon_vtx.x() + (muon_p.x() / muon_p.z()) * (zCenter - muon_vtx.z());
  double projy =
    muon_vtx.y() + (muon_p.y() / muon_p.z()) * (zCenter - muon_vtx.z());
  Gaudi::XYZTVector projVtx =
    Gaudi::XYZTVector(projx, projy, zCenter, muon_vtx.t());
  vtx->setPosition(Gaudi::XYZPoint(projVtx.x(), projVtx.y(), projVtx.z()));
  return true;
}

bool
MasterAnaDev::adjustVertexIntoPassiveTarget(
  SmartRef<Minerva::Vertex>& vtx,
  const SmartRef<Minerva::Particle>& muonPart) const
{
  ...
  info()
    << "***** targetID, z start, z center, z end, vertex pos, targetCode = "
    << targ->getTargetID() << " " << targ->getZStart() << " "
    << targ->getZCenter() << " " << targ->getZEnd() << " "
    << vtx->position().z() << " " << targetCode << endmsg;
  if (!targ)
    return false;

  //! If this event comes from a target, project to the zCenter of it and update
  //! the vertex position to that point.
...
//=======================================================
const Minerva::NuclearTarget*
MasterAnaDev::getTarget(const Gaudi::XYZPoint& p, int& targetCode) const
{
  const Minerva::NuclearTarget* targ = 0;
  targetCode = 0;

  //! Loop over targets to find one in which the muon might have originated
  for (vector<Minerva::NuclearTarget*>::const_iterator itTarg =
         m_targets.begin();
       itTarg != m_targets.end();
       ++itTarg) {
    debug() << "Checking muon against target " << (*itTarg)->getName()
            << endmsg;

    double zLow =
      m_nukeTool->getZ_nPlanesUS((*itTarg), m_nplanes_USVertexLimit);
    double zHigh =
      m_nukeTool->getZ_nPlanesDS((*itTarg), m_nplanes_DSVertexLimit);
    ...
    if (FiducialPointTool->isFiducial(p, 99999999.9, zLow, zHigh)) {
      debug() << "   Z position of the event is consistent with an event from "
                 "the target"
              << endmsg;
      targ = *itTarg;
      ...
      //! targetCode = 1000*targetID + targetZ.  targetZ is 0 if material is
      //! unknown
      ...
      if (mat)
        debug() << "Mat Z after cast: " << targetCode << ", "
                << TMath::Nint(mat->Z()) << endl;
...
const Minerva::NuclearTarget*
MasterAnaDev::getTarget(const Minerva::GenMinInteraction* truth,
                        int& targetCode) const
{
  return getTarget(
    Gaudi::XYZPoint(truth->Vtx().x(), truth->Vtx().y(), truth->Vtx().z()),
    targetCode);
}
...
//=======================================================
const Minerva::NuclearTarget*
MasterAnaDev::getExactTarget(const Gaudi::XYZPoint& p, int& targetCode) const
{
  targetCode = 0;
  // When calculating target Z:
  // Use Nint to round to nearest integer Z when calculating target code
  ...
  if (targ) {
    ...
    if (targ->getTargetID() != 3 && 4978.95 <= p.z() && p.z() <= 4983.98)
      targetCode = 1000 * 3;
    if (5637.53 <= p.z() && p.z() <= 5645.73)
      targetCode = 1000 * 4;
    ...
    if (mat)
      ...
      warning()
        << "Cannot identify a section material.  Something may be wrong."
        << endmsg;
  }
...
const Minerva::NuclearTarget*
MasterAnaDev::getExactTarget(const Minerva::GenMinInteraction* truth,
                             int& targetCode) const
{
  return getExactTarget(
    Gaudi::XYZPoint(truth->Vtx().x(), truth->Vtx().y(), truth->Vtx().z()),
    targetCode);
}
...
//=======================================================
bool
MasterAnaDev::getReferenceZ(vector<int>& intVec, double x, double y) const
{
  ...
  for (vector<Minerva::NuclearTarget*>::const_iterator itTarg =
    ...
    if (mat)
      ...
      debug() << "Could not get Z for reference target = "
              << (*itTarg)->getName() << " at (x,y) = ( " << x << ", " << y
              << " ) " << endmsg;
...
//================================================================
bool
MasterAnaDev::getReferenceDistToDivision(vector<double>& dVec,
                                         double x,
                                         double y) const
{
...
bool
MasterAnaDev::getDistToTarget(vector<double>& dVec, double z) const
{
...
MasterAnaDev::fillPlaneLowZMap()
{
  ...
  for (DePlanes::const_iterator plane = planes.begin(); plane != planes.end();
       ++plane) {
    ...
    debug() << "module number: " << modno << " And plane: " << planeno
            << " with view: " << view << endmsg; // Oscar

    // firgure out how far this plane extends US in z before encroaching on
    // another plane
    double deltaZ;
    ...
    if (printPlanes) {
      ...
      //  const double zHigh   = zCenter + deltaZ; //an approximation.  there's
      //  a different gap between planes and modules
      ...
      std::cout << "Module: " << modno << ", Plane: " << planeno
                << ", view: " << viewStr[view]
                << Form("; zCenter: %.2fmm", (*plane)->getZCenter())
                << Form("; zRange[est]: [%.2f,%.2f]", zLow, zHigh)
                << Form("; zRange: [%.2f,%.2f]", zStart, zStop) << std::endl;
    }
  }
...
bool
MasterAnaDev::getVtxPlane(double z, int& modno, int& planeno) const
{
  bool foundPlane = false;

  if (z < m_planeLowZMap.begin()->first) {
    debug() << "MasterAnaDev::getVtxPlane: z point " << z
            << " is farther upstream than any plane." << endmsg;
    ...
    std::map<double, std::pair<int, int>>::const_iterator itPlane =
      --m_planeLowZMap.lower_bound(z);
    const double zLow = itPlane->first;
    modno = itPlane->second.first;
    planeno = itPlane->second.second;

    // firgure out how far DS this plane extends in z before encroaching on
    // another plane, starting from the US point of the plane
    double deltaZ;
    ...
    if (z < zHigh)
      ...
      debug() << "getVtxPlane: Position within fiducial limits but not inside "
                 "a plane at z = "
              << z << endmsg;

    debug() << Form("  getVtxPlane: z = %f - modno = %d, planeno = %d - lowz = "
                    "%f, deltaz = %f, highz = %f",
                    z,
                    modno,
                    planeno,
...
Minerva::ModuleID
MasterAnaDev::getModuleID(int mod) const
{
...
MasterAnaDev::getBlobbingClusters(
  Minerva::PhysicsEvent* event,
  const SmartRef<Minerva::Prong>& muonProng) const
{
  // this gets in-time clusters that are not XTalk and not on the muon prong
  SmartRefVector<Minerva::IDCluster> blobClusters =
    getAnalyzableNonMuIDClusters(event, muonProng);

  for (SmartRefVector<Minerva::IDCluster>::iterator i = blobClusters.begin();
       i != blobClusters.end();) {
    // mark all clusters as unused
    (*i)->setHistory(Minerva::MinervaRecoBase::Unused);

    // remove HCAL and LA clusters which are analyzable for recoil energy but
    // not blobbing
    if (Minerva::IDCluster::HCAL == (*i)->subdet() ||
        Minerva::IDCluster::LowActivity == (*i)->type())
...
SmartRefVector<Minerva::IDCluster>
MasterAnaDev::removeUsedClusters(
  SmartRefVector<Minerva::IDCluster>& clusters) const
{
  SmartRefVector<Minerva::IDCluster> used;
  for (SmartRefVector<Minerva::IDCluster>::iterator i = clusters.begin();
       i != clusters.end();) {
...
double
MasterAnaDev::fillBlobBranches(Minerva::PhysicsEvent* event,
                               SmartRefVector<Minerva::IDCluster>& idclusters,
                               const string& prefix,
                               int color) const
{
                               ...
                               int color) const
{
  ...
  event->setIntData(string("blob_" + prefix + "_nClus"),
                    idclusters.size() + odclusters.size());
  event->setIntData(string("blob_" + prefix + "_nClus_nucl"), nucl.size());
  event->setIntData(string("blob_" + prefix + "_nClus_tracker"),
                    tracker.size());
  ...
  if (0 != color) {
    for (SmartRefVector<Minerva::IDCluster>::iterator i = idclusters.begin();
         i != idclusters.end();
         ++i)
      m_hitTaggerTool->applyColorTag(*i, color);

    for (SmartRefVector<Minerva::ODCluster>::iterator i = odclusters.begin();
         i != odclusters.end();
         ++i)
      m_hitTaggerTool->applyColorTag(*i, color);
  }
  ...
  // if MC, add e by particle type
  if (haveNeutrinoMC()) {
    for (std::map<MCPartType::t_type, string>::iterator i =
           PART_TYPE_MAP.begin();
         i != PART_TYPE_MAP.end();
         ++i) {
      ...
      SmartRefVector<Minerva::IDCluster> nucl_fType =
        getClustersCreatedBy<Minerva::IDCluster, Minerva::IDDigit>(nucl, type);
      SmartRefVector<Minerva::IDCluster> tracker_fType =
        getClustersCreatedBy<Minerva::IDCluster, Minerva::IDDigit>(tracker,
                                                                   type);
      SmartRefVector<Minerva::IDCluster> ecal_fType =
        getClustersCreatedBy<Minerva::IDCluster, Minerva::IDDigit>(ecal, type);
      SmartRefVector<Minerva::IDCluster> hcal_fType =
        getClustersCreatedBy<Minerva::IDCluster, Minerva::IDDigit>(hcal, type);
      SmartRefVector<Minerva::ODCluster> odclusters_fType =
        getClustersCreatedBy<Minerva::ODCluster, Minerva::ODDigit>(odclusters,
                                                                   type);
      ...
      event->setDoubleData(string("blob_" + prefix + "_E_tracker_" + name),
                           trackerE);
...
/////////////////////////////////////////////////////////////////////////////////////////
void
MasterAnaDev::separateClustersBySubdet(
  ...
  SmartRefVector<Minerva::IDCluster>& hcal) const
{
  for (SmartRefVector<Minerva::IDCluster>::const_iterator i = clusters.begin();
       i != clusters.end();
       ++i) {
...
void
MasterAnaDev::declareBlobBranches(const string& prefixBefore)
{
  // loop over vertex blob radius variations for all blobs but the recoil blob
  const unsigned int nVtxRadii =
    (prefixBefore == RECOILBLOB_PREFIX) ? 1 : m_nuVtxBlobRadii.size();
  for (unsigned int i = 0; i != nVtxRadii; ++i) {
    ...
    for (std::map<MCPartType::t_type, string>::iterator i =
      ...
      declareDoubleEventBranch("blob_" + prefix + "_E_tracker_" + i->second,
                               0.);
...
SmartRefVector<ClusterType>
MasterAnaDev::getClustersCreatedBy(SmartRefVector<ClusterType> clusters,
                                   MCPartType::t_type primType,
                                   MCPartType::t_type fsType) const
{

  SmartRefVector<ClusterType> rval;
  for (typename SmartRefVector<ClusterType>::iterator clus = clusters.begin();
       clus != clusters.end();
       ++clus) {

    if (primType == getParticleCreatedBy<ClusterType, DigitType>(
                      *clus, MCPartStatus::kPrimary)) {
      if (MCPartType::kAny == fsType ||
          fsType == getParticleCreatedBy<ClusterType, DigitType>(
                      *clus, MCPartStatus::kFinal))
        rval.push_back(*clus);
    }
  }
...
template<typename ClusterType, typename DigitType>
MCPartType::t_type
MasterAnaDev::getParticleCreatedBy(SmartRef<ClusterType> cluster,
                                   MCPartStatus::t_type status) const
{
  if (status == MCPartStatus::kPrimary) {
    ...
    for (typename SmartRefVector<DigitType>::iterator dig = digits.begin();
         dig != digits.end();
         ++dig) {
      ...
      if (!digVec.empty())
        TruthMatcher->getPrimaryTG4Trajectory(
          digVec, constTrajectory, fraction, other_energy);
      if (0 == constTrajectory) { // no trajectory means Xtalk
        xtalkEnergy += normE;
        ...
        switch (pdg) {
          case 2112: // neutron
          {
            ...
            // double neutron_ke = neutron_mom*neutron_mom/2/MinervaUnits::M_n;
            // //<--- nonrelativistic?
            double neutron_ke =
              sqrt(pow(neutron_mom, 2) + pow(MinervaUnits::M_n, 2)) -
              MinervaUnits::M_n;
    ...
    if (!clusVec.empty())
      TruthMatcher->getTG4Trajectory(
        clusVec, constTrajectory, fraction, other_energy);
  ...
  warning() << "Asked for particle with unknown status.  status can be Primary "
               "or Final."
            << endmsg;
...
void
MasterAnaDev::calcMomentumByRangeVars(Minerva::PhysicsEvent* event) const
{
  // Declare vars
  ...
  for (unsigned int prong = 0; prong < pion_prongs.size(); prong++) {
    ...
    // loop prong's tracks
    const SmartRefVector<Minerva::Track>& tracks =
      pion_prongs[prong]->minervaTracks();
    info() << "prong ntracks: " << tracks.size() << endmsg;
    for (SmartRefVector<Minerva::Track>::const_iterator t = tracks.begin();
         t != tracks.end();
         ++t) {
      ...
      for (std::vector<Minerva::Node*>::const_iterator n = nodes.begin();
           n != nodes.end();
           ++n) {
        std::vector<Minerva::Node*>::const_iterator current = n;
        std::vector<Minerva::Node*>::const_iterator next = n;
        ++next;
        ...
        Gaudi::XYZPoint end(
          (*next)->state().x(), (*next)->state().y(), (*next)->state().z());
        ...
        // ignore the case where one node is upstream and the other is
        // downstream of the boundary
        if (start.z() > tracker_ecal_boundary &&
            end.z() > tracker_ecal_boundary) {
  ...
  event->setContainerDoubleData("hadron_track_length_area",
                                hadron_track_length_area);
  event->setContainerDoubleData("hadron_track_length_area_trkr",
                                hadron_track_length_area_trkr);
  event->setContainerDoubleData("hadron_track_length_area_ecal",
                                hadron_track_length_area_ecal);
...
//==============================================================================
double
MasterAnaDev::calcOpeningAngle(Gaudi::LorentzVector& pion_vec,
                               Gaudi::LorentzVector& muon_vec) const
{
  double pionMuonAngle = pion_vec.px() * muon_vec.px() +
                         pion_vec.py() * muon_vec.py() +
                         pion_vec.pz() * muon_vec.pz();
  pionMuonAngle /= sqrt(pow(pion_vec.px(), 2.0) + pow(pion_vec.py(), 2.0) +
                        pow(pion_vec.pz(), 2.0));
  pionMuonAngle /= sqrt(pow(muon_vec.px(), 2.0) + pow(muon_vec.py(), 2.0) +
                        pow(muon_vec.pz(), 2.0));
...
//==============================================================================
void
MasterAnaDev::fillIsoProngInfo(Minerva::PhysicsEvent* event,
                               double muon_time) const
{
  ...
  std::vector<Minerva::Prong*>* new_iso_ID_prongs =
    new std::vector<Minerva::Prong*>;
  Minerva::IDClusterVect unused_clusts_for_blobs =
    event->select<Minerva::IDCluster>("Unused", "!LowActivity&!XTalkCandidate");
  StatusCode sc = m_primaryBlobProngTool->makeShowerBlobProngs(
    event, unused_clusts_for_blobs, new_iso_ID_prongs);
  if (sc.isFailure()) {
  ...
  //! before you start adding them to the evtMgr, user has control !
  //--------------------------------------------------------------------------
  if (new_iso_ID_prongs->size())
    addObject(event, *new_iso_ID_prongs);
  ...
  int p_index = 0;
  for (Minerva::ProngVect::iterator p = prongs.begin();
       p != prongs.end() && p_index < 30;
       ++p, ++p_index) {
    warning() << "Prong Creation Signature = " << (*p)->creationSignature()
              << endmsg;
    ...
    if ((*p)->creationSignature() ==
      ...
      double prong_energy =
        m_caloUtils->applyCalConsts(*p, "CCInclusive", false);
      ...
      for (Minerva::IDBlobVect::iterator it_blob = blobs.begin();
           it_blob != blobs.end();
           it_blob++) {
...
//==============================================================================
////// Return the particle hypothesis with the largest score for a given
///particle
////// type
//////==============================================================================
//
...
StatusCode
MasterAnaDev::getBestParticle(SmartRef<Minerva::Prong> prong,
                              ...
                              Minerva::Particle::ID partType) const
{
  ...
  verbose() << "  Prong has " << partHypVec.size() << " particle hypotheses"
            << endmsg;
  for (itPart = partHypVec.begin(); itPart != partHypVec.end(); ++itPart) {
    double score1 = -9.0;
    if ((*itPart)->isMultiMass())
      continue;
    if ((*itPart)->hasDoubleData("score1"))
      score1 = (*itPart)->getDoubleData("score1");
    verbose() << "    score1 " << score1 << " part " << (*itPart)->idcode()
              << endmsg;
...
// truthIsPlausible( ) is now a pure virtual function of MinervaAnalysisTool, so
// you must implement it It is called automatically by PhysicsEventAnalysisAlg
// AFTER reconstructEvent() and interpretEvent() are run. If it returns false,
// the event is not included in the analysis ntuple
//===================================================================================================================
bool
MasterAnaDev::truthIsPlausible(const Minerva::PhysicsEvent* physEvent) const
{

  info() << "Entering truthIsPlausible( ) now ..... " << endmsg;

  if (NULL == physEvent) {
    debug() << "Physics Event is a NULL pointer - no sense in proceeding !"
            << endmsg;
    return false;
  }

  // This might need more work.
  // Look at all prongs. If ONE prong is the mc event as defined below then true
  // else false
  if (!physEvent->hasInteractionVertex()) {
    bool isPlausible = false;
    VertexVect used_vtx =
      physEvent->select<Minerva::Vertex>("Used", "StartPoint");
    for (Minerva::VertexVect::iterator itvtx = used_vtx.begin();
         itvtx != used_vtx.end();
         ++itvtx) {
      ProngVect primProngs;
      ...
      for (ProngVect::const_iterator itProng = primProngs.begin();
           itProng != primProngs.end();
           ++itProng) {
        if ((*itProng)->MinosTrack() || (*itProng)->MinosStub())
          isPlausible = muonIsPlausible((*itProng));
        if (isPlausible)
          return true;
      }
  ...
  //------------------------------------------------------------------------------------------------------------
  // There are 2 methods available here. One uses muonIsPlausible( ), the other
  // is a more general method. The general method could be used for the muon or
  // proton analysis, if using this tool.
  //------------------------------------------------------------------------------------------------------------
  if (m_useMuonIsPlausible) {
        ...
        if ((*itProng)->hasIntData("look_for_plausibility")) {
          if ((*itProng)->getIntData("look_for_plausibility") == 1) {
            ...
            if (!is_muon_plausible) {
              debug() << "This MC muon is not plausible because it has a MC "
                         "energy fraction below threshold !"
                      << endmsg;
              return false;
            } else {
          ...
          debug() << "This primary Prong does not have look_for_plausibility "
                     "set and is probably not a muon Prong !"
                  << endmsg;
        }
      }
      if (!foundDeProng)
        throw MinervaException(
          "truthIsPlausible: Did not find muon Prong amongst primary Prongs, "
          "even though you had marked it earlier !");

    } else {
      debug() << "There are no Primary Prongs in this Physics Event !"
              << endmsg;
      return false;
    }
    ...
    // This relies on the assumption that rock muons are filtered out early on
    // and are not part of the physics event.
    //------------------------------------------------------------------------------------------
    SmartRef<Minerva::Track> deLongestTrack;
    bool foundaTrack = findLongestMinervaTrack(physEvent, deLongestTrack);
    verbose() << "Inside truthIsPlausible(): Value of foundaTrack "
              << foundaTrack << endmsg;
    if (foundaTrack && deLongestTrack->nNodes()) {
      debug() << "Inside truthIsPlausible(): Chi2 of deLongestTrack from "
                 "findLongestMinervaTrack( ) is "
              << deLongestTrack->chi2PerDoF()
              << " Nodes: " << deLongestTrack->nNodes() << endmsg;
    } else {
    ...
    //----------------------------------------------------------------------------------------------------------------
    // Find all the Prongs that hold this track and verify if they all carry MC
    // identities If not, then the track probably comes from a data overlay and
    // not MC. Hence it is not your signal event !
    //----------------------------------------------------------------------------------------------------------------
    SmartRefVector<Minerva::Prong> prongsFromLongestTrack;
    StatusCode sc = m_objectAssociator->getProngs_containingMinervaTrack(
      prongsFromLongestTrack, deLongestTrack);
    if (sc.isFailure()) {
    ...
    if (prongsFromLongestTrack.size()) {
      for (SmartRefVector<Minerva::Prong>::iterator itProng =
             prongsFromLongestTrack.begin();
           itProng != prongsFromLongestTrack.end();
           ++itProng) {
        if (!prongIsMC(*itProng))
          return false;
      }
    } else {
      throw MinervaException(
        "The longest track does not belong to a Prong! Please investigate!");
    }
...
//=============================================================================
StatusCode
MasterAnaDev::tagTruth(Minerva::GenMinInteraction* truth) const
{
  ...
  const SmartRefVector<Minerva::TG4Trajectory> pri_trajectories =
    truth->trajectories();
  SmartRefVector<Minerva::TG4Trajectory>::const_iterator it_mcpart;

  int nPiPlus = 0, nChargedK = 0, nLambda = 0, nK0 = 0, nSigma = 0,
      nPiMinus = 0, nPi0 = 0;
  std::vector<int> otherPDG, t_pi_charge;
  std::vector<double> t_pi_px, t_pi_py, t_pi_pz, t_pi_E, t_pi_theta;

  for (int iter = 0; iter < 20; iter++) {
    t_pi_px.push_back(-1.0);
    t_pi_py.push_back(-1.0);
    t_pi_pz.push_back(-1.0);
    t_pi_E.push_back(-1.0);
    t_pi_theta.push_back(-9.0);
    t_pi_charge.push_back(0);
  }

  for (it_mcpart = pri_trajectories.begin();
       it_mcpart != pri_trajectories.end();
       ++it_mcpart) {
    if ((*it_mcpart)->GetParentId() != 0)
      continue;
    ...
    if (abs(partPDG) == 211 && nChargePi < 20) {
      ...
      nChargedK++;
    } else if ((abs(partPDG) == 130 || abs(partPDG) == 310 ||
                abs(partPDG) == 311) &&
               nK0 < 20) {
      nK0++;
  ...
  double t_mu_px = t_mu4p.px(), t_mu_py = t_mu4p.py(), t_mu_pz = t_mu4p.pz(),
         t_mu_E = t_mu4p.E();
  ...
  truth->setDoubleData("muon_theta_wrtbeam",
                       m_minCoordSysTool->thetaWRTBeam(truth->PrimFSLepton()));
  ...
  // True target ID based on NukeCCInclusive.  I think NukeCCInclusive macros
  // use these branches for studying signal definitions.
  // TODO: Is this what standard reconstruction needs too?
  int targetCode = 0;
  getExactTarget(truth, targetCode);

  // Unpack targetCode into a target ID (position in z) and struck nucleus
  // (target Z).
  ...
  truth->setIntData(
    "target_code",
    targetCode); // TODO: Do we need this?  My notes on NukeCCInclusive say no.
  ...
  if (0 < targetID) {
    truth->setDoubleData(
      "target_zDist",
      m_nukeTool->getNuclearTarget(targetID)->distanceToTarget(
        truth->Vtx().z()));
    truth->setDoubleData(
      "target_dist_to_division",
      m_nukeTool->getNuclearTarget(targetID)->distanceToDivision(
        truth->Vtx().x(), truth->Vtx().y()));
  }

  //! Cut 3: Is the vertex in the fiducial/analyzable region
  bool isFiducialXY = FiducialPointTool->isFiducial(
    truth, m_signalApothem, m_usFiducialModID, m_dsFiducialModID);
  bool isAnalyzableXY = FiducialPointTool->isFiducial(
    truth, m_analyzeApothem, m_usAnalyzableModID, m_dsAnalyzableModID);
  ...
  // TODO: NukeCCInclusive team, there's lots of other code about FS particles
  // that could go here.

  //--------------------------------
  // Is it inside fiducial vol ?
  //--------------------------------
  bool is_fiducial = FiducialPointTool->isFiducial(
    truth, m_signalApothem, m_signalUpstreamZ, m_signalDownstreamZ);
  if (truth) {
...
void
MasterAnaDev::tagProtonProngTruth(Minerva::PhysicsEvent* event,
                                  SmartRef<Minerva::Prong> protonProng) const
{
  ...
  //------------------------------------------------------------------
  sc = TruthMatcher->getPrimaryTG4Trajectory(
    protonProng, prong_traj, prong_traj_fractionE, prong_traj_otherE);
  if (sc == StatusCode::FAILURE) {
    debug()
      << "Truth Matching Failed while getting the proton candidate trajectory"
      << endmsg;
    return;
  }
...
void
MasterAnaDev::tagSecondaryProtonProngTruth(
  Minerva::PhysicsEvent* event,
  Minerva::ProngVect protonProngs) const
{
  ...
  for (SmartRefVector<Minerva::Prong>::iterator itProng = protonProngs.begin();
       itProng != protonProngs.end();
       ++itProng) {
    const Minerva::TG4Trajectory* prong_traj = NULL;
    double prong_traj_fractionE = -1.0, prong_traj_otherE = -1.0;
    // int prong_traj_PDG = -1, prong_traj_ID;
    ...
    //------------------------------------------------------------------
    sc = TruthMatcher->getPrimaryTG4Trajectory(
      *itProng, prong_traj, prong_traj_fractionE, prong_traj_otherE);
    if (sc == StatusCode::FAILURE) {
      debug()
        << "Truth Matching Failed while getting the proton candidate trajectory"
        << endmsg;
...
std::vector<double>
MasterAnaDev::tagMichelProngTruth(Minerva::Prong michelProng) const
{
  ...
  std::vector<const Minerva::IDCluster*> idclusters;
  SmartRefVector<Minerva::IDCluster> clustersFromProng =
    michelProng.getAllIDClusters();
  idclusters.insert(
    idclusters.begin(), clustersFromProng.begin(), clustersFromProng.end());
  const Minerva::TG4Trajectory* prong_traj = NULL;
  ...
  //------------------------------------------------------------------
  sc = TruthMatcher->getTG4Trajectory(
    idclusters, prong_traj, prong_traj_fractionE, prong_traj_otherE);
  if (sc == StatusCode::FAILURE) {
    debug()
      << "Truth Matching Failed while getting the michel candidate trajectory"
      << endmsg;
    return bestPart;
  }
  ...
  debug() << "This is the matched information(pdg,pdg,fraction): ("
          << bestPart[0] << "," << bestPart[1] << "," << bestPart[2] << ")"
          << endmsg;
...
std::vector<double>
MasterAnaDev::tagBlobTruth(Minerva::IDBlob blob) const
{
  ...
  iddigits.insert(
    iddigits.begin(), digitsFromBlob.begin(), digitsFromBlob.end());
  ...
  //------------------------------------------------------------------
  sc = TruthMatcher->getTG4Trajectory(
    iddigits, blob_traj, blob_traj_fractionE, blob_traj_otherE);
  if (sc == StatusCode::FAILURE) {
    debug()
      << "Truth Matching Failed while getting the blob candidate trajectory"
      << endmsg;
    return bestPart;
  }
...
std::vector<double>
MasterAnaDev::tagDigitsTruth(SmartRefVector<Minerva::IDDigit> digits) const
{
  ...
  //------------------------------------------------------------------
  sc = TruthMatcher->getTG4Trajectory(
    iddigits, digits_traj, digits_traj_fractionE, digits_traj_otherE);
  if (sc == StatusCode::FAILURE) {
    debug()
      << "Truth Matching Failed while getting the digits candidate trajectory"
      << endmsg;
    return bestPart;
  }
  digits_parent_traj =
    TruthMatcher->getPrimaryTG4Trajectory(digits_traj, false);
  //! Set michel candidate PDG, four-momentum and position
...
//==================================================
bool
MasterAnaDev::createParticles(Minerva::PhysicsEvent* event,
                              Minerva::ProngVect& hadronProngs) const
{
  bool status = true;
  ProngVect primaryProngs = event->primaryProngs();
  std::vector<Minerva::Particle::ID> particleHypotheses;
  ...
  for (ProngVect::iterator itProngs = primaryProngs.begin();
    ...
    if ((*itProngs)->filtertaglist()->filterTagExists(
          AnaFilterTags::PrimaryMuon()))
      continue;
    ...
    StatusCode sc_dEdX =
      m_particleMaker->makeParticles(*itProngs, particleHypotheses, toolsToUse);
    if (!sc_dEdX) {
      debug() << "Could not make particles in prong!" << endmsg;
    }

    // make an endpoint vertex blob for the hadron prong - currently this
    // depends on the prong order!!!
    debug() << "Make endpoint blob" << endmsg;
    SmartRef<Minerva::Vertex> endpoint_vtx;
    m_objectAssociator->getVertex_fromTrackBack(
      endpoint_vtx, (*itProngs)->minervaTracks().back());
    if (!endpoint_vtx) {
    ...
    if (foundMichel) {
      ...
      (*itProngs)->setIntData("endMichel_category",
                              michelProng.getIntData("category"));
      ...
      (*itProngs)->setIntData("endMichel_ndigits",
                              michelProng.getIntData("ndigits"));
      (*itProngs)->setDoubleData("endMichel_energy",
                                 michelProng.getDoubleData("energy"));
      // (*itProngs)->setDoubleData("endMichel_energy_uncorrected",michelProng.getDoubleData("energy_uncorrected"));
      (*itProngs)->setDoubleData("endMichel_slice_energy",
                                 michelProng.getDoubleData("slice_energy"));
...
void
MasterAnaDev::tagMichelElectrons(Minerva::PhysicsEvent* event) const
{
  //-- Calculate and Store Michel electron data (Matching will occur in later
  //macros)
  ...
  if (!int_vtx)
    error() << "No Interaction Vertex Found!" << endmsg;
  else {
  ...
  for (unsigned int i = 0; i < pionProngs.size(); i++) {
    Gaudi::XYZPoint endpoint =
      (pionProngs[i]->minervaTracks().back())->lastState().position();
  ...
  for (unsigned int v = 0; v < vertices.size(); ++v) {
    ...
    if (match) {
      ...
      matched_michel_avg_idx[v] =
        m_improvedmichelTool->GetMatchedAvgNofMichelIndex();
      matched_michel_end_idx[v] =
        m_improvedmichelTool->GetMatchedEndMichelIndex();
      matched_michel_ov_idx[v] =
        m_improvedmichelTool->GetMatchedOneViewMichelIndex();

      // new 2018.11.21
      // time slice of vertex v's michel.
      SmartRefVector<Minerva::IDCluster> michel_clusters =
        m_improvedmichelTool->GetMichelClusters(matched_michel_idx[v]);
      ...
      matched_michel_cal_energy[v] =
        m_improvedmichelTool->GetMichelCalorimetricEnergy();
      matched_michel_vis_energy[v] =
        m_improvedmichelTool->GetMichelVisibleEnergy();
      ...
      matched_michel_fit_residual[v] =
        m_improvedmichelTool->GetMichelFitResidual();

      matched_michel_avg_dist[v] =
        m_improvedmichelTool->GetMatchedAvgNofMichelDist();
      matched_michel_end_dist[v] =
        m_improvedmichelTool->GetMatchedEndMichelDist();
      matched_michel_ov_dist[v] =
        m_improvedmichelTool->GetMatchedOneViewMichelDist();

      // new 2018.11.21
      // get the module corresponding to vertex v's michel's avg position
      Gaudi::XYZPoint hitXYZMAvg(matched_michel_position_x[v],
                                 matched_michel_position_y[v],
                                 matched_michel_position_z[v]);
      const Minerva::DePlane* trkrHitPlaneAvg =
        m_InnerDetector->getDePlane(hitXYZMAvg);
      if (!trkrHitPlaneAvg) // Cases where we're not in a tracker module (and
                            // I'm too lazy to find the regular module
      {
        matched_michel_avg_module[v] = -6;
      } else {
        const Minerva::PlaneID& trkrHitPlaneIdAvg =
          trkrHitPlaneAvg->getPlaneID();
      ...
      // new 2018.11.21
      // get the first module corresponding to vertex v's michel's endpoint
      // position
      if (matched_michel_end_Z1[v] > 0) {
        Gaudi::XYZPoint hitXYZMEnd1(matched_michel_end_X1[v],
                                    matched_michel_end_Y1[v],
                                    matched_michel_end_Z1[v]);
        const Minerva::DePlane* trkrHitPlaneEnd1 =
          m_InnerDetector->getDePlane(hitXYZMEnd1);
        if (!trkrHitPlaneEnd1) // Cases where we're not in a tracker module (and
                               // I'm too lazy to find the regular module
        {
          matched_michel_end_module1[v] = -6;
        } else {
          const Minerva::PlaneID& trkrHitPlaneIdEnd1 =
            trkrHitPlaneEnd1->getPlaneID();
      ...
      // new 2018.11.21
      // get the second module corresponding to vertex v's michel's endpoint
      // position

      if (matched_michel_end_Z2[v] > 0) {
        Gaudi::XYZPoint hitXYZMEnd2(matched_michel_end_X2[v],
                                    matched_michel_end_Y2[v],
                                    matched_michel_end_Z2[v]);
        const Minerva::DePlane* trkrHitPlaneEnd2 =
          m_InnerDetector->getDePlane(hitXYZMEnd2);
        if (!trkrHitPlaneEnd2) // Cases where we're not in a tracker module (and
                               // I'm too lazy to find the regular module
        {
          matched_michel_end_module2[v] = -6;
        } else {
          const Minerva::PlaneID& trkrHitPlaneIdEnd2 =
            trkrHitPlaneEnd2->getPlaneID();
  ...
  if (haveNeutrinoMC()) {
    // get list of TG4Trajectories in event in order to track lineage of Michel
    // electron
  ...
  for (int m = 0; m < michel_count; m++) {
    ...
    has_michel_cal_energy[m] =
      m_improvedmichelTool->GetMichelCalorimetricEnergy(m);
    ...
    Gaudi::XYZPoint hitXYZ(has_michel_position_x[m],
                           has_michel_position_y[m],
                           has_michel_position_z[m]);
    const Minerva::DePlane* hasTrkrHitPlaneAvg =
      m_InnerDetector->getDePlane(hitXYZ);
    if (!hasTrkrHitPlaneAvg) // Cases where we're not in a tracker module
    {
      has_michel_avg_module[m] = -6;
    } else {
      const Minerva::PlaneID& hasTrkrHitPlaneIdAvg =
        hasTrkrHitPlaneAvg->getPlaneID();
    ...
    if (fitPass) {
      has_michel_fit_residual[m] =
        m_improvedmichelTool->GetMichelFitResidual(m);
      ...
      // new 2018.11.21. module numbers corresponding to positions
      Gaudi::XYZPoint hitXYZEnd1(
        has_michel_end_X1[m], has_michel_end_Y1[m], has_michel_end_Z1[m]);
      const Minerva::DePlane* hasTrkrHitPlaneEnd1 =
        m_InnerDetector->getDePlane(hitXYZEnd1);
      if (!hasTrkrHitPlaneEnd1) // Cases where we're not in a tracker module
      {
        has_michel_end_module1[m] = -6;
      } else {
        const Minerva::PlaneID& hasTrkrHitPlaneIdEnd1 =
          hasTrkrHitPlaneEnd1->getPlaneID();
        has_michel_end_module1[m] = hasTrkrHitPlaneIdEnd1.module();
      }

      Gaudi::XYZPoint hitXYZEnd2(
        has_michel_end_X2[m], has_michel_end_Y2[m], has_michel_end_Z2[m]);
      const Minerva::DePlane* hasTrkrHitPlaneEnd2 =
        m_InnerDetector->getDePlane(hitXYZEnd2);
      if (!hasTrkrHitPlaneEnd2) // Cases where we're not in a tracker module
      {
        has_michel_end_module2[m] = -6;
      } else {
        const Minerva::PlaneID& hasTrkrHitPlaneIdEnd2 =
          hasTrkrHitPlaneEnd2->getPlaneID();
    ...
    if (haveNeutrinoMC()) {
      ...
      if (TruthMatcher->getDataFraction(functional_michel_clusters) < 0.5) {
        // find TG4Trajectory that created the Michel leading the the Michel
        // track
        StatusCode parent_sc = TruthMatcher->getTG4Trajectory(
          functional_michel_clusters, MichelParent, fraction, other_energy);
        if (!parent_sc)
          continue;
        ...
        for (Minerva::TG4Trajectories::iterator it = trajectories->begin();
          ...
          // if(traject->GetInitialPosition() ==
          // MichelParent->GetFinalPosition()) {
          if (traject->GetParentId() == MichelParent->GetTrackId()) {
            if (abs(traject->GetPDGCode()) == 14)
              continue;
            trueMichel = traject;
        ...
        MichelPrimaryParent =
          TruthMatcher->getPrimaryTG4Trajectory(MichelParent, false);
        has_michel_primary_particle_trackID[m] =
          MichelPrimaryParent->GetTrackId();
        has_michel_primary_particle_PDG[m] = MichelPrimaryParent->GetPDGCode();
        has_michel_primary_particle_p[m] =
          MichelPrimaryParent->GetInitialMomentum().P();
        has_michel_primary_particle_px[m] =
          MichelPrimaryParent->GetInitialMomentum().px();
        has_michel_primary_particle_py[m] =
          MichelPrimaryParent->GetInitialMomentum().py();
        has_michel_primary_particle_pz[m] =
          MichelPrimaryParent->GetInitialMomentum().pz();
        has_michel_primary_particle_E[m] =
          MichelPrimaryParent->GetInitialMomentum().E();
      }
    } // end haveNeutrinoMC
  }   // end michel loop
  ...
  event->setContainerDoubleData("has_michel_fit_residual",
                                has_michel_fit_residual);
  ...
  event->setContainerDoubleData("matched_michel_fit_residual",
                                matched_michel_fit_residual);
  event->setContainerIntData("matched_michel_end_module1",
                             matched_michel_end_module1);
  event->setContainerIntData("matched_michel_end_module2",
                             matched_michel_end_module2);

  if (haveNeutrinoMC()) {
    event->setContainerIntData("has_michel_primary_particle_trackID",
    ...
    event->setContainerIntData("has_michel_primary_particle_PDG",
                               has_michel_primary_particle_PDG);
    event->setContainerDoubleData("has_michel_primary_particle_p",
                                  has_michel_primary_particle_p);
    event->setContainerDoubleData("has_michel_primary_particle_px",
                                  has_michel_primary_particle_px);
    event->setContainerDoubleData("has_michel_primary_particle_py",
                                  has_michel_primary_particle_py);
    event->setContainerDoubleData("has_michel_primary_particle_pz",
                                  has_michel_primary_particle_pz);
    event->setContainerDoubleData("has_michel_primary_particle_E",
                                  has_michel_primary_particle_E);
  }
}

//=======================================================================================================
// Find proton prongs, pick the most energetic one and save the others in a
// vector of prongs -protonProng and protonPart are the Minerva::Prong and
// Minerva::Particle of the
//  most energetic proton found
// secondaryProtonProngs is a vector of all extra proton prongs found besides
// the most energetic one.
//=======================================================================================================
bool
MasterAnaDev::getProtonProngs(Minerva::ProngVect prongs,
                              Minerva::ProngVect& secondaryProtonProngs,
                              SmartRef<Minerva::Prong>& protonProng,
                              SmartRef<Minerva::Particle>& protonPart) const
{
  debug() << "Enter MasterAnaDev::getProtonProngs" << endmsg;

  bool foundProton = false;
  double max_proton_E = -1.0;
  std::map<SmartRef<Minerva::Prong>, SmartRef<Minerva::Particle>>
    protonProngPartMap;
  ...
  for (prong = prongs.begin(); prong != prongs.end(); prong++) {
    ...
    verbose() << "   prong bit-field signature = "
              << (*prong)->typeBitsToString() << endmsg;
    ...
    if (tracks.empty()) {
      warning() << "  This prong contains an empty vector of tracks, skipping!"
                << endmsg;
      continue;
    ...
    if (m_minCoordSysTool->inFiducial(endpoint.x(),
                                      endpoint.y(),
                                      endpoint.z(),
                                      m_ProtonApothem,
                                      m_ProtonZLow,
      ...
      debug() << "  Considering a prong with " << partHypVec.size()
              << " particle hypotheses and " << tracks.size() << " tracks."
              << endmsg;

      //-- loop over all particle hypotheses
      for (part = partHypVec.begin(); part != partHypVec.end(); part++) {

        debug() << "   Found a " << (*part)->idcode()
                << " with signature: " << (*part)->methodSignature()
                << " and score: " << (*part)->score() << endmsg;

        //---------------------------------------------------------------------------------------
        //! for now we will only accept protons whose mass was not change during
        //! the fit. there has not been sufficient studies for us to choose the
        //! proton-pion fit over the proton-proton fit based on the score alone.
        //!--------------------------------------------------------------------------------------
        ...
        if ((*part)->idcode() == Minerva::Particle::Proton &&
          ...
          if ((*part)->methodSignature().find("dEdX") != std::string::npos) {
            if (track->chi2PerDoF() < m_maxProtonChi2 &&
                (*part)->score() > m_minProtonScore) {
              // prongHasProtonPartandPassProtonCuts = true;
              passProtonCut = true;
            } else {
              debug()
                << "  Chi2/DoF and/or score is not consistent with a proton"
                << endmsg;
            }
          } else {
            debug() << "  Method signature is not consistent with a proton"
                    << endmsg;
          }
          ...
          if (passProtonCut) {
            prongPart = (*part);
            debug()
              << "   Method signature and/or score is consistent with a proton!"
              << endmsg;
  ...
  for (std::map<SmartRef<Minerva::Prong>, SmartRef<Minerva::Particle>>::iterator
         it = protonProngPartMap.begin();
       it != protonProngPartMap.end();
       ++it) {
...
//========================================
bool
MasterAnaDev::ImprovedtagMichels(Minerva::PhysicsEvent* event,
                                 Minerva::GenMinInteraction* truth) const
{
  ...
  const SmartRefVector<Minerva::Vertex> vertices =
    event->select<Minerva::Vertex>();
  debug() << "Looking for Improved michels in " << vertices.size() - 1
          << " vertex points" << endmsg;
  // event->setIntData("has_n_vertex_points", vertices.size()-1);
  warning() << "Loking for Improved michels in " << vertices.size() - 1
            << " vertex points" << endmsg;
  ...
  for (SmartRefVector<Minerva::Vertex>::const_iterator itVtx = vertices.begin();
       itVtx != vertices.end();
       ++itVtx) {

    SmartRef<Minerva::Vertex> myvtx = (*itVtx);
    //--no need to look at the endpoint of the muon track
    if ((*itVtx)->getIncomingTrack() == muonProng->minervaTracks()[0])
      continue;
    if (!m_masterAnaDevRecoUtils->vertexTracksInTime(myvtx, muonTime))
      continue; // if any tracks associated with the vertex are outside tracking
                // time window don't use it

    verbose()
      << "This vertex is not at the end point of a Muon. Vertex type is = "
      << (*itVtx)->type() << endmsg;
    ...
    bool match =
      m_improvedmichelTool->Match(x, y, z, max_distance, max_distance);
    if (!match) {
      debug() << "'Did NOT Found a michel electron' Said the ImprovedMichelTool"
              << endmsg;
      improved_vtx_type = 99;
      continue;
    }
    debug() << "'Found a michel electron' Said the ImprovedMichelTool"
            << endmsg;
    ...
    std::vector<const Minerva::IDCluster*> idclusters;
    const SmartRefVector<Minerva::IDCluster> clustersfromcandidate =
      m_improvedmichelTool->GetMichelClusters();
    idclusters.insert(idclusters.begin(),
                      clustersfromcandidate.begin(),
                      clustersfromcandidate.end());
    SmartRefVector<Minerva::IDDigit> alliddigits;
    // Loop over id clusters, and extract hit information
    for (SmartRefVector<Minerva::IDCluster>::const_iterator itClus =
           clustersfromcandidate.begin();
         itClus != clustersfromcandidate.end();
         ++itClus) {
      ...
      for (SmartRefVector<Minerva::IDDigit>::const_iterator itDigit =
             iddigits.begin();
           itDigit != iddigits.end();
           ++itDigit) {
        ...
        improved_michel_hit_time_diff_cluster.push_back((*itDigit)->calTime() -
                                                        clus_t);
      } // end loop digits
    }   // end loop clusters
    ...
    michel_dist =
      sqrt(pow(michel_x - x, 2) + pow(michel_y - y, 2) + pow(michel_z - z, 2));
  ...
  event->setContainerIntData("improved_michel_vertex_type",
                             improved_michel_vertex_type);
  event->setContainerIntData("improved_michel_in_vertex_point",
  ...
  event->setContainerDoubleData("improved_michel_tdiff_vec",
                                improved_michel_tdiff_vec);
  ...
  event->setContainerDoubleData("improved_michel_dist_vec",
                                improved_michel_dist_vec);
  event->setContainerDoubleData("improved_michel_evis_vec",
                                improved_michel_evis_vec);
  event->setContainerDoubleData("improved_michel_ecalo_vec",
                                improved_michel_ecalo_vec);
  event->setContainerDoubleData("improved_michel_hit_charges",
                                improved_michel_hit_charges);
  event->setContainerDoubleData("improved_michel_hit_times",
                                improved_michel_hit_times);
  event->setContainerDoubleData("improved_michel_hit_time_diff_vtx",
                                improved_michel_hit_time_diff_vtx);
  event->setContainerDoubleData("improved_michel_hit_time_diff_cluster",
                                improved_michel_hit_time_diff_cluster);
  event->setContainerDoubleData("improved_michel_matched_energy_fraction",
                                improved_michel_matched_energy_fraction);
  event->setContainerDoubleData("improved_michel_data_energy_fraction",
                                improved_michel_data_energy_fraction);
  event->setContainerIntData("improved_michel_view_vec",
                             improved_michel_view_vec);
  event->setContainerIntData(
    "improved_michel_match_vec",
    improved_michel_match_vec); // whether this track is Michel tagged
  event->setContainerIntData("improved_michel_matched_pdg",
                             improved_michel_matched_pdg);
  event->setContainerIntData("improved_michel_matched_primary_pdg",
                             improved_michel_matched_primary_pdg);
  event->setContainerIntData("improved_michel_ndigits",
                             improved_michel_ndigits);
  verbose() << "Finished setting Improved michel variables" << endmsg;

  if (improved_michel_view_vec.size() > 0)
    foundImprovedMichel = true;
...
//=====================================
bool
MasterAnaDev::hasTruthMichelElectron(Minerva::PhysicsEvent* event) const
{

  debug() << "Entering MasterAnaDev::hasTruthMichelElectron(  ) ....."
          << endmsg;
  ...
  Minerva::TG4Trajectories* alltrajects =
    get<Minerva::TG4Trajectories>("MC/TG4Trajectories");
  for (Minerva::TG4Trajectories::iterator it = alltrajects->begin();
       it != alltrajects->end();
       ++it) {
    Minerva::TG4Trajectory* traject = *it;
    ...
    if (traject->GetPDGCode() == 211) {
      pion_plus_ids_momentum[traject->GetTrackId()] =
        (traject->GetInitialMomentum()).P();
    }
    if (traject->GetPDGCode() == -211) {
      pion_minus_ids_momentum[traject->GetTrackId()] =
        (traject->GetInitialMomentum()).P();
    }
    ...
    if (traject->GetProcessName() == "Decay" && traject->GetPDGCode() == -11) {
      positron_parent_pos[traject->GetParentId()] =
        traject->GetInitialPosition();
    }

    if (traject->GetProcessName() == "muMinusCaptureAtRest" &&
        traject->GetPDGCode() == 11) {
      electron_parent_pos[traject->GetParentId()] =
        traject->GetInitialPosition();
    }
  ...
  //! loop over electrons
  for (std::map<int, Gaudi::LorentzVector>::iterator electron =
         electron_parent_pos.begin();
       electron != electron_parent_pos.end();
       ++electron) {
    ...
    if (muon_minus_id_parent.count(electron->first)) {
      ...
      // if( FiducialPointTool->isFiducial( p, m_analyzeApothem,
      // m_michel_upstreamZ, m_michel_downstreamZ ) ){
      event->setIntData("truth_has_michel_electron", 1);
      hasMichel = true;
      // check if muon parent is a pion-
      if (pion_minus_ids_momentum.count(
            muon_minus_id_parent[electron->first])) {
        pion_minus_momentum.push_back(
          pion_minus_ids_momentum[muon_minus_id_parent[electron->first]]);
        //}
      }
    }
  }
  ...
  //! loop over positrons
  for (std::map<int, Gaudi::LorentzVector>::iterator positron =
         positron_parent_pos.begin();
       positron != positron_parent_pos.end();
       ++positron) {
    ...
    if (muon_plus_id_parent.count(positron->first)) {
      ...
      if (pion_plus_ids_momentum.count(muon_plus_id_parent[positron->first])) {
        pion_plus_momentum.push_back(
          pion_plus_ids_momentum[muon_plus_id_parent[positron->first]]);
        //}
      }
    }
  }

  event->setContainerDoubleData("truth_has_michel_from_pion_plus_momentum",
                                pion_plus_momentum);
  event->setContainerDoubleData("truth_has_michel_from_pion_minus_momentum",
                                pion_minus_momentum);
...
void
MasterAnaDev::TruthFittedMichel(Minerva::PhysicsEvent* event,
                                Minerva::GenMinInteraction* truth) const
{
  ...
  if (truth) {
    ...
    for (Minerva::TG4Trajectories::iterator it = alltrajects->begin();
         ...
         ++it) {
      Minerva::TG4Trajectory* traject = *it;
      const Minerva::TG4Trajectory* parenttraject =
        TruthMatcher->getPrimaryTG4Trajectory(traject, false);

      //! look for charged pions
      if (traject->GetPDGCode() == 211) {
        std::cout << "Looking at charged pi+ " << std::endl;
        std::cout << "GetTRACKId " << traject->GetTrackId() << std::endl;
        FittedMichel_all_true_piontrajectory_pdgvec.push_back(
          traject->GetPDGCode());
        FittedMichel_all_true_piontrajectory_ParentIDvec.push_back(
          traject->GetParentId());
        FittedMichel_all_true_piontrajectory_ParentPDGvec.push_back(
          parenttraject->GetPDGCode());
        FittedMichel_all_true_piontrajectory_energyvec.push_back(
          traject->GetInitialMomentum().E());
        FittedMichel_all_true_piontrajectory_momentumvec.push_back(
          traject->GetInitialMomentum().P());
        FittedMichel_all_true_piontrajectory_momentumxvec.push_back(
          traject->GetInitialMomentum().px());
        FittedMichel_all_true_piontrajectory_momentumyvec.push_back(
          traject->GetInitialMomentum().py());
        FittedMichel_all_true_piontrajectory_momentumzvec.push_back(
          traject->GetInitialMomentum().pz());
        FittedMichel_all_true_piontrajectory_finalxvec.push_back(
          traject->GetFinalPosition().x());
        FittedMichel_all_true_piontrajectory_finalyvec.push_back(
          traject->GetFinalPosition().y());
        FittedMichel_all_true_piontrajectory_finalzvec.push_back(
          traject->GetFinalPosition().z());
        FittedMichel_all_true_piontrajectory_initialxvec.push_back(
          traject->GetInitialPosition().x());
        FittedMichel_all_true_piontrajectory_initialyvec.push_back(
          traject->GetInitialPosition().y());
        FittedMichel_all_true_piontrajectory_initialzvec.push_back(
          traject->GetInitialPosition().z());
        FittedMichel_all_true_piontrajectory_trackIDvec.push_back(
          traject->GetTrackId());
      ...
      //! look for muons from decays
      else if (traject->GetProcessName() == "Decay" &&
               traject->GetPDGCode() == -13) {
        std::cout << "Looking at muon+ " << std::endl;
        std::cout << "GetParentId " << traject->GetParentId() << std::endl;
        std::cout << "GetTRACKId " << traject->GetTrackId() << std::endl;

        FittedMichel_all_true_muontrajectory_pdgvec.push_back(
          traject->GetPDGCode());
        FittedMichel_all_true_muontrajectory_ParentIDvec.push_back(
          traject->GetParentId());
        FittedMichel_all_true_muontrajectory_ParentPDGvec.push_back(
          parenttraject->GetPDGCode());
        FittedMichel_all_true_muontrajectory_energyvec.push_back(
          traject->GetInitialMomentum().E());
        FittedMichel_all_true_muontrajectory_momentumvec.push_back(
          traject->GetInitialMomentum().P());
        FittedMichel_all_true_muontrajectory_momentumxvec.push_back(
          traject->GetInitialMomentum().px());
        FittedMichel_all_true_muontrajectory_momentumyvec.push_back(
          traject->GetInitialMomentum().py());
        FittedMichel_all_true_muontrajectory_momentumzvec.push_back(
          traject->GetInitialMomentum().pz());
        FittedMichel_all_true_muontrajectory_finalxvec.push_back(
          traject->GetFinalPosition().x());
        FittedMichel_all_true_muontrajectory_finalyvec.push_back(
          traject->GetFinalPosition().y());
        FittedMichel_all_true_muontrajectory_finalzvec.push_back(
          traject->GetFinalPosition().z());
        FittedMichel_all_true_muontrajectory_trackIDvec.push_back(
          traject->GetTrackId());
      }
      ...
      //! look for electrons from decays
               ...
               abs(traject->GetPDGCode()) == 11) {
        std::cout << "Looking at positron + " << std::endl;
        std::cout << "GetParentId " << traject->GetParentId() << std::endl;
        std::cout << "GetTRACKId " << traject->GetTrackId() << std::endl;

        FittedMichel_all_true_micheltrajectory_pdgvec.push_back(
          traject->GetPDGCode());
        FittedMichel_all_true_micheltrajectory_ParentIDvec.push_back(
          traject->GetParentId());
        FittedMichel_all_true_micheltrajectory_ParentPDGvec.push_back(
          parenttraject->GetPDGCode());
        FittedMichel_all_true_micheltrajectory_energyvec.push_back(
          traject->GetInitialMomentum().E());
        FittedMichel_all_true_micheltrajectory_momentumvec.push_back(
          traject->GetInitialMomentum().P());
        FittedMichel_all_true_micheltrajectory_momentumxvec.push_back(
          traject->GetInitialMomentum().px());
        FittedMichel_all_true_micheltrajectory_momentumyvec.push_back(
          traject->GetInitialMomentum().py());
        FittedMichel_all_true_micheltrajectory_momentumzvec.push_back(
          traject->GetInitialMomentum().pz());
        FittedMichel_all_true_micheltrajectory_finalxvec.push_back(
          traject->GetFinalPosition().x());
        FittedMichel_all_true_micheltrajectory_finalyvec.push_back(
          traject->GetFinalPosition().y());
        FittedMichel_all_true_micheltrajectory_finalzvec.push_back(
          traject->GetFinalPosition().z());
        FittedMichel_all_true_micheltrajectory_initialxvec.push_back(
          traject->GetInitialPosition().x());
        FittedMichel_all_true_micheltrajectory_initialyvec.push_back(
          traject->GetInitialPosition().y());
        FittedMichel_all_true_micheltrajectory_initialzvec.push_back(
          traject->GetInitialPosition().z());
        FittedMichel_all_true_micheltrajectory_trackIDvec.push_back(
          traject->GetTrackId());
      }

      else if (traject->GetProcessName() == "muMinusCaptureAtRest" &&
               traject->GetPDGCode() == 11) {
        ...
        FittedMichel_all_true_othertrajectory_pdgvec.push_back(
          traject->GetPDGCode());
        FittedMichel_all_true_othertrajectory_ParentIDvec.push_back(
          traject->GetParentId());
        FittedMichel_all_true_othertrajectory_ParentPDGvec.push_back(
          parenttraject->GetPDGCode());
        FittedMichel_all_true_othertrajectory_energyvec.push_back(
          traject->GetInitialMomentum().E());
        FittedMichel_all_true_othertrajectory_momentumvec.push_back(
          traject->GetInitialMomentum().P());
        FittedMichel_all_true_othertrajectory_momentumxvec.push_back(
          traject->GetInitialMomentum().px());
        FittedMichel_all_true_othertrajectory_momentumyvec.push_back(
          traject->GetInitialMomentum().py());
        FittedMichel_all_true_othertrajectory_momentumzvec.push_back(
          traject->GetInitialMomentum().pz());
        FittedMichel_all_true_othertrajectory_finalxvec.push_back(
          traject->GetFinalPosition().x());
        FittedMichel_all_true_othertrajectory_finalyvec.push_back(
          traject->GetFinalPosition().y());
        FittedMichel_all_true_othertrajectory_finalzvec.push_back(
          traject->GetFinalPosition().z());
        FittedMichel_all_true_othertrajectory_trackIDvec.push_back(
          traject->GetTrackId());
      }
    } // end of TG4Trajectories loop
  }   // end of if Truth
  ...
  if (nmichels > 0) {
    for (int imichel = 0; imichel < nmichels; imichel++) {
      double michelenergy =
        m_improvedmichelTool->GetMichelCalorimetricEnergy(imichel);
      int fitpass = m_improvedmichelTool->GetMichelFitPass(imichel);
      std::vector<const Minerva::IDCluster*> idclusters;
      std::vector<const Minerva::IDCluster*> michelclusters;
      SmartRefVector<Minerva::IDCluster> clustersfromcandidate =
        m_improvedmichelTool->GetMichelClusters(imichel);
      idclusters.insert(idclusters.begin(),
                        clustersfromcandidate.begin(),
                        clustersfromcandidate.end());
      ...
      for (SmartRefVector<Minerva::IDCluster>::iterator itClus =
             clustersfromcandidate.begin();
           itClus != clustersfromcandidate.end();
           ++itClus) {
      ...
      //      int michel_view  =
      //      m_improvedmichelTool->GetMichelViewCode(imichel);
      ...
      //      std::cout << "Michel candidate has " << michelclusters.size() << "
      //      clusters." << std::endl;
      double fraction;
      if (truth) {
        ...
        if (fraction > 0.5) {
          ...
          FittedMichel_michel_datafraction.push_back(
            double(TruthMatcher->getDataFraction(michelclusters)));
          ...
          StatusCode parent_sc = TruthMatcher->getTG4Trajectory(
            ...
            candidate_traj,
            datafrac,
            other_energy); // find   TG4Trajectory that created the Michel
                           // leading the the Michel track
          ...
          CandidateParent =
            TruthMatcher->getPrimaryTG4Trajectory(candidate_traj, false);

          FittedMichel_reco_micheltrajectory_pdgvec.push_back(
            candidate_traj->GetPDGCode());
          FittedMichel_reco_micheltrajectory_ParentIDvec.push_back(
            candidate_traj->GetParentId());
          FittedMichel_reco_micheltrajectory_ParentPDGvec.push_back(
            CandidateParent->GetPDGCode());
          FittedMichel_reco_micheltrajectory_energyvec.push_back(
            candidate_traj->GetInitialMomentum().E());
          FittedMichel_reco_micheltrajectory_momentumvec.push_back(
            candidate_traj->GetInitialMomentum().P());
          FittedMichel_reco_micheltrajectory_momentumxvec.push_back(
            candidate_traj->GetInitialMomentum().px());
          FittedMichel_reco_micheltrajectory_momentumyvec.push_back(
            candidate_traj->GetInitialMomentum().py());
          FittedMichel_reco_micheltrajectory_momentumzvec.push_back(
            candidate_traj->GetInitialMomentum().pz());
          FittedMichel_reco_micheltrajectory_finalxvec.push_back(
            candidate_traj->GetFinalPosition().x());
          FittedMichel_reco_micheltrajectory_finalyvec.push_back(
            candidate_traj->GetFinalPosition().y());
          FittedMichel_reco_micheltrajectory_finalzvec.push_back(
            candidate_traj->GetFinalPosition().z());
          FittedMichel_reco_micheltrajectory_initialxvec.push_back(
            candidate_traj->GetInitialPosition().x());
          FittedMichel_reco_micheltrajectory_initialyvec.push_back(
            candidate_traj->GetInitialPosition().y());
          FittedMichel_reco_micheltrajectory_initialzvec.push_back(
            candidate_traj->GetInitialPosition().z());
          FittedMichel_reco_micheltrajectory_trackIDvec.push_back(
            candidate_traj->GetTrackId());
          FittedMichel_reco_true_michel_indexvec.push_back(imichel);

          FittedMichel_true_primaryparent_pdgvec.push_back(
            CandidateParent->GetPDGCode());
          FittedMichel_true_primaryparent_energyvec.push_back(
            CandidateParent->GetInitialMomentum().E());
          FittedMichel_true_primaryparent_momentumvec.push_back(
            CandidateParent->GetInitialMomentum().P());
          FittedMichel_true_primaryparent_momentumxvec.push_back(
            CandidateParent->GetInitialMomentum().px());
          FittedMichel_true_primaryparent_momentumyvec.push_back(
            CandidateParent->GetInitialMomentum().py());
          FittedMichel_true_primaryparent_momentumzvec.push_back(
            CandidateParent->GetInitialMomentum().pz());
          FittedMichel_true_primaryparent_finalxvec.push_back(
            CandidateParent->GetFinalPosition().x());
          FittedMichel_true_primaryparent_finalyvec.push_back(
            CandidateParent->GetFinalPosition().y());
          FittedMichel_true_primaryparent_finalzvec.push_back(
            CandidateParent->GetFinalPosition().z());
          FittedMichel_true_primaryparent_trackIDvec.push_back(
            CandidateParent->GetTrackId());
  ...
  event->setContainerIntData("FittedMichel_all_piontrajectory_trackID",
                             FittedMichel_all_true_piontrajectory_trackIDvec);
  event->setContainerIntData("FittedMichel_all_piontrajectory_ParentID",
                             FittedMichel_all_true_piontrajectory_ParentIDvec);
  event->setContainerIntData("FittedMichel_all_piontrajectory_pdg",
                             FittedMichel_all_true_piontrajectory_pdgvec);
  event->setContainerIntData("FittedMichel_all_piontrajectory_ParentPDG",
                             FittedMichel_all_true_piontrajectory_ParentPDGvec);
  event->setContainerDoubleData("FittedMichel_all_piontrajectory_energy",
                                FittedMichel_all_true_piontrajectory_energyvec);
  event->setContainerDoubleData(
    "FittedMichel_all_piontrajectory_momentum",
    FittedMichel_all_true_piontrajectory_momentumvec);
  event->setContainerDoubleData(
    "FittedMichel_all_piontrajectory_momentumx",
    FittedMichel_all_true_piontrajectory_momentumxvec);
  event->setContainerDoubleData(
    "FittedMichel_all_piontrajectory_momentumy",
    FittedMichel_all_true_piontrajectory_momentumyvec);
  event->setContainerDoubleData(
    "FittedMichel_all_piontrajectory_momentumz",
    FittedMichel_all_true_piontrajectory_momentumzvec);
  event->setContainerDoubleData("FittedMichel_all_piontrajectory_finalx",
                                FittedMichel_all_true_piontrajectory_finalxvec);
  event->setContainerDoubleData("FittedMichel_all_piontrajectory_finaly",
                                FittedMichel_all_true_piontrajectory_finalyvec);
  event->setContainerDoubleData("FittedMichel_all_piontrajectory_finalz",
                                FittedMichel_all_true_piontrajectory_finalzvec);
  event->setContainerDoubleData(
    "FittedMichel_all_piontrajectory_initialx",
    FittedMichel_all_true_piontrajectory_initialxvec);
  event->setContainerDoubleData(
    "FittedMichel_all_piontrajectory_initialy",
    FittedMichel_all_true_piontrajectory_initialyvec);
  event->setContainerDoubleData(
    "FittedMichel_all_piontrajectory_initialz",
    FittedMichel_all_true_piontrajectory_initialzvec);

  event->setContainerIntData("FittedMichel_all_muontrajectory_trackID",
                             FittedMichel_all_true_muontrajectory_trackIDvec);
  event->setContainerIntData("FittedMichel_all_muontrajectory_ParentID",
                             FittedMichel_all_true_muontrajectory_ParentIDvec);
  event->setContainerIntData("FittedMichel_all_muontrajectory_pdg",
                             FittedMichel_all_true_muontrajectory_pdgvec);
  event->setContainerIntData("FittedMichel_all_muontrajectory_ParentPDG",
                             FittedMichel_all_true_muontrajectory_ParentPDGvec);
  event->setContainerDoubleData("FittedMichel_all_muontrajectory_energy",
                                FittedMichel_all_true_muontrajectory_energyvec);
  event->setContainerDoubleData(
    "FittedMichel_all_muontrajectory_momentum",
    FittedMichel_all_true_muontrajectory_momentumvec);
  event->setContainerDoubleData(
    "FittedMichel_all_muontrajectory_momentumx",
    FittedMichel_all_true_muontrajectory_momentumxvec);
  event->setContainerDoubleData(
    "FittedMichel_all_muontrajectory_momentumy",
    FittedMichel_all_true_muontrajectory_momentumyvec);
  event->setContainerDoubleData(
    "FittedMichel_all_muontrajectory_momentumz",
    FittedMichel_all_true_muontrajectory_momentumzvec);
  event->setContainerDoubleData("FittedMichel_all_muontrajectory_finalx",
                                FittedMichel_all_true_muontrajectory_finalxvec);
  event->setContainerDoubleData("FittedMichel_all_muontrajectory_finaly",
                                FittedMichel_all_true_muontrajectory_finalyvec);
  event->setContainerDoubleData("FittedMichel_all_muontrajectory_finalz",
                                FittedMichel_all_true_muontrajectory_finalzvec);

  event->setContainerIntData("FittedMichel_all_micheltrajectory_trackID",
                             FittedMichel_all_true_micheltrajectory_trackIDvec);
  event->setContainerIntData(
    "FittedMichel_all_micheltrajectory_ParentID",
    FittedMichel_all_true_micheltrajectory_ParentIDvec);
  event->setContainerIntData("FittedMichel_all_micheltrajectory_pdg",
                             FittedMichel_all_true_micheltrajectory_pdgvec);
  event->setContainerIntData(
    "FittedMichel_all_micheltrajectory_ParentPDG",
    FittedMichel_all_true_micheltrajectory_ParentPDGvec);
  event->setContainerDoubleData(
    "FittedMichel_all_micheltrajectory_energy",
    FittedMichel_all_true_micheltrajectory_energyvec);
  event->setContainerDoubleData(
    "FittedMichel_all_micheltrajectory_momentum",
    FittedMichel_all_true_micheltrajectory_momentumvec);
  event->setContainerDoubleData(
    "FittedMichel_all_micheltrajectory_momentumx",
    FittedMichel_all_true_micheltrajectory_momentumxvec);
  event->setContainerDoubleData(
    "FittedMichel_all_micheltrajectory_momentumy",
    FittedMichel_all_true_micheltrajectory_momentumyvec);
  event->setContainerDoubleData(
    "FittedMichel_all_micheltrajectory_momentumz",
    FittedMichel_all_true_micheltrajectory_momentumzvec);
  event->setContainerDoubleData(
    "FittedMichel_all_micheltrajectory_finalx",
    FittedMichel_all_true_micheltrajectory_finalxvec);
  event->setContainerDoubleData(
    "FittedMichel_all_micheltrajectory_finaly",
    FittedMichel_all_true_micheltrajectory_finalyvec);
  event->setContainerDoubleData(
    "FittedMichel_all_micheltrajectory_finalz",
    FittedMichel_all_true_micheltrajectory_finalzvec);

  event->setContainerIntData("FittedMichel_all_othertrajectory_trackID",
                             FittedMichel_all_true_othertrajectory_trackIDvec);
  event->setContainerIntData("FittedMichel_all_othertrajectory_ParentID",
                             FittedMichel_all_true_othertrajectory_ParentIDvec);
  event->setContainerIntData("FittedMichel_all_othertrajectory_pdg",
                             FittedMichel_all_true_othertrajectory_pdgvec);
  event->setContainerIntData(
    "FittedMichel_all_othertrajectory_ParentPDG",
    FittedMichel_all_true_othertrajectory_ParentPDGvec);
  event->setContainerDoubleData(
    "FittedMichel_all_othertrajectory_energy",
    FittedMichel_all_true_othertrajectory_energyvec);
  event->setContainerDoubleData(
    "FittedMichel_all_othertrajectory_momentum",
    FittedMichel_all_true_othertrajectory_momentumvec);
  event->setContainerDoubleData(
    "FittedMichel_all_othertrajectory_momentumx",
    FittedMichel_all_true_othertrajectory_momentumxvec);
  event->setContainerDoubleData(
    "FittedMichel_all_othertrajectory_momentumy",
    FittedMichel_all_true_othertrajectory_momentumyvec);
  event->setContainerDoubleData(
    "FittedMichel_all_othertrajectory_momentumz",
    FittedMichel_all_true_othertrajectory_momentumzvec);
  event->setContainerDoubleData(
    "FittedMichel_all_othertrajectory_finalx",
    FittedMichel_all_true_othertrajectory_finalxvec);
  event->setContainerDoubleData(
    "FittedMichel_all_othertrajectory_finaly",
    FittedMichel_all_true_othertrajectory_finalyvec);
  event->setContainerDoubleData(
    "FittedMichel_all_othertrajectory_finalz",
    FittedMichel_all_true_othertrajectory_finalzvec);

  event->setContainerIntData("FittedMichel_michel_fitPass",
                             FittedMichel_michel_fitpass);
  event->setContainerDoubleData("FittedMichel_michel_datafraction",
                                FittedMichel_michel_datafraction);

  event->setContainerDoubleData("FittedMichel_michel_time",
                                FittedMichel_michel_timevec);
  event->setContainerDoubleData("FittedMichel_michel_energy",
                                FittedMichel_michel_energyvec);
  event->setContainerIntData("FittedMichel_michel_isoverlay",
                             FittedMichel_michel_isoverlayvec);

  event->setContainerDoubleData("FittedMichel_michel_x1",
                                FittedMichel_michel_x1_vec);
  event->setContainerDoubleData("FittedMichel_michel_x2",
                                FittedMichel_michel_x2_vec);
  event->setContainerDoubleData("FittedMichel_michel_u1",
                                FittedMichel_michel_u1_vec);
  event->setContainerDoubleData("FittedMichel_michel_u2",
                                FittedMichel_michel_u2_vec);
  event->setContainerDoubleData("FittedMichel_michel_y1",
                                FittedMichel_michel_y1_vec);
  event->setContainerDoubleData("FittedMichel_michel_y2",
                                FittedMichel_michel_y2_vec);
  event->setContainerDoubleData("FittedMichel_michel_v1",
                                FittedMichel_michel_v1_vec);
  event->setContainerDoubleData("FittedMichel_michel_v2",
                                FittedMichel_michel_v2_vec);
  event->setContainerDoubleData("FittedMichel_michel_z1",
                                FittedMichel_michel_z1_vec);
  event->setContainerDoubleData("FittedMichel_michel_z2",
                                FittedMichel_michel_z2_vec);

  event->setContainerIntData("FittedMichel_true_primaryparent_pdg",
                             FittedMichel_true_primaryparent_pdgvec);
  event->setContainerIntData("FittedMichel_true_primaryparent_trackID",
                             FittedMichel_true_primaryparent_trackIDvec);
  event->setContainerDoubleData("FittedMichel_true_primaryparent_energy",
                                FittedMichel_true_primaryparent_energyvec);
  event->setContainerDoubleData("FittedMichel_true_primaryparent_momentum",
                                FittedMichel_true_primaryparent_momentumvec);
  event->setContainerDoubleData("FittedMichel_true_primaryparent_momentumx",
                                FittedMichel_true_primaryparent_momentumxvec);
  event->setContainerDoubleData("FittedMichel_true_primaryparent_momentumy",
                                FittedMichel_true_primaryparent_momentumyvec);
  event->setContainerDoubleData("FittedMichel_true_primaryparent_momentumz",
                                FittedMichel_true_primaryparent_momentumzvec);
  event->setContainerDoubleData("FittedMichel_true_primaryparent_finalx",
                                FittedMichel_true_primaryparent_finalxvec);
  event->setContainerDoubleData("FittedMichel_true_primaryparent_finaly",
                                FittedMichel_true_primaryparent_finalyvec);
  event->setContainerDoubleData("FittedMichel_true_primaryparent_finalz",
                                FittedMichel_true_primaryparent_finalzvec);

  event->setContainerIntData("FittedMichel_reco_micheltrajectory_trackID",
                             FittedMichel_reco_micheltrajectory_trackIDvec);
  event->setContainerIntData("FittedMichel_reco_micheltrajectory_pdg",
                             FittedMichel_reco_micheltrajectory_pdgvec);
  event->setContainerIntData("FittedMichel_reco_micheltrajectory_ParentID",
                             FittedMichel_reco_micheltrajectory_ParentIDvec);
  event->setContainerIntData("FittedMichel_reco_micheltrajectory_ParentPDG",
                             FittedMichel_reco_micheltrajectory_ParentPDGvec);
  event->setContainerDoubleData("FittedMichel_reco_micheltrajectory_energy",
                                FittedMichel_reco_micheltrajectory_energyvec);
  event->setContainerDoubleData(
    "FittedMichel_reco_micheltrajectory_momentumx",
    FittedMichel_reco_micheltrajectory_momentumxvec);
  event->setContainerDoubleData("FittedMichel_reco_micheltrajectory_momentum",
                                FittedMichel_reco_micheltrajectory_momentumvec);
  event->setContainerDoubleData(
    "FittedMichel_reco_micheltrajectory_momentumy",
    FittedMichel_reco_micheltrajectory_momentumyvec);
  event->setContainerDoubleData(
    "FittedMichel_reco_micheltrajectory_momentumz",
    FittedMichel_reco_micheltrajectory_momentumzvec);
  event->setContainerDoubleData("FittedMichel_reco_micheltrajectory_finalx",
                                FittedMichel_reco_micheltrajectory_finalxvec);
  event->setContainerDoubleData("FittedMichel_reco_micheltrajectory_finaly",
                                FittedMichel_reco_micheltrajectory_finalyvec);
  event->setContainerDoubleData("FittedMichel_reco_micheltrajectory_finalz",
                                FittedMichel_reco_micheltrajectory_finalzvec);
  event->setContainerDoubleData("FittedMichel_reco_micheltrajectory_initialx",
                                FittedMichel_reco_micheltrajectory_initialxvec);
  event->setContainerDoubleData("FittedMichel_reco_micheltrajectory_initialy",
                                FittedMichel_reco_micheltrajectory_initialyvec);
  event->setContainerDoubleData("FittedMichel_reco_micheltrajectory_initialz",
                                FittedMichel_reco_micheltrajectory_initialzvec);

  event->setContainerIntData("FittedMichel_reco_michel_index",
                             FittedMichel_reco_michel_indexvec);
  event->setContainerIntData("FittedMichel_reco_true_michel_index",
                             FittedMichel_reco_true_michel_indexvec);
}

bool
MasterAnaDev::findLongestMinervaTrack(
  Minerva::Prong* deProng,
  SmartRef<Minerva::Track>& longestTrackInsideProng) const
{

  verbose() << "Entering MasterAnaDev::findLongestMinervaTrack( ) ....."
            << endmsg;
  ...
  if (minervaTrkVec.size()) {
    ...
    for (Minerva::TrackVect::iterator itTrk = minervaTrkVec.begin();
         itTrk != minervaTrkVec.end();
         ++itTrk) {
    ...
    if (longestTrackInsideProng->nNodes())
      debug() << "For each prong " << longestTrackInsideProng->chi2PerDoF()
              << " Nodes: " << longestTrackInsideProng->nNodes() << endmsg;
  ...
  verbose() << "Exiting MasterAnaDev::findLongestMinervaTrack( ) ....."
            << endmsg;
  return true;
}
...
bool
MasterAnaDev::findLongestMinervaTrack(
  Minerva::ProngVect deProngVec,
  SmartRef<Minerva::Track>& longestMinervaTrack) const
{

  bool foundTrackInsideAProng = false;

  //------------------------------------------------------------------------------------------------------
  // This returns the longest track (whichever prong it belongs to) after
  // looping over all the Prongs
  //------------------------------------------------------------------------------------------------------
  unsigned int prong_nodes = 0;
  SmartRef<Minerva::Track> insideProngTrack;

  for (ProngVect::iterator itProngs = deProngVec.begin();
       itProngs != deProngVec.end();
       ++itProngs) {
    foundTrackInsideAProng =
      findLongestMinervaTrack(*itProngs, insideProngTrack);
  ...
  if (!longestMinervaTrack->nNodes()) {
    ...
    debug() << "After looping over all Prongs "
            << longestMinervaTrack->chi2PerDoF()
            << " Nodes: " << longestMinervaTrack->nNodes() << endmsg;
    return true;
  }
}
...
bool
MasterAnaDev::findLongestMinervaTrack(
  const Minerva::PhysicsEvent* physEvent,
  SmartRef<Minerva::Track>& longestMinervaTrack) const
{

  //----------------------------------------------------------------------------------------
  // This returns the longest track found (whichever Primary Prong it belongs
  // to)
  //----------------------------------------------------------------------------------------
  ProngVect primaryProngs = physEvent->primaryProngs();
  if (!primaryProngs.size()) {
...
//================================
string
MasterAnaDev::getRecoilChannel(const int muonCharge,
                               const int targetID,
                               const int targetZ) const
{
...
//==============================================================================
Minerva::IDClusterVect
MasterAnaDev::getHadronIDClusters(const Minerva::PhysicsEvent* event,
                                  SmartRef<Minerva::Prong> muonProng,
                                  SmartRef<Minerva::Prong> excludeProng) const
{
  ...
  Minerva::IDClusterVect allClustersID =
    event->select<Minerva::IDCluster>("Used:Unused", "!XTalkCandidate");
  ...
  for (; i_allClustersID != allClustersID.end(); ++i_allClustersID) {
    // skip if not in time window
    if ((*i_allClustersID)->time() < mintimeCut ||
        maxtimeCut < (*i_allClustersID)->time())
      continue;

    // skip if it's a muon cluster
    if (std::find(muonClustersID.begin(),
                  muonClustersID.end(),
                  (*i_allClustersID)) != muonClustersID.end())
      continue;

    nonmuon_clusters_id.push_back(*i_allClustersID);
  }

  info() << "Got " << nonmuon_clusters_id.size()
         << " ID Hadron Clusters No Muon" << endmsg;

  // This is inefficient, but this is the only way I know how to offload this
  if (excludeProng) {
    Minerva::IDClusterVect tmp_nonmuon_clusters_id;
    const Minerva::IDClusterVect excludeClustersID =
      excludeProng->getAllIDClusters();
    debug() << "Got " << excludeClustersID.size() << " ID Exclude Clusters"
            << endmsg;

    // Minerva::IDClusterVect::iterator i_nonmuon_clusters_id =
    // nonmuon_clusters_id.begin();
    for (uint i = 0; i < nonmuon_clusters_id.size(); ++i) {
      // skip if in hadron cluster
      if (std::find(excludeClustersID.begin(),
                    excludeClustersID.end(),
                    nonmuon_clusters_id[i]) != excludeClustersID.end())
        continue;
  ...
  debug() << "Got " << nonmuon_clusters_id.size() << " ID Hadron Clusters"
          << endmsg;
...
SmartRefVector<Minerva::IDCluster>
MasterAnaDev::getAnalyzableNonMuIDClusters(const Minerva::PhysicsEvent* event,
                                           const SmartRef<Minerva::Prong>& muon,
                                           bool useWideTimeWindow) const
{
  ...
  const SmartRefVector<Minerva::IDCluster> muClusters =
    muon->getAllIDClusters();
  for (SmartRefVector<Minerva::IDCluster>::iterator i = clusters.begin();
       i != clusters.end();) {
...
SmartRefVector<Minerva::ODCluster>
MasterAnaDev::getAnalyzableNonMuODClusters(const Minerva::PhysicsEvent* event,
                                           const SmartRef<Minerva::Prong>& muon,
                                           bool useWideTimeWindow) const
{
  // get in-time clusters with analyzable type
  ...
  // remove muons clusters
  const SmartRefVector<Minerva::ODCluster> muClusters =
    muon->getAllODClusters();
  for (SmartRefVector<Minerva::ODCluster>::iterator i = clusters.begin();
       i != clusters.end();) {
    if (find(muClusters.begin(), muClusters.end(), *i) != muClusters.end())
      i = clusters.erase(i);
    else
...
double
MasterAnaDev::getRecoilEnergy(const Minerva::PhysicsEvent* event,
                              const SmartRef<Minerva::Prong>& muon,
                              const string& channel,
                              Minerva::NeutrinoInt* ccqeHyp) const
{
  bool useWideWindow = true;
  SmartRefVector<Minerva::IDCluster> idClusters =
    getAnalyzableNonMuIDClusters(event, muon, useWideWindow);
  SmartRefVector<Minerva::ODCluster> odClusters =
    getAnalyzableNonMuODClusters(event, muon, useWideWindow);
  double caloEwide =
    m_caloUtils->applyCalConsts(idClusters, odClusters, channel);
...
double
MasterAnaDev::getvisibleenergy(const Minerva::PhysicsEvent* event,
                               const SmartRef<Minerva::Prong>& muon) const
{
  bool useWideWindow = true;
  SmartRefVector<Minerva::IDCluster> idClusters =
    getAnalyzableNonMuIDClusters(event, muon, useWideWindow);
  ...
  double totalE = 0.0;
  double tempE = 0.0;
  for (SmartRefVector<Minerva::IDCluster>::iterator idClus = idClusters.begin();
       idClus != idClusters.end();
       ++idClus) {
    SmartRefVector<Minerva::IDDigit> digits = (*idClus)->digits();

    for (SmartRefVector<Minerva::IDDigit>::iterator itDig = digits.begin();
         itDig != digits.end();
         ++itDig) {
      tempE =
        (*itDig)->normEnergy() * (*idClus)->getDigitOwnershipFraction(*itDig);
      totalE += tempE;
    }
  }

  for (SmartRefVector<Minerva::ODCluster>::iterator odClus = odClusters.begin();
       odClus != odClusters.end();
       ++odClus) {
    SmartRefVector<Minerva::ODDigit> digits = (*odClus)->digits();

    for (SmartRefVector<Minerva::ODDigit>::iterator itDig = digits.begin();
         itDig != digits.end();
         ++itDig) {
      tempE =
        (*itDig)->normEnergy() * (*odClus)->getDigitOwnershipFraction(*itDig);
      totalE += tempE;
    }
  }
...
int
MasterAnaDev::GetTargetFromSegment(int segment,
                                   int& vtx_module,
                                   int& vtx_plane) const
{
  int targetID = -1;
  ...
  if (targetID == -999)
    warning() << "Can't find any module/plane for given segment " << segment
              << endmsg;
  return targetID;
}
...
MasterAnaDev::discrPairToPaddle(unsigned int minervaID) const
{
