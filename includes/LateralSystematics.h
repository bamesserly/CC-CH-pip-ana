...
class BirksShiftUniverse : public CVUniverse {
 public:
  ...
  virtual double GetTpi(RecoPionIdx hadron) const override {
    double shift_val = GetVecElem("MasterAnaDev_pion_E_Birks", hadron);
    return shift_val + CVUniverse::GetTpi(hadron);
  }

  virtual double GetdEdxScore(RecoPionIdx hadron) const override {
    double shift_val = GetVecElem("MasterAnaDev_piFit_score1_Birks", hadron);
  ...
  virtual double GetdEdxScore(int hadron) const {
    double shift_val =
        m_nsigma > 0
            ...
            : GetVecElem("MasterAnaDev_piFit_score1_BetheBloch_biasDown",
                         hadron);
    return shift_val + CVUniverse::GetdEdxScore(hadron);
  }
...
class DetectorMassShiftCVUniverse : public CVUniverse {
 public:
  ...
  virtual double GetTpi(int hadron) const {
    double shift_val =
        m_nsigma > 0 ? GetVecElem("MasterAnaDev_pion_E_Mass_biasUp", hadron)
...
class TrackAngleShiftCVUniverse : public CVUniverse {
 public:
  ...
  virtual double GetThetapi(int hadron) const {
    double shift_val =
        (CCNuPionIncShifts::pion_angle_res) * (gRandom->Gaus(0.0, 1.0));
...
class BeamAngleShiftCVUniverse : public CVUniverse {
 public:
  ...
  virtual double GetThetapi(int hadron) const {
    double shift_val =
        m_nsigma > 0 ? GetVecElem("MasterAnaDev_pion_theta_biasUp", hadron)
 ...
 public:
  ...
  virtual double GetWeight() const override {
    ...
    if (CVUniverse::GetPionCandidates().size() > 0) {
      double Tpi =
          CVUniverse::GetTpi(CVUniverse::GetHighestEnergyPionCandidateIndex(
              CVUniverse::GetPionCandidates())) /
          1000;
