...
#define TAU \
  6.283185307179586476925287  // also known as "two pi" to the unenlightened
 ...
 */
bool xyz2rgb(unsigned char* r, unsigned char* g, unsigned char* b, double x,
             double y, double z, bool clipOutOfGamut = false) {
  double rl = 3.2406 * x - 1.5372 * y - 0.4986 * z;
  double gl = -0.9689 * x + 1.8758 * y + 0.0415 * z;
  double bl = 0.0557 * x - 0.2040 * y + 1.0570 * z;
  bool needsClip =
      (rl < 0.0 || rl > 1.0 || gl < 0.0 || gl > 1.0 || bl < 0.0 || bl > 1.0);
  if (needsClip) {
 ...
 * Returns whether the value was out of the RGB gamut
 */
bool lab2rgb(unsigned char* R, unsigned char* G, unsigned char* B, double l,
             double a, double b, bool clipOutOfGamut = false) {
  double x, y, z;
  lab2xyz(&x, &y, &z, l, a, b);
  return xyz2rgb(R, G, B, x, y, z, clipOutOfGamut);
}

/* Convert from a qualitative parameter c and a quantitative parameter l to a
 * 24-bit pixel These formulas were invented by me to obtain maximum contrast
 * without going out of gamut if the parameters are in the range 0-1
 */
