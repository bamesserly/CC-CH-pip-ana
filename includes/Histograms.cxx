...
Histograms::Histograms(const Histograms& h)
    : m_label(h.m_label),
      ...
      m_bg_subbed_data    (MH1D(&h.m_bg_subbed_data)),
      m_cross_section     (MH1D(&h.m_cross_section)),
      m_effden            (h.m_effden ),
      m_efficiency        (MH1D(&h.m_efficiency)),
      m_effnum            (h.m_effnum ),
      m_migration         (h.m_migration ),
      m_selection_data    (MH1D(&h.m_selection_data) ),
      m_selection_mc      (h.m_selection_mc),
      m_tuned_bg          (MH1D(&h.m_tuned_bg) ),
      m_unfolded          (MH1D(&h.m_unfolded) ),
      m_wsideband_data    (MH1D(&m_wsideband_data   )),
      m_wsidebandfit_data (MH1D(&m_wsidebandfit_data)),
