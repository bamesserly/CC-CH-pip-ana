...
double CVUniverse::GetPTmuonTrue() const {
  return sqrt(pow(GetDouble("truth_muon_px"), 2.0) +
              pow(GetDouble("truth_muon_py"), 2.0));
}
double CVUniverse::GetPZmuonTrue() const { return GetDouble("truth_muon_pz"); }
double CVUniverse::GetEmuTrue() const {
  return sqrt(pow(GetPmuTrue(), 2.0) + pow(MasterAnaDevConsts::MUON_MASS, 2.0));
}
...
double CVUniverse::GetTpi(RecoPionIdx hadron) const {
  ...
  //          - MasterAnaDevConsts::CHARGED_PION_MASS)/0.96;
  ...
  return Epi - MasterAnaDevConsts::CHARGED_PION_MASS;
}
...
double CVUniverse::GetTpiTrue(TruePionIdx idx) const {
  ...
  return t_pi_E - MasterAnaDevConsts::CHARGED_PION_MASS;
}
...
double CVUniverse::GetEpiTrueMatched(RecoPionIdx hadron) const {
  return GetTpiTrueMatched(hadron) + MasterAnaDevConsts::CHARGED_PION_MASS;
}
  ...
  for (const auto& pi_idx : GetPionCandidates()) {
    ...
    if (abs(GetVecElem("MasterAnaDev_hadron_tm_PDGCode", pi_idx)) ==
        211)  // TODO may want to only not add pion mass when is proton or
              // neutron
      etracks += MasterAnaDevConsts::CHARGED_PION_MASS;
  }
  // std::cout << "\n";
  return etracks;
}
...
double CVUniverse::CalcQ2(const double Enu, const double Emu,
  ...
  double Q2 =
      2.0 * Enu *
          (Emu - sqrt(pow(Emu, 2.0) - pow(MasterAnaDevConsts::MUON_MASS, 2.0)) *
                     cos(Thetamu)) -
      pow(MasterAnaDevConsts::MUON_MASS, 2.0);
  if (Q2 < 0.) Q2 = 0.0;
  return Q2;
}

double CVUniverse::CalcWexp(const double Q2, const double Ehad) const {
  double W = pow(MasterAnaDevConsts::PROTON_MASS, 2.0) - Q2 +
             2.0 * (MasterAnaDevConsts::PROTON_MASS)*Ehad;
  W = W > 0 ? sqrt(W) : 0.0;
  return W;
}
...
double CVUniverse::GetWeight() const {
  ...
  //  wgt_lowq2 = GetLowQ2PiWarpWeight(q2, MasterAnaDevShifts::kLowQ2PiChannel);
  //}
...
int CVUniverse::GetHighestEnergyPionCandidateIndex(
  ...
  if (pion_candidate_idxs.empty()) {
    return MasterAnaDevConsts::kEmptyPionCandidateVector;  // == -2
  }
  ...
  for (uint iter = 0; iter < pion_candidate_idxs.size(); ++iter) {
    ...
    if (current_idx == MasterAnaDevConsts::kIsVertexPion) {
