...
void FractionFit(TH1D* data_hist, TObjArray* mc_templates,
                 ...
                 bool be_verbose, double& midscale, double& hiscale) {
  ...
  for (int w = 0; w != mc_templates->GetEntries(); ++w) {
    ...
    double initial_frac =
        tmp_hist->Integral(lower_limit, upper_limit) / total_integral;
    if (be_verbose)
      std::cout << "initial " << w << " fraction: " << initial_frac
                << std::endl;
    switch (w) {
      case kSigParamId:
  ...
  if (be_verbose)
    std::cout << "    CV fit status " << fit_status << " and chi2/ndf "
              << fit->GetChisquare() << "/" << fit->GetNDF() << std::endl;
  ...
  for (int w = 0; w != mc_templates->GetEntries(); ++w) {
    ...
    if (be_verbose) {
      switch (w) {
        case kLoWParamId:
          std::cout << "fit low fraction: " << fit_frac << " +/- " << fit_err
                    << std::endl;
          std::cout << "fit low scale: " << fit_frac / initial_frac
                    << std::endl;
          break;
        case kMidWParamId:
          std::cout << "fit midw fraction: " << fit_frac << " +/- " << fit_err
                    << std::endl;
          std::cout << "fit midw scale: " << fit_frac / initial_frac
                    << std::endl;
          midscale = fit_frac / initial_frac;
          break;
        case kHiWParamId:
          std::cout << "fit hiw fraction: " << fit_frac << " +/- " << fit_err
                    << std::endl;
          std::cout << "fit hiw scale: " << fit_frac / initial_frac
                    << std::endl;
          hiscale = fit_frac / initial_frac;
          break;
        case kSigParamId:
          std::cout << "fit sig fraction: " << fit_frac << " +/- " << fit_err
                    << std::endl;
          std::cout << "fit sig scale: " << fit_frac / initial_frac
                    << std::endl;
          break;
      }
    }
    ...
    error =
        sqrt(pow(fit_err / N_raw, 2.0) + pow(fit_frac, 2.0) / pow(N_raw, 3.0));
...
double WSidebandFitter::MinimizerFunc(const double* par) {
  ...
  for (int i = 0; i < nbins; ++i) {
    ...
    chi2 += pow((sig_entries + hiW_entries + midW_entries + loW_entries -
                 data_entries),
                2) /
            (pow(error, 2) + pow(mc_error, 2));
    ndf++;
  }
  double tot_chi2 = pow(
      (tot_sig + tot_hiW + tot_midW + tot_loW - tot_data) / sqrt(tot_data), 2);
  m_chi2 = chi2;
  m_ndf = ndf;
  // std::cout << m_chi2 << " " << m_ndf << " " << tot_chi2 << " " << hiW_scale
  // << " " << loW_scale << "\n";
  return chi2;
}
