...
void setLabPalette(double lStart = 0.1, double lEnd = 0.9) {
  // A colour palette based on the La*b* colour space, and stolen from
  ...
  for (int i = 0; i < nCols; ++i) {
    ...
    while (wasClipped && r > 0) {
      ...
      wasClipped = lab2rgb(&red, &green, &blue, l, a, b, true);
      r -= 0.01;
    }
    reds[i] = red / 255.;
    greens[i] = green / 255.;
...
void setCoolWarmPalette() {
  ...
  if (!initialized) {
    gStyle->SetNumberContours(NCont);
    Double_t stops[NRGBs] = {0.,   0.05, 0.1,  0.15, 0.2,  0.25, 0.3,
                             0.35, 0.4,  0.45, 0.5,  0.55, 0.6,  0.65,
                             0.7,  0.75, 0.8,  0.85, 0.9,  0.95, 1.};
    Double_t red[NRGBs] = {0.23,  0.285, 0.348, 0.415, 0.484, 0.554, 0.619,
                           0.688, 0.754, 0.814, 0.867, 0.913, 0.947, 0.966,
                           0.968, 0.957, 0.932, 0.892, 0.839, 0.774, 0.706};
    Double_t green[NRGBs] = {0.299, 0.38,  0.466, 0.547, 0.622, 0.69,  0.744,
                             0.793, 0.83,  0.854, 0.864, 0.837, 0.795, 0.74,
                             0.674, 0.598, 0.519, 0.425, 0.322, 0.2,   0.016};
    Double_t blue[NRGBs] = {0.754, 0.823, 0.888, 0.939, 0.975, 0.996, 0.999,
                            0.988, 0.961, 0.918, 0.863, 0.795, 0.717, 0.637,
                            0.557, 0.477, 0.406, 0.333, 0.265, 0.203, 0.15};
    int colmin =
        TColor::CreateGradientColorTable(NRGBs, stops, red, green, blue, NCont);
    std::cout << "colmin is " << colmin << std::endl;
    for (int i = 0; i < NCont; ++i) colors[i] = colmin + i;
