...
void FillCounters(const CCPiMacroUtil& util, CVUniverse* universe,
  ...
  TH1D* h_total_wgts =
      new TH1D(Form("total_weights_%i", type), "total_weights", 50, -1., 5.);
  TH1D* h_total_check_wgts = new TH1D(Form("total_check_weights_%i", type),
                                      "total_check_weights", 50, -1., 5.);
  TH1D* h_flux_wgts =
      new TH1D(Form("flux_weights_%i", type), "flux_weights", 50, -1., 5.);
  TH1D* h_genie_wgts =
      new TH1D(Form("genie_weights_%i", type), "genie_weights", 50, -1., 5.);
  TH1D* h_rpa_wgts =
      new TH1D(Form("rpa_weights_%i", type), "rpa_weights", 50, -1., 5.);
  TH1D* h_nrp_wgts =
      new TH1D(Form("nrp_weights_%i", type), "nrp_weights", 50, -1., 5.);
  TH1D* h_2p2h_wgts =
      new TH1D(Form("2p2h_weights_%i", type), "2p2h_weights", 50, -1., 5.);
  TH1D* h_mueff_wgts =
      new TH1D(Form("mueff_weights_%i", type), "mueff_weights", 50, -1., 5.);
  ...
  if (type == kMC) {
    ...
    PlotTH1_1(h_total_check_wgts, Form("wgts_total_check_%s", tag), -1,
              use_log_scale);
  }
}
