...
void plot_models(Plotter p, MnvH1D* data, TH1D* mc, std::string outdir = ".",
  ...
  // mc_xsec->SetTitle("GENIE v3.0.6 - G18_02b");
...
void plotNuisance(int signal_definition_int = 1, int plot_errors = 0) {
  ...
  cout << "Reading data input from " << fin1.GetName() << endl;

  // Nuisance input
  TFile fin2("nuisance/nuisance_ME_FHC_tracker_GENIE_v3_0_6_G18_02a_02_11a_CH_50M.root",
             "READ");
  const std::string model_label = "GENIE v3.0.6 - G18_02a";

  //TFile fin2("nuisance/nuisance_ME_FHC_tracker_GENIE_v3_0_6_G18_02b_02_11a_CH_50M.root",
  //           "READ");
  //const std::string model_label = "GENIE v3.0.6 - G18_02b";

  cout << "Reading model input from " << fin1.GetName() << endl;
  ...
  for (auto reco_var : variables) {
    ...
    TH1D* mc_cross_section = (TH1D*)fin2.Get(var_name.c_str());
    mc_cross_section->SetTitle(model_label.c_str());

    // Plot data + model together
    plot_models(plot_info, reco_var->m_hists.m_cross_section,
                mc_cross_section);
